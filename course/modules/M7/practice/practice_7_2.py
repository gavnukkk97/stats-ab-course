...
Запуск из корня репозитория: python3 course/modules/M7/practice/practice_7_2.py
...
def post_exposure_day_fractions(exposure_hour, days):
    """Доля каждого календарного дня после контакта: окно числителя и знаменателя."""
    return np.clip(np.arange(1, days + 1) - np.asarray(exposure_hour)[:, None] / 24, 0, 1)
...
# Интегрируем ту же интенсивность сессий только за post-exposure окно,
# включая неполный день первого контакта. Доэкспозиционные сессии не входят.
sessions_full_days = RNG.poisson(0.8, size=(N_USERS, DAYS))
# Независимый поток для прореживания сохраняет воспроизводимость остальных логов.
sessions_ud = np.random.default_rng(7201).binomial(
    sessions_full_days, post_exposure_day_fractions(exposure_hour, DAYS)).sum(axis=1)
...
fig.tight_layout()
fig.savefig(IMAGE_DIR / "practice_7_2_dedup.png", dpi=150)
plt.close(fig)
...
for K in K_LIST:
    ...
    m_a, m_b = s_a[n_a > 0] / n_a[n_a > 0], s_b[n_b > 0] / n_b[n_b > 0]
...
def welch_from_summaries(n_a, sum_a, sq_a, n_b, sum_b, sq_b):
    """Точная реконструкция Welch по n, sum, sumsq независимых пользователей.

    Бакеты служат хранением достаточных статистик; не считаются равновесными юнитами.
    При больших средних используйте стабильное объединение центральных моментов.
    """
    na, nb = np.sum(n_a), np.sum(n_b)
    ma, mb = np.sum(sum_a)/na, np.sum(sum_b)/nb
    va = (np.sum(sq_a) - na*ma**2)/(na-1)
    vb = (np.sum(sq_b) - nb*mb**2)/(nb-1)
    se2 = va/na + vb/nb
    dof = se2**2 / ((va/na)**2/(na-1) + (vb/nb)**2/(nb-1))
    statistic = (mb-ma)/np.sqrt(se2)
    return mb/ma-1, statistic, float(2*stats.t.sf(abs(statistic), dof))

sq_a = np.bincount(buckets[1000][~group_b], weights=gmv_a**2, minlength=1000)
sq_b = np.bincount(buckets[1000][group_b], weights=gmv_b**2, minlength=1000)
exact_lift, exact_t, exact_p = welch_from_summaries(n_a,s_a,sq_a,n_b,s_b,sq_b)
assert np.isclose(exact_t, t_user.statistic)
results.append({"путь": "n/sum/sumsq (точно)", "строк на анализ": 2000,
                "оценка lift GMV": f"{exact_lift:.3%}", "p-value": f"{exact_p:.4f}",
...
print("     bucket means — приближение; один seed не доказывает сохранение уровня и мощности.")
...
# условно один GROUP BY на эксперимент, дальше статистика по 2K
# строк. Это модель стоимости по замеру одной метрики; 300 разных срезов не исполняются.
# Время одной операции = лучшая из 30 попыток; подготовка разных срезов стоит дополнительно.
...
fig.tight_layout()
fig.savefig(IMAGE_DIR / "practice_7_2_buckets.png", dpi=150)
plt.close(fig)
...
fig.tight_layout()
fig.savefig(IMAGE_DIR / "practice_7_2_partitions.png", dpi=150)
plt.close(fig)
