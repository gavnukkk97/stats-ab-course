...
Запуск из корня репозитория: python3 course/modules/M6/practice/practice_6_1.py
...
print(f"\n   агрегат (T против C по всем)        : {naive:+.4f} = {naive*100:+.1f} п.п.")
print(f"   внутри iOS                          : {diff_ios:+.4f} = {diff_ios*100:+.1f} п.п.")
print(f"   внутри Android                      : {diff_andr:+.4f} = {diff_andr*100:+.1f} п.п.")
...
for ax, (plat, sub) in zip(axes[:2], df.groupby("platform")):
    ...
    d = rates[1] - rates[0]
    ax.set_title(f"{plat.upper()} (n={len(sub):,}): эффект {d*100:+.1f} п.п.", fontsize=11)
    ax.set_ylabel("конверсия в заказ")
    ax.set_ylim(0, 0.145)
    ax.spines[["top", "right"]].set_visible(False)

fig.suptitle(
    f"Внутри обеих платформ новая выдача ХУЖЕ ({TRUE_EFFECT*100:+.1f} п.п.), "
    f"но в агрегате {naive*100:+.1f} п.п.:\nплатформа — конфаундер (iOS раньше получили выдачу и выше конверсия)",
    fontsize=12, fontweight="bold")
fig.tight_layout(rect=(0, 0, 1, 0.86))
fig.savefig(IMAGE_DIR / "practice_6_1_simpson.png", dpi=150)
plt.close(fig)
...
fig.savefig(IMAGE_DIR / "practice_6_1_collider.png", dpi=150)
plt.close(fig)
...
from causal_utils import path_blocked
...
assert d_separated(coll + [("c", "desc")], "a", "b", {"desc"}, verbose=False) is False
