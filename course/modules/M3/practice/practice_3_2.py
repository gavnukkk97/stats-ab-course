...
Запуск из корня репозитория: python3 course/modules/M3/practice/practice_3_2.py
...
def p_cluster_se(checks_a, m_a, checks_b, m_b):
    ...
    SE^2 = G/(G-1) * sum_i (S_i - theta*m_i)^2 / M^2 (G кластеров)."""
    z_list, thetas = [], []
    for checks, m in ((checks_a, m_a), (checks_b, m_b)):
        S_i = checks.sum(axis=1)
        M = m.sum()
        theta = S_i.sum() / M
        se2 = len(m) / (len(m)-1) * np.sum((S_i - theta * m) ** 2) / M**2
...
fig.tight_layout()
fig.savefig(IMAGE_DIR / "practice_3_2_aa_levels.png", dpi=150)
plt.close(fig)
...
fig.tight_layout()
fig.savefig(IMAGE_DIR / "practice_3_2_power.png", dpi=150)
plt.close(fig)
