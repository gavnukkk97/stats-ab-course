...
Запуск из корня репозитория: python3 course/modules/M3/practice/practice_3_3.py
...
fig.tight_layout()
fig.savefig(IMAGE_DIR / "practice_3_3_aa_level.png", dpi=150)
plt.close(fig)
...
fig.tight_layout()
fig.savefig(IMAGE_DIR / "practice_3_3_ab_power.png", dpi=150)
plt.close(fig)
...
fig.tight_layout()
fig.savefig(IMAGE_DIR / "practice_3_3_agreement.png", dpi=150)
plt.close(fig)
...
# %% [markdown]
# ## Изменение знаменателя: точная шкала эффекта и пределы t по L
# Контрольный R_A центрирует mean(L_A)=0, но общий знаменатель для SE
# не подходит автоматически при изменившейся частоте заказов.
# %%
xa_demo, ya_demo = np.array([800, 1800, 3000, 2200]), np.array([1, 2, 4, 4])
xb_demo, yb_demo = np.array([1100, 2000, 1700, 3300]), np.array([1, 2, 2, 4])
ra_demo, rb_demo = xa_demo.sum()/ya_demo.sum(), xb_demo.sum()/yb_demo.sum()
mean_lb = (xb_demo-ra_demo*yb_demo).mean()
print(f"Знаменатель изменился: точный ratio effect = {rb_demo-ra_demo:.2f}; "
      f"mean(L_B)/mean(Y_B) = {mean_lb/yb_demo.mean():.2f}; "
      f"деление на mean(Y_A) даёт другое число {mean_lb/ya_demo.mean():.2f}.")
