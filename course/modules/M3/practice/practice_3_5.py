...
2) CUPED руками: theta после вычитания внутригрупповых средних,
...
Запуск из корня репозитория: python3 course/modules/M3/practice/practice_3_5.py
...
def cuped(X, Y, T):
    """ANCOVA-1: общий theta после удаления групповых средних, точная OLS-эквивалентность."""
    xc = X - np.where(T == 1, X[T == 1].mean(), X[T == 0].mean())
    yc = Y - np.where(T == 1, Y[T == 1].mean(), Y[T == 0].mean())
...
ax.legend()
fig.tight_layout()
fig.savefig(IMAGE_DIR / "practice_3_5_se.png", dpi=150)
print("Сохранено: practice_3_5_se.png")

# %% [markdown]
# ## Шаг 2. CUPED руками и через OLS — это одна и та же оценка
# Руками: θ по остаткам X,Y после удаления средних рук, Y_cv = Y − θ·X,
...
fig.tight_layout()
fig.savefig(IMAGE_DIR / "practice_3_5_bias.png", dpi=150)
