...
Запуск из корня репозитория: python3 course/modules/M1/practice/practice_1_3.py
...
for name, d in DISTS.items():
    ...
    for n, ts, es, cov in zip(SAMPLE_SIZES, d["theory_se"], d["emp_se"], d["coverage"]):
        print(f"   {n:>6} | {ts:>10.2f} | {es:>10.2f} | {mc_report(round(cov*N_SIM), N_SIM)}")
...
print(f"   -> эвристический ориентир n: "
      f"{100 * SKEW_CHECK**2:,.0f} наблюдений")
print("   (варианты: n > 122.56·s² — Денг; n > 355·s² — Kohavi et al.)")
print(f"   При n = 1000 < {100 * SKEW_CHECK**2:,.0f} проверяем фактическое покрытие:")
...
fig.tight_layout()
fig.savefig(IMAGE_DIR / "clt_means_histograms.png", dpi=150)
...
fig.tight_layout()
fig.savefig(IMAGE_DIR / "se_scaling_loglog.png", dpi=150)
...
print("   «выглядит нормальным» ≠ «аппроксимация достаточна»; эвристика Денга ~12500 не является гарантией.")
