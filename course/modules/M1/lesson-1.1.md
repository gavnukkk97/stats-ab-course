...
**Цель урока:** выбрать описательную статистику под вопрос и оценить влияние асимметрии, хвостов и малой выборки.
...
**Медиана** — значение, не меньше которого и не больше которого находится хотя бы половина наблюдений. При ties нельзя говорить «ровно 50% строго меньше и 50% строго больше». Для чётного n используем полусумму двух центральных порядковых статистик.
...
Одна закупка на 5 000 ₽ подняла среднее с 745 ₽ по четырём остальным заказам до 1 596 ₽ — в 2,14 раза. Если заменить 5 000 на 500 000 ₽, среднее станет (2 980+500 000)/5=100 596 ₽; медиана останется 760 ₽. Среднее честно учитывает деньги, но не является описанием типичного заказа.
...
![Среднее против медианы на скошенном чеке](images/v01_mean_vs_median.png)
...
Ключевой кейс — доля выручки топ-1% пользователей. Если удалена доля пользователей f с долей выручки q, среднее остальных относительно исходного равно $(1-q)/(1-f)$; относительное падение — $(q-f)/(1-f)$. При f=1% и q=20% это 19,19%, а не ровно 20%. Киты — реальные деньги; исключение меняет метрику и требует бизнес-обоснования.
...
![Эффект кита](images/v02_whale_effect.png)
*Рис. 2. Один корпоративный заказ: среднее −52%, медиана −2,3%. Медиана устойчива к выбросам, среднее — нет.*
...
В [X5 ур.2] это показано симуляцией: при 10 бросках доля орлов гуляет от 0 до 100%, при 10 000 — от 49 до 51%. Мораль: рядом с каждым «средним по сегменту» всегда печатайте $n$, и не используйте универсальный запрет n<30; оцените «шумовую полку» через $\sigma/\sqrt{n}$.

![Закон малых чисел](images/v03_law_small_numbers.png)
...
- Медиана: 5-е из 9 значений = 850 ₽ — снизилась с 870 ₽ на 2,30%.
...
|---|---|---|---|---|
| розница | 6 | 4720 | 787 | 785 |
| b2b | 4 | 15000 | 3750 | 1410 |

«Средний чек b2b в 4,8 раза выше» — но в b2b всего 4 заказа, и 74,7% суммы — один заказ. Закон малых чисел: разброс среднего при STD ≈ 4 500 ₽ и n = 4 — это $\pm 2250$ ₽ только от шума. Так выглядит типичная ловушка дашборда.
...
Сначала выполните самостоятельную версию [`exercise_1_1.py`](exercises/exercise_1_1.py): прогноз → расчёт → письменное решение. Готовый [practice_1_1.py](practice/practice_1_1.py) ниже — разбор для сверки после попытки.

Файл [practice_1_1.py](practice/practice_1_1.py) (percent-формат, открывается в VS Code/Jupyter как ноутбук, запускается и как скрипт: `python3 practice/practice_1_1.py`).
...
**Задача 3.** Продакт смотрит сегмент из 25 новых юзеров: конверсия в первый заказ 8% при платформенной 12%. «Просела на треть, отключаем канал». Обоснуйте контраргумент числом.
...
Для заранее заданной альтернативы «избыток орлов» $P(K\ge7)=176/1024=17{,}1875\%$. Для вопроса о честности без направления учитываем оба хвоста: $P(K\ge7\text{ или }K\le3)=352/1024=34{,}375\%$. Ни один результат не даёт основания отвергнуть честность при α=5%; это не доказательство честности.
</details>

**Задача 5.** На одном наборе из 100 заказов сравните: (а) сумму всей выручки / число заказов; (б) среднее личных средних чеков трёх пользователей. u1: 1 заказ на 11 200 ₽; u2: 9 заказов суммарно на 8 000 ₽; u3: 90 заказов суммарно на 72 000 ₽. Почему оценки разные и на какие вопросы отвечают?

<details><summary>Ответ</summary>

(а) $(11200+8000+72000)/100=912$ ₽ за заказ. (б) $(11200+8000/9+72000/90)/3\approx4296{,}30$ ₽ — средний по пользователям личный средний чек. В первом случае вес пользователя пропорционален числу его заказов, во втором всем дан одинаковый вес. Это разные estimand: AOV и средний пользовательский AOV, а не два способа получить одну метрику.
</details>
