...
Запуск из корня репозитория: python3 course/modules/M2/practice/practice_2_3.py
...
sup_boot = ((b[ib][:, :, None] > a[ia][:, None, :]) + 0.5*(b[ib][:, :, None] == a[ia][:, None, :])).mean(axis=(1, 2))
...
print(f"   AUC = P(B>A)+0.5P(B=A), оценка U/(mn) = {sup_hat:.2f},"
      f" бут-CI [{ci_sup[0]:.2f}; {ci_sup[1]:.2f}]")
print(f"   Отвержения при α={ALPHA}: permutation={p_perm<ALPHA}, Welch={p_welch<ALPHA}, MW={mw.pvalue<ALPHA}")
print("   Сравнивать p как рейтинг методов нельзя: H0 и условия калибровки различаются.")
print("   Обычная перестановка здесь тестирует обменяемость, а не только равенство средних.")
...
fig.tight_layout()
fig.savefig(IMAGE_DIR / "practice_2_3_permutation.png", dpi=150)
...
for k, v in alpha_hat.items():
    ...
    print(f"   {k:<14}: {mc_report(cnt[k], WORLDS)}")
print("   При одинаковых распределениях MW и permutation калиброваны; нулевое распределение")
...
lo, hi = mc_interval(np.array(vals)*WORLDS, WORLDS)
ax.errorbar(range(len(vals)), vals, yerr=[np.array(vals)-lo, hi-np.array(vals)], fmt="none", color="black", capsize=4)
...
ax.set_title("A/A с китами и неравными группами: t-тесты могут нарушать уровень,\n"
             "Манн–Уитни и permutation держат уровень")
fig.tight_layout()
fig.savefig(IMAGE_DIR / "practice_2_3_alpha.png", dpi=150)
...
for positions, values in [(xpos-w/2, body_vals), (xpos+w/2, tail_vals)]:
    values=np.asarray(values); lo,hi=mc_interval(values*WORLDS_POW,WORLDS_POW)
    ax.errorbar(positions, values, yerr=[values-lo,hi-values], fmt="none", ecolor="black", capsize=3)
ax.set_ylabel("мощность; черты — MC 95% CI")
ax.set_title("Кто что видит: ранги сильнее в теле распределения,\n"
             "тесты среднего — в хвосте (эффект-то в деньгах = в среднем)")
ax.legend(fontsize=9)
fig.tight_layout()
fig.savefig(IMAGE_DIR / "practice_2_3_power.png", dpi=150)
...
print(f"   ({alpha_hat['permutation']:.1%}) калиброваны при обменяемости H0.")
...
for scenario, results in hits.items():
    for method, count in results.items():
        print(scenario, method, mc_report(count, WORLDS_POW))
print("Monte Carlo uncertainty of permutation p near .05: SE≈sqrt(.05*.95/B); for B=200 ≈.0154.")
print("Близкие доли мощности не ранжируют методы без учёта MC-интервалов и различий estimand.")
