...
**Цель урока:** разложить вариацию в ANOVA, проверить общую гипотезу и связать сравнение групп с регрессией.
...
$k$ групп → $\binom{k}{2}$ попарных сравнений: 4 группы → 6, 5 групп → 10 (FWER зависит от совместного распределения тестов). Вместо россыпи — одна гипотеза:
...
![ANOVA: межгрупповая и внутригрупповая вариативность](images/v27_anova_ss.png)
*Рис. 1. ANOVA: F = шум между группами / шум внутри групп — во сколько раз различия средних больше случайности.*

![F-распределение при нулевой гипотезе и наблюдаемая статистика](images/v28_anova_f.png)
...
- **гомоскедастичность** (равные дисперсии групп): при неравных $n$ её нарушение коварно — как у t Стьюдента в 2.2. Запасной выход — **Уэлч-ANOVA** (`scipy.stats.f_oneway(..., equal_var=False)` в SciPy ≥1.16). `scipy.stats.alexandergovern` реализует отдельный критерий Alexander–Govern, не Welch ANOVA;
- **нормальность остатков**: при разумных $n$ в каждой группе ЦПТ прощает умеренную скошенность (как в 2.2), на крошечных группах с китами — нет. Запасной выход — **Краскел–Уоллис**: ранговый аналог ANOVA, обобщение Манн–Уитни на $k$ групп. Внимание, ловушка из 2.3 действует и здесь: KW калибруется под равенством распределений, а ранговые различия нельзя универсально интерпретировать как различия медиан.
...
![ANOVA как регрессия с индикаторами групп; добавление ковариаты](images/v31_anova_cuped.png)
*Рис. 3. Мост в М3: ANOVA — это регрессия с dummy; дописали столбец пре-периода — получили ANCOVA/CUPED с CI короче в 1/√(1−ρ²) раз.*

В реальном продукте до конца наблюдения второй заказ может не произойти: это правое цензурирование. Нельзя применять этот пример только к вернувшимся пользователям. Нужен фиксированный горизонт и метрика повторного заказа для всех либо survival-анализ с учётом цензурирования.

## Разбор на данных

A/B/n-тест 4 вариантов пост-заказного экрана, время до второго заказа (дни; в этой учебной модели второй заказ полностью наблюдён у каждого участника), $n = 5$ на группу (учебные числа, чтобы всё сошлось на салфетке):
...
Отвергаем H0: не все средние вариантов одинаковы. Доля объяснённой вариативности $\eta^2 = SS_b/SS_{tot} = 193{,}75/353{,}75 = 0{,}55$ — группа объясняет 55% разброса. Post-hoc (попарные t + Холм): D отличается от A (adj p = 0,006), C (0,015) и B (0,034); A, B, C между собой — нет. Вывод для бизнеса: агрессивный апселл (D) откладывает повторный заказ.

Под нормальным H0 с равными дисперсиями p ANOVA равномерен. Практика оценивает долю отвержений и FWER шести зависимых попарных тестов с Monte Carlo CI. Независимая формула 1−0,95⁶≈26% показана только как ориентир, а не теория для этих пар.

## Подводные камни

1. **Попарные t без поправки при k>2.** Семейный риск растёт; его конкретное значение зависит от зависимости сравнений. Заранее задайте семейство и применяйте Холм/Тьюки/Даннетт при их условиях; значимый omnibus сам по себе не защищает произвольный post-hoc.
2. **«ANOVA значим → все группы разные».** Грозит: хотя бы одна пара различается, но не обязательно все шесть (наши данные: виноват только D). Правильно: post-hoc обязателен; omnibus лишь «разрешает» вглядеться, не штрафуя множественностью (omnibus не заменяет контроль всего семейства попарных выводов).
...
Сначала выполните самостоятельную версию [`exercise_2_5.py`](exercises/exercise_2_5.py): прогноз → расчёт → письменное решение. Готовый [practice_2_5.py](practice/practice_2_5.py) ниже — разбор для сверки после попытки.

Файл: [practice_2_5.py](practice/practice_2_5.py) (percent-формат, numpy/scipy/statsmodels/pandas, seed зафиксирован). Что делает:

1. **ANOVA руками** на 4×5 из разбора: SS, df, MS, F, p (+ $\eta^2$) + сверка с `scipy.stats.f_oneway` — полное совпадение.
2. **Под H0 p равномерен**: 3000 миров × 4 группы: доля p<0,05 у омнибуса ≈ 5%; 6 попарных t без поправки — FWER ≈ 20% (26% — только ориентир независимости, а не точная теория этих пар).
...
Графики: [practice_2_5_anova.png](images/practice_2_5_anova.png) (разложение SS между/внутри), [practice_2_5_puniform.png](images/practice_2_5_puniform.png) (равномерность p под H0 и FWER попарных).
...
$\binom52=10$ сравнений. Из-за общих групп они зависимы; формула $1-0{,}95^{10}\approx40\%$ относится к независимым проверкам, а не является точным FWER всех пар. В полном H0 все полученные открытия ложны; FWER — доля экспериментов хотя бы с одним таким открытием. Для пары решений применяйте Холм/Тьюки с подходящими условиями.
</details>
...
$N=4\cdot8=32$, $df_b=3$, $df_w=N-k=28$; $MS_b=50$, $MS_w=360/28\approx12{,}8571$, $F=3{,}88889$. Критическое F(3,28)≈2,94669, p≈0,019266: отвергаем H0. Нужны скорректированные сравнения, чтобы определить различающиеся пары.
</details>
...
1. k групп дают $\binom k2$ попарных сравнений с общими наблюдениями. Они зависимы; оцените семейный риск и примените подходящий FWER-контроль.
