...
Запуск:  python3 practice_2_2.py
...
fig.suptitle("Уэлч держит номинал всегда; Стьюдент ломается, когда большие сигмы "
             "встречают маленькие n", fontsize=12, fontweight="bold")
fig.tight_layout()
fig.savefig(HERE / "practice_2_2_student_welch.png", dpi=150)
plt.close(fig)
...
fig.tight_layout()
fig.savefig(HERE / "practice_2_2_paired.png", dpi=150)
plt.close(fig)
...
print(f"   правило Денга: для нормальности СРЕДНЕГО нужно ~{100 * SKEW ** 2:,.0f} наблюдений")
...
ax.set_ylabel("фактическая доля p<0.05 под H0")
ax.set_title(f"Скошенный чек (асимметрия {SKEW:.0f}): эмпирическая alpha t-теста плывёт,\n"
             f"пока n << 100·s² ≈ {100 * SKEW ** 2:,.0f}")
ax.legend(fontsize=9)
fig.tight_layout()
fig.savefig(HERE / "practice_2_2_skew_alpha.png", dpi=150)
plt.close(fig)
...
print("1) При равных дисперсиях Стьюдент и Уэлч эквивалентны — оба держат 5%.")
print("2) Неравные дисперсии + разные n: у Стьюдента alpha доезжает до ~37%")
print("   (каждый третий «прокрас» — ложный) или падает почти до нуля —")
print("   в зависимости от того, где шум и где малая группа. Уэлч держит 5%:")
print("   equal_var=False — дефолт без недостатков.")
print("3) Парный анализ режет шум: SD разностей < SD групп при корреляции до/после.")
print("   Один и тот же сдвиг: парный p = 0.012 против непарного 0.44;")
print("   экономия выборки ~x11.")
print("4) На скошенном чеке при n=30 одновыборочный t-тест ошибается втрое")
print("   чаще номинала (15% против 5%); двухвыборочный Уэлч держит номинал")
print("   (разность симметричнее). «Магического n=30» нет, есть правило Денга")
print("   ~100·s². Чек теста — не Shapiro на данных, а A/A-симуляция alpha.")
