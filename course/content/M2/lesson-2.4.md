...
**Цель урока:** понять, почему 20 метрик в одном A/B-тесте почти гарантируют ложное «открытие» (FWER $= 1-(1-\alpha)^m \to 64\%$), уметь выбирать и считать поправки (Бонферрони, Холм, BH) руками и кодом, отличать FWER от FDR и распознавать множественность не только по метрикам, но и по сегментам, моментам проверки (peeking) и садам расходящихся тропинок p-hacking'а.

## Зачем это аналитику

Онлайн-дашборд «ЕдаДома» после теста новой версии главной страницы: конверсия, средний чек, AOV, число заказов, Retention 7/30, CR в повторный заказ, время сессии, CTR баннера, доля доставок вовремя, NPS… — легко набрать 20 метрик. Каждая проверяется на α = 5%. Вопрос на миллион: какова вероятность, что **хоть одна** прокрасится просто от шума, когда эффекта нет вообще? Ответ: $1-(1-0{,}05)^{20} = 64\%$. Не «немного выше 5%», а «почти наверняка» — симулятор АБ прямо называет это примером «20 цветов текста»: кто-то обязательно «откроет» цвет, который ничего не делает. X5 в том же духе: 30 фейковых A/B-тестов дали ~1,5 «значимых» результата; на 18 метрик порог Бонферрони уже не 0,05, а 0,0028.
...
Ожидаемое число ложных прокрасок — просто $\alpha \cdot m$: при 20 метриках в среднем **одна метрика прокрасится в каждом тесте**, даже если ни один эффект не существует. Это и есть «20 метрик — одна прокрасится».
...
Нюанс для честности: формула предполагает независимые тесты. Метрики дашборда коррелированы (одни и те же юзеры) — тогда FWER ниже формульного, но не нулевой: в практике при корреляции 0,5 и $m=20$ эмпирический FWER ≈ 41% вместо 64%. Проблема не исчезает — просто слегка замедляется.

![20 независимых тестов: хотя бы одна ложная «находка» в 64% с](visuals/v21_multiple_testing.png)
...
![Поправки: Бонферрони режет всё; Холм — тот же контроль FWER,](visuals/v22_corrections.png)
...
Механика та же, что и с метриками: одна проверка — α = 5%; серия проверок — $1-(1-\alpha)^{\text{число взглядов}}$, с поправкой на зависимость (взгляды сильно коррелированы, поэтому не все 50%, но стабильно в разы выше номинала). Лечится это иначе — не Бонферрони на взгляды, а специальными методами последовательного тестирования (group sequential, mSPRT, always-valid p-value) — им посвящён урок 3.7. Здесь важен мостик: **подглядывание — это множественная проверка гипотез, растянутая во времени**.

![Подглядывание = множественные сравнения во времени: в A/A-те](visuals/v26_peeking.png)
...
И обратный расчёт на салфетке: сколько метрик можно проверить, чтобы FWER не превысил 20%? $1-0{,}95^m \le 0{,}2 \Rightarrow m \le \ln 0{,}8/\ln 0{,}95 \approx 4{,}3$ — **четыре метрики**. Дальше без поправок нельзя, а с поправками — привет, Холм.
...
5. **Peeking как «мониторинг».** Делают: смотрят p-value ежедневно, останавливаются на первом прокрасе. Грозит: фактическая ошибка I рода 25–40% (X5, Atlamos). Правильно: фиксировать длительность/размер заранее (М3.1) или использовать последовательные методы (М3.7); поправки Бонферрони/Холма на «взгляды» — не решение.
...
Файл: `practice_2_4.py` (percent-формат, numpy/scipy/statsmodels, seed зафиксирован). Что делает:
...
1. Команда проверяет 40 метрик без поправок. Каков FWER? Сколько метрик максимум можно проверить, чтобы FWER не превысил 20%?
...
Три группы → до трёх сравнений: A1 vs A2 (ложно красится в ~α миров — и это ещё не значит, что сплит исправен), фича vs A1, фича vs A2; без поправки FWER = 1−0,95³ ≈ 14%, с поправкой — ещё консервативнее. Плюс трафик делится на три части: мощность каждого сравнения падает (при том же объёме — заметно, см. М3.1). «Надёжность» чинят платформенно: SRM-тест после каждого запуска (порог p < 0,001) и регулярные А/А-тесты аудитом пайплайна (М4.2), а не вторым контролем в каждом эксперименте.
</details>
...
m = 1800 проверок; BH-порог для минимального p — порядка $q/m \approx 3 \cdot 10^{-5}$, так что p = 0,008 «прошёл BH» быть не может (если применялся ко всем 1800) — значит, поправка считалась по какому-то подсемейству. Вопросы: какое семейство фиксировалось до анализа; заявлен ли этот сегмент в дизайн-доке (найденный после данных — HARKing); каков план репликации (новый тест на этом сегменте). Разведочный результат по сегменту — гипотеза для нового эксперимента, а не основание для раскатки.
</details>

## Шпаргалка

1. $FWER = 1-(1-\alpha)^m$: m=20 → 64%, m=40 → 87%; ожидание ложных прокрасок = $\alpha m$.
...
4. Бонферрони: $\alpha/m$, гарантия всегда, но консервативен; корреляция метрик делает его ещё жесте, чем нужно.
5. Холм: сортировка + пороги $\alpha/(m-i+1)$, шаг вниз до первого провала; тот же FWER, мощнее Бонферрони — дефолт для FWER.
6. BH: максимальный ранг с $p_{(k)} \le (k/m)q$; FDR-контроль для десятков гипотез; при полном H0 эквивалентен по уровню, выигрывает мощностью при живых эффектах.
7. Correlated метрики снижают FWER против формулы независимости, но не спасают (ρ=0,5, m=20 → ~41%).
8. Peeking = множественность во времени: ежедневные взгляды → α до 25–40%; лечение — фиксация горизонта или sequential-методы (М3.7), а не «Бонферрони на взгляды».
