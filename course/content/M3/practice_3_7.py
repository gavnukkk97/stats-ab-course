...
2) упрощённый mSPRT (смешанный SPRT на нормальных данных, смесь по сдвигу):
   always-valid p-value = min(1/Lambda); проверяем, что при ежедневных
   проверках A/A-уровень держится (~5%), а граница решимости растёт;
...
Запуск:  python3 practice_3_7.py
...
fig.tight_layout()
fig.savefig(HERE / "practice_3_7_peeking.png", dpi=150)
...
#   Lambda_k = sqrt(tau^2/(tau^2+v_k)) * exp( x_k^2 * tau^2 / (2 v_k (tau^2+v_k)) )
...
# Always-valid p-value: p_tilde_k = min_{j<=k} 1/Lambda_j — его можно
...
print("   Граница растёт ~sqrt(k*ln k): ранние дни труновые, потом мягче —")
...
axes[1].plot(K_DAYS[:40], bound_z[:40], lw=2.6, color="#DD8452", label="mSPRT-граница (растёт)")
...
axes[1].set_title("Граница решимости растёт: ранние дни — самые строгие")
axes[1].legend(fontsize=9, loc="lower right")
fig.suptitle("mSPRT: можно смотреть каждый день — уровень держит растущая граница",
             fontsize=11.5, fontweight="bold")
fig.tight_layout()
fig.savefig(HERE / "practice_3_7_msprt.png", dpi=150)
...
fig.tight_layout()
fig.savefig(HERE / "practice_3_7_speed.png", dpi=150)
