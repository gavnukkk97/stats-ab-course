...
Запуск:  python3 practice_3_6.py
...
# (numpy lstsq) — предсказание вычитаем с весом theta (у обученной на y модели
# он получается ~1). Важно: ВСЕ признаки — пре-периода, поэтому модель можно
# обучать прямо на данных эксперимента: признаки не затронуты тритманом,
# а значит предсказание — легальная ковариата (правило DoorDash CUPAC).
...
beta_ml, *_ = np.linalg.lstsq(FEATS, y, rcond=None)
pred_ml = FEATS @ beta_ml  # CUPAC-ковариата: предсказание метрики по пре-данным
(diff_cupac, se_cupac), theta_ml = cuped(y, treat, pred_ml)
...
print(f"   theta_CUPED = {theta1:.3f}, theta_CUPAC = {theta_ml:.3f}, R2_CUPED = {r2_cuped:.2f}, R2_CUPAC = {r2_cupac:.2f}")
...
print("   а без центрирования/взаимодействий (наивный Freedman-OLS) гарантии нет.")
...
def estimate_leaky(rng):
    ...
    pred_pre = feats_pre @ np.linalg.lstsq(feats_pre, yw, rcond=None)[0]
    out["CUPAC (только пре)"] = dim_estimate(yw - pred_pre, tw)
    feats_leak = np.column_stack([np.ones(2 * N_BIAS), xw1, xw2, w])
    pred_leak = feats_leak @ np.linalg.lstsq(feats_leak, yw, rcond=None)[0]
    out["CUPAC с утечкой (in-experiment)"] = dim_estimate(yw - pred_leak, tw)
...
fig.tight_layout()
fig.savefig(HERE / "practice_3_6_variance_reduction.png", dpi=150)
...
ax.plot(r2s, days_base * (1 - r2s), lw=2.6, color="#4C72B0", label="дни теста = 28 * (1 - R²)")
...
ax.set_xlabel("R² ковариаты (или ML-предсказания) с метрикой")
ax.set_ylabel("дни теста при той же мощности")
ax.set_title("Экономия дней от снижения дисперсии: остаточная σ² × (1 − R²)")
ax.legend(fontsize=9, loc="upper right")
fig.tight_layout()
fig.savefig(HERE / "practice_3_6_days_saving.png", dpi=150)
