...
**Цель урока:** перерасти CUPED с одной ковариатой (урок 3.5) в полный арсенал: CUPAC — ML-предсказание метрики по пре-данным как ковариата (DoorDash), нелинейные поправки на бустинге (Яндекс), «непортимая» регрессионная поправка Lin (2013); понимать, почему ML-ковариата легальна только на пре-признаках, во что превращается выигрыш в днях теста ($\text{дни} \times (1-R^2)$), что такое чувствительность метрики (мощность × распределение эффектов) и когда весь этот арсенал не помогает.

## Зачем это аналитику

«ЕдаДома» тестирует «умную корзину» (пересборка рекомендаций на главном экране). Метрика — выручка юзера за 2 недели теста. Урок 3.5 дал CUPED по одной ковариате «выручка пре-периода»: корреляция с метрикой $\rho \approx 0{,}6$ → $R^2 \approx 0{,}36$ → SE сжимается всего в $1/\sqrt{1-0{,}36} = 1{,}25$ раза. Тест вместо 28 дней — 18. Неплохо, но обидно: в пре-данных юзера ещё заказы, частота визитов, любимые категории, средний чек — сигнала там на $R^2 \approx 0{,}7$. Забрать его целиком линейный CUPED по одной переменной не может.

Второй вопрос — безопасность. Руководитель DATA-команды справедливо боится: «вы натравите ML на A/B-тест, он что-то напредсказывает, и мы перестанем доверять цифрам». Урок отвечает на этот страх точно: где проходит граница между легальной ковариатой и утечкой, и какая регрессионная поправка математически «не может ухудшить» классическую разность средних.
...
- длительность теста: $\text{дни}_{new} = \text{дни}_0 \cdot (1-R^2)$ — дни пропорциональны $n$.
...
![Больше информативных пре-ковариат → сильнее сжатие: от CUPED](visuals/v44_cupac.png)
...
- **модель можно обучать на данных эксперимента** (на таргете $Y$ и пре-фичах) — признаков это не портит: $\hat Y$ остаётся функцией $X_{pre}$, а $X_{pre} \perp T$ по построению;
...
- против переобучения — cross-fitting (обучай на фолдах, предсказывай на чужих) и честный out-of-sample $R^2$, против «протёкших» фич — A/A-тест и дизайн-ревью признаков.
...
### 4. Lin (2013): регрессионная поправка, которую нельзя испортить

История: Freedman (2008) раскритиковал OLS-поправку в рандомизированных экспериментах — при неверной спецификации она может и смещать оценку, и **ухудшать** её точность, а классические SE врут. Lin («Agnotic notes…», AoAS 2013) ответил конструкцией:
...
3. без взаимодействий и центрирования (кейс Freedman) гарантий нет — наивный OLS действительно может сделать хуже.
...
Практический вывод: **связка «ML-ковариата + Lin-скелет» — безопасный максимум**. Даже если модель слабая или ковариаты бесполезны, оценка не разваливается (в практике: Lin на чисто шумовых ковариатах дал SE ровно как у DiM, отношение 1,000). Это же свойство «do no harm» продаёт и свежая волна AI-ковариат (LLM-«двойники» юнитов, arXiv 2606.08853): слабый прогноз не увеличивает асимптотическую дисперсию.

### 5. Один каркас: CUPED/CUPAC/ML-RATE как doubly robust (Jeunen)

Jeunen (arXiv 2603.08370) показывает: разность средних — это IPS-оценка с оптимальным контрольным вариатом, а CUPED, CUPAC и ML-RATE структурно эквивалентны **doubly robust** регрессии: оценка соединяет предсказание модели $\mu(X)$ и известный propensity ($1/2$ у рандомизации). В рандомизированном A/B propensity известен точно, поэтому плохая модель **не смещает** оценку — она лишь не помогает (та же мораль, что у Lin, только из каузальной теории). Это мост к М6: всё семейство «предскажи и вычти» — одна математика с разными $f$.
...
- снижение $\sigma$ (CUPED/CUPAC) сдвигает всю кривую мощности влево — детектится больше эффектов из того же $F(\delta)$; это дешевле, чем наращивать $n$ или резать $\alpha$;
...
- **Ratio-метрики.** CUPED напрямую на числитель/знаменатель не работает — сначала линеаризация (урок 3.3), потом VR (так делали ShareChat: GBDT-контрольные вариаты, −30% данных при той же точности).
- **Кластеризация.** VR применяется на юните рандомизации (урок 3.2), иначе фиктивный выигрыш.

![Дни теста × (1−R²): R² ковариат конвертируется прямо в сэкон](practice_3_6_days_saving.png)
*Рис. 2. Дни теста × (1−R²): R² ковариат конвертируется прямо в сэкономленные дни (из практики).*
...
Из практики `practice_3_6.py` (n = 10 000 на руку, истинный эффект +25 руб. выручки, ковариаты: выручка/заказы/активность пре-периода + нелинейные признаки):

| оценщик | оценка, руб. | SE | CI 95% | сжатие SE |
|---|---|---|---|---|
| DiM (raw) | 26,22 | 1,21 | [23,8; 28,6] | — |
| CUPED (1 ковариата, $R^2 = 0{,}36$) | 25,14 | 0,97 | [23,2; 27,0] | ×1,25 |
| CUPAC (7 признаков, $R^2 = 0{,}72$) | 24,53 | 0,64 | [23,3; 25,8] | ×1,89 |
...
Lin на **чисто шумовых** ковариатах: эффект 26,21, SE 1,21 — отношение к DiM ровно 1,000. Гарантия «нельзя ухудшить» работает: плохие ковариаты бесплатны.
...
| CUPAC (только пре-признаки) | 24,98 | −0,02 | 95,5% |
| CUPAC с утечкой (in-experiment фича) | 22,20 | **−2,80** | **68,3%** |
...
4. **Выкинуть юзеров без пре-данных.** Делают: «новых юзеров мало, уберём из анализа». Грозит: фильтр после рандомизации = отбор, смещение (урок 2.4 про условные выборки). Правильно: импутация средним/нулём + дамми «новый юзер» как ковариата, гибрид pre/in-ковариат (Etsy), или отдельный анализ сегмента.
...
Файл: `practice_3_6.py` (percent-формат, numpy/scipy/matplotlib/pandas, без sklearn — роль «ML» играет `numpy.linalg.lstsq` по 7 признакам, seed зафиксирован). Что делает:
...
4. **Симуляция утечки**: 2000 миров с in-experiment фичей — смещение −2,80 руб., покрытие 68% против 95–96%.
5. **График экономии дней** от $R^2$ с точками CUPED/CUPAC и ориентиром DoorDash (−46%).

Графики: `practice_3_6_variance_reduction.png` (SE-лестница + гистограммы смещения), `practice_3_6_days_saving.png` (дни от $R^2$).

![Лестница DiM → CUPED → CUPAC и цена утечки: ковариата из окн](practice_3_6_variance_reduction.png)
...
$R^2 = \rho^2 = 0{,}36$. SE сжимается в $1/\sqrt{1-0{,}36} = 1{,}25$ раза; эквивалентный трафик $1/(1-0{,}36) = 1{,}56$ (как будто юзеров стало на 56% больше); длительность $\times (1-0{,}36) = 0{,}64$ — 28 дней превращаются в 18.
</details>
...
Ковариата валидна, пока является функцией величин, независимых от тритмента. Предсказание $\hat Y = f(X_{pre})$ — функция только пре-признаков, а они измерены до рандомизации: $X_{pre} \perp T$ по построению, в каком $f$ их ни пропусти (и обученном на чём угодно — на таргете эксперимента в том числе: это меняет $f$, но не зависимость признаков от $T$). In-experiment признак $W$ сам затронут тритманом: $W \not\perp T$, и поправка вычитает из оценки «эффект тритмента на $W$», смещая её к нулю (в практике: 22,2 вместо 25). Смягчение — поправка $\tau(X)$ (Deng KDD'23), но проще не пускать такие фичи.
</details>
...
Ингредиенты: центрирование ковариат, полные взаимодействия $T \times X$, робастные (HC) SE. Гарантии: (1) состоятельность $\hat\tau$ для ATE при любой спецификации ковариат; (2) асимптотическая дисперсия не хуже, чем у разности средних («нельзя ухудшить DiM»); (3) корректные SE при гетероскедастичности. В наивном OLS без взаимодействий (кейс Freedman) гарантии (1)–(2) теряются: при неверной спецификации поправка может смещать оценку и увеличивать её дисперсию.
</details>
...
(а) Оставить в анализе, импутировать ковариаты средним и добавить дамми «новый юзер»: сохраняет рандомизацию, но у новых ковариата слабая — их часть выигрыша теряется; Lin гарантирует, что хуже DiM не станет. (б) Выкинуть новых — нельзя: фильтр после рандомизации меняет выборку и может смещать (доля новых может отличаться по рукам). (в) Гибрид pre + in-experiment ковариат (Etsy): для новых использовать признаки, появившиеся в первые дни теста, с поправкой $\tau(X)$ — закрывает пробел, но требует аккуратности с утечками. Отдельно можно взвешивать по прогнозу дисперсии юнита (Meta) — метод работает и без длинной истории.
</details>
...
Чувствительность = мощность против эффекта × распределение реальных эффектов, $S \sim \int \text{power}(\delta)\,dF(\delta)$. Если у A и $\sigma$ меньше, и эффекты мельче в одной пропорции, то стандаризованные эффекты $\delta/\sigma$ совпадают — чувствительности примерно равны, хотя «по дисперсии» A выглядит лучше. Меньшая дисперсия выгодна только против эффектов фиксированного размера; решение о метрике принимается по ожидаемому числу детектируемых эффектов, а не по σ alone. Именно поэтому Денг (гл. 10) настаивает: чувствительность метрики — про пару «мощность × распределение эффектов».
</details>

## Шпаргалка

1. Многоковариатный CUPED: $\mathrm{Var} \times (1-R^2)$; SE × $\sqrt{1-R^2}$; эквивалентный трафик $1/(1-R^2)$; дни теста × $(1-R^2)$.
2. CUPAC (DoorDash): ковариата = ML-предсказание $\hat Y$ по пре-признакам; модель любая, цель — out-of-sample $R^2$; обучать можно и на эксперименте, признаки — только пре. Результат: −46% длительности.
...
4. Lin (2013): центрирование + взаимодействия $T{\times}X$ + HC-SE ⇒ состоятельность всегда, дисперсия ≤ DiM («нельзя ухудшить»); ExP/ByteDance: раздельные коэффициенты по рукам = «правильный прод-CUPED».
5. Jeunen: CUPED/CUPAC/ML-RATE — одно семейство doubly robust оценок; в рандомизированном тесте плохая модель не смещает, а лишь не помогает.
