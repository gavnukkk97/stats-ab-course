...
**Цель урока:** научиться осознанно выбирать юнит рандомизации (юзер, сессия, заказ, кластер) и юнит анализа; понять, что происходит со стандартными критериями, когда наблюдения внутри юнита зависимы (занижение SE, ложная значимость), уметь считать ICC, design effect и эффективный размер выборки; освоить кластерные стандартные ошибки как рабочее лекарство.
...
Выбор — не техническая деталь, а решение, от которого зависит, *что вообще независимо в ваших данных*. Рандомизация перемешивает юниты назначения — и только их. Если промо рандомизировано по заказам, то у одного юзера часть заказов со скидкой, часть без: заказы одного человека похожи между собой (бюджет, любимые рестораны), и «независимых» наблюдений у вас ровно столько, сколько юзеров, — а не заказов. t-тест, посчитанный по 30 000 заказов 6 000 юзеров, врёт про свою точность; мы увидим в практике, что его фактический α уезжает с 5% к 25–30%. Это не экзотика: в статье koch-kir «Кластерные эксперименты» ровно этот сюжет — скидка на доставку не меняет число заказов (поюзерная метрика, t-тест корректен), но средний чек (метрика уровня заказа) ломает стандартный критерий.

![Рандомизация даёт независимость ЮНИТАМ (юзерам), а не их зак](visuals/v35_randomization_unit.png)
*Рис. 1. Рандомизация даёт независимость ЮНИТАМ (юзерам), а не их заказам — кластеры зависимы.*
...
Правило, которое снимает 90% споров (koch-kir, «Кластерные эксперименты»): **юнит анализа = юнит рандомизации; MDE и мощность считаются на юните анализа**. Рандомизация гарантирует независимость именно между юнитами назначения — критерий может опереться только на эту независимость. Всё, что мельче, — зависимые обрывки одних и тех же юнитов.
...
![A/A-тест: наивный t по заказам даёт α≈30% (заниженные SE), п](practice_3_2_aa_levels.png)
*Рис. 3. A/A-тест: наивный t по заказам даёт α≈30% (заниженные SE), по юзерам — честные 5% (из практики).*
...
Пример: 30 000 заказов у 5 000 юзеров ($m=6$), $\rho=0{,}44$ → DEFF $= 3{,}2$ → эффективных наблюдений 9 375. Формула размера выборки из урока 3.1 остаётся прежней, только $n$ теперь — эффективное: юзеров понадобится во столько раз больше, во сколько DEFF больше единицы; в практике посчитаем точно. Полезные ориентиры:
...
Это «ratio-оценщик» по кластерам: невязка кластера $S_i - \hat\theta m_i$ — вклад кластера в отклонение, и дисперсия суммы невязок делится на общий размер в квадрате. Дальше как всегда: $z = (\hat\theta_A - \hat\theta_B)/\sqrt{SE_A^2 + SE_B^2}$. При равных $m$ это в точности эквивалентно t-тесту на юзерских средних $\bar y_i$ — «анализ на юните рандомизации» из раздела 1 является частным случаем кластерного подхода.
...
Теперь ICC и DEFF руками. Внутрикластерная дисперсия: каждая пара отклоняется от своего среднего на ±40 → $MS_W = 40^2 = 1600$. Дисперсия юзерских средних $= \tau^2 + \sigma^2/m = 13\,417$ → $\tau^2 = 13\,417 - 800 = 12\,617$; $\widehat{ICC} = 12\,617/(12\,617+1600) = 0{,}89$. DEFF $= 1+(2-1)\cdot0{,}89 = 1{,}89$; предсказанное завышение SE $= \sqrt{1{,}89} = 1{,}37$ — совпало с наблюдённым 1,39 (расхождение — шум шести кластеров). Эффективных наблюдений из 12 заказов: $12/1{,}89 = 6{,}3 \approx$ **6 юзеров**. Двенадцать заказов не стоят двенадцати наблюдений — они стоят примерно шесть.
...
Файл `practice_3_2.py` (percent-формат, numpy/scipy/matplotlib, seed зафиксирован). Модель: юзеры «ЕдаДома» с заказами; чек юзера $= e^{u_i + e_{ij}}$, $u_i$ — «уровень кошелька» (корреляция заказов внутри юзера), $m_i = 1 + \text{Poisson}(5)$ заказов на юзера (в среднем 6), 500 юзеров на группу, 2 000 A/A-прогонов. Четыре блока:

1. **A/A: три критерия** — наивный t по заказам, t по юзерским средним, кластерные SE (ratio-оценщик по кластерным суммам): гистограммы p-value и доли p<0,05 (`practice_3_2_aa_levels.png`).
...
4. **А/B с реальным эффектом +3% к чеку**: мощность трёх критериев (`practice_3_2_power.png`).
