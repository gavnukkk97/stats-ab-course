...
**Цель урока:** понять, почему среднее по выборке — «дрожащая» величина; научиться измерять это дрожание стандартной ошибкой $SE = \sigma/\sqrt{n}$ и превращать её в доверительный интервал; жёстко отделить распределение **данных** от распределения **оценки** и понять, при каких $n$ ЦПТ действительно работает.
...
Отсюда главный вопрос продакта: «средний чек вырос на 40 ₽ — это рост или шум?». Ответить без стандартной ошибки невозможно. Доверительный интервал — это язык, на котором аналитик говорит с бизнесом: не «средний чек 1240 ₽», а «1240 ± 95 ₽, всё, что меньше волатильности — не обсуждаем». И это же фундамент всех АБ-тестов (М2–М3): тест — сравнение двух дрожащих оценок, и кто не понимает дрожь одной, не поймёт и дрожь разности.

Но у ЦПТ дурная слава «магического n = 30». На скошенных денежных метриках (чек, выручка на пользователя) обычный интервал при недостаточных $n$ врёт: в нашей симуляции ниже покрытие упадёт до 76–90% вместо обещанных 95%. Правило, которое мы выучим (эмпирика Алекса Денга: нужно порядка $100\,s^2$ наблюдений, где $s$ — асимметрия), скажет, когда формуле можно верить, а когда — нет. Девиз модуля: **не верь — проверь симуляцией**.
...
Для всего урока рабочая модель «ЕдаДома» (она же в `practice_1_3.py`):
...
![Точность дорожает: SE ~ 1/√n](visuals/v09_se_sqrt_n.png)
...
![ЦПТ на скошенных данных](visuals/v08_clt_intuition.png)
...
Алекс Денг в гл. 8.5 своей книги разбирает этот вопрос через разложение Эджворта: ошибка нормальной аппроксимации на хвостах ведёт себя как $s/\sqrt{n}$. Требуя, чтобы она была меньше 0,25% на 95%-х квантилях, получаем $n > 122{,}56\,s^2$; если дисперсия неизвестна (наш случай), Kohavi et al. выводят $355\,s^2$. Практическое правило Денга: **порядка $100\,s^2$ наблюдений** — «лучшее правило большого пальца, чем $n > 30$».

Примеры из книги (пересказ):

| Метрика | Асимметрия $s$ | Нужно наблюдений |
|---|---|---|
| Нескошенная метрика | ~0 | десятки |
...
Наш чек: $\sigma_{\log} = 1{,}2$ даёт $s = (e^{1{,}44}+2)\sqrt{e^{1{,}44}-1} \approx 11{,}2$, значит для честного нормального интервала нужно порядка $100 \cdot 11{,}2^2 \approx 12\,500$ заказов. В практике ниже увидите: при $n = 1000$ гистограмма средних *выглядит* нормально, а покрытие интервала — 94,0% вместо 95%. Выглядит нормальным ≠ аппроксимация достаточна.

Важно: правило про $s$ относится к среднему. Для разности средних $\Delta = \bar{Y} - \bar{X}$ при равных группах скошенность частично гасится — Денг отмечает, что симметричность $\Delta$ ускоряет сходимость. Для перцентилей (p95, p99) нормальных приближений «из коробки» нет — их мы в уроке 1.4 отдадим бутстрапу.
...
![100 доверительных интервалов](visuals/v10_ci_coverage.png)
...
4. **$t/z$-интервал на скошенной метрике при $n \ll 100\,s^2$.** Делают: CI среднего чека по 100–1000 заказов. Грозит: покрытие ниже номинала — в нашей симуляции 76,5% при $n=5$, 89,4% при $n=100$ и даже 94,0% при $n=1000$: тесты чаще ложатся лицом в грязь, чем обещано. Правильно: проверить правило $100\,s^2$; добрать $n$, умерить хвосты (лог/винзоризация — М3.4) или строить процентильные интервалы (урок 1.4).
...
Файл: `practice_1_3.py` (percent-формат, только numpy/scipy/matplotlib, seed зафиксирован). Что делает:
...
2. **Гистограммы 2×4** (`clt_means_histograms.png`): распределение средних по строкам — чек/доставка, по колонкам — $n$; поверх — нормальная плотность $N(\mu, \sigma^2/n)$ по ЦПТ.
3. **График log-log** (`se_scaling_loglog.png`): эмпирическая SE против $\sigma/\sqrt{n}$.
...
- у чека при $n = 5$ и даже $30$ распределение средних скошено вправо, интервал промахивается в 15–23% случаев; при $n = 1000$ «на глаз» всё нормально, но покрытие 94,0% < 95% — до правила Денга ($\approx 12\,500$) ещё далеко;
...
$100\cdot6^2 = 3600$; для $s = 17{,}9$: $100\cdot17{,}9^2 \approx 32\,000$ (у Денга «более 30 000»). Хвосты напрямую конвертируются в требуемый объём выборки.
</details>
...
4. Скорость сходимости зависит от асимметрии: правило Денга $n \sim 100\,s^2$ (варианты $122{,}56\,s^2$ и $355\,s^2$); «n = 30» — не гарантия.
