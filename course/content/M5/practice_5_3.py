...
Запуск:  python3 practice_5_3.py
...
def expected_loss_grid(aa, ba, ab, bb):
    ...
    t1 = np.trapz(g * stats.beta.cdf(g, ab, bb) * stats.beta.pdf(g, aa, ba), g)
    t2 = np.trapz(g * stats.beta.sf(g, aa, ba) * stats.beta.pdf(g, ab, bb), g)
    return t1 - t2
...
def rope_verdict(d_samples, rope=ROPE, cred=0.95):
    ...
    if lo > rope:
        return "различие подтверждено", lo, hi
...
print(f"   байес о решении (Stucchio): EL(B) = {el_b * 100:.2f} п.п. < порога заботы"
      f" {EPS * 100:.1f} п.п. -> КАТИМ B: даже если ошиблись, в среднем теряем"
      " лишь 0,02 п.п.")
...
ax.legend()
fig.tight_layout()
fig.savefig(HERE / "practice_5_3_posteriors.png", dpi=150)
plt.close(fig)
...
print(
    ...
    "     700 псевдонаблюдений «эксперта» задавили 200 реальных."
)
...
ax.legend()
fig.tight_layout()
fig.savefig(HERE / "practice_5_3_scenarios.png", dpi=150)
plt.close(fig)
...
print("   при n=200 медиана B со спекулятивным априором 0,269 против истины 0,13 —\n"
      "   приор задавил данные; к n=20 000 данные побеждают (0,137 против 0,129 у\n"
...
#  (а) частотный, 1 взгляд на 28-й день: p<0,05 -> альфа = 5%;
#  (б) частотный, ежедневный взгляд: стоп при p<0,05 (урок 3.7);
...
def daily_paths(rng, pA, pB):
    ...
    p_val = 2 * stats.norm.sf(np.abs((conv_B - conv_A) / (se * n_k)))
...
# катим вариант с меньшим EL), но истинный эффект разный:
...
print(
    ...
    "   EL контролирует матожидание потерь, а не частоту ошибок по мирам —\n"
    "   это другое обещание, чем у α (доля ложных побед среди миров)."
)
...
ax.legend(fontsize=8)
fig.tight_layout()
fig.savefig(HERE / "practice_5_3_peeking.png", dpi=150)
plt.close(fig)
