...
**Цель урока:** понять, почему фиксированный A/B-тест — это дорогой способ учиться (половина трафика уходит заведомо худшему варианту, пока тест «созревает»), и уметь измерить эту цену регретом; разобраться в трёх алгоритмах multi-armed bandit — epsilon-greedy, UCB, Thompson sampling (и почему третий «естественно возникает из байеса»: сэмплируем θ из апостериоров и показываем вариант с лучшим сэмплом); научиться выбирать между бандитом и тестом: короткие кампании с короткоживущим трафиком (промо-креативы, заголовки) — бандит; вывод с гарантиями для отчёта — тест; и знать, как бандит ломается о нестационарность (эффект новизны, сезонность) и что с этим делать (скользящее окно, дисконтирование).
...
Но у бандитов есть жёсткая граница применимости: они **оптимизируют, а не выводят**. Бандит не даст вам p-value, доверительный интервал и «доказательство, что креатив X лучше на 2 п.п.» для отчёта — он просто раздавал трафик правильно. Для продуктовых решений с длительными эффектами (новый экран оплаты живёт годами, нужны гарантии, сегменты, guardrails — весь аппарат М3–М4) A/B-тест незаменим. Этот урок — про то, где проходит граница.
...
- **нужен вывод с гарантиями**: «раскатили новый экран оплаты, эффект +0,8 п.п., 95% ДИ [+0,2; +1,4]» — бандит такого не скажет никогда: он не даёт ни p-value, ни несмещённой оценки эффекта (его данные — смесь политики и удачи, оценка по ним смещена — отбор «победителя» из шумных данных, см. winner's curse в 4.5); для отчёта, guardrails, сегментов и долгосрочных эффектов — A/B-тест целиком (М3–М4);
...
- **скользящее окно**: апостериор по последним $W$ показам каждой руки (в практике $W = 1500$: доля испорченной руки упала до 3%, регрет второй половины — 37 конверсий, в 9 раз меньше);
...
![Мир меняется — бандит должен забывать: скользящее окно проти](practice_5_4_nonstationary.png)
...
![Цена знания против цены решения: A/B платит regret за тест, ](visuals/v67_regret.png)
*Рис. 3. Цена знания против цены решения: A/B платит regret за тест, Thompson — учится на ходу.*

## Разбор на данных

Из практики `practice_5_4.py` (seed 54; руки 10% / 12% / 14%, 500 прогонов).
...
3. **Наивный Thompson на дрейфующую метрику.** Грозит: 345 конверсий регрета во второй половине вместо 37 — бандит «помнит» устаревший мир (в практике 73% трафика испорченной руке). Правильно: скользящее окно/дисконтирование с периодом короче скорости дрейфа; для сезонности — контекст (день недели/время) в модели.
...
Файл: `practice_5_4.py` (percent-формат, numpy, seed, Agg, без scipy — бандит это цикл). Что делает:
...
3. **Нестационарность**: лучшая рука портится на середине (14% → 6%) — наивный Thompson против скользящего окна W=1500: регрет второй половины (345 против 37) и доля показов испорченной руки (73% против 3%).

Графики: `practice_5_4_regret.png` (регрет линейный против логарифмического + переливание трафика), `practice_5_4_promo.png` (потерянные конверсии кампании), `practice_5_4_nonstationary.png` (реакция на слом: наивный vs окно).
...
ε-greedy с постоянным ε обязан отдавать 10% показов равномерно всем рукам — в том числе худшим — всегда, как бы ни были уверены данные (в практике доля лучшей руки сходится к ~90%, а не 100%). У Thompson вероятность показать руку = апостериорной вероятности, что она лучшая: с ростом данных вероятность аутсайдеров стремится к нулю (апостериоры расходятся и перестают перекрываться), и исследование затухает само — без параметров.
</details>
...
Средние 0,140 и 0,100, SE разности ≈ sqrt(0,12/5000 + 0,09/5000) ≈ 0,0066 → z ≈ 6. P(сэмпл второй руки выше) = Φ(−6) ≈ 1e−9 — практически никогда: апостериоры почти не перекрываются. Пока вероятность не ноль, показы аутсайдеру — «страховка»: если бы наше prior о мире был неверен (рука дрейфует, данные были артефактом), Thompson заметил бы это; плата за страховку при z=6 — один показ на миллиард. Исследование исчезает ровно с той скоростью, с которой растёт уверенность.
</details>
...
2. Fixed A/B: регрет растёт линейно (600 конверсий на 30 тыс. в практике), треть трафика вечно у худшей руки. Бандиты: логарифмически (Thompson — 57).
...
7. Нестационарность: наивный Thompson помнит всё (регрет 2-й половины 345 в практике); скользящее окно W=1500 перестраивается за ~W показов (37 конверсий; доля испорченной руки 3% против 73%); альтернатива — дисконтирование; окно короче скорости дрейфа.
