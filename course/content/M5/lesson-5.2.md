...
**Цель урока:** научиться безошибочно переводить между частотным и байесовским языками — «в 95% таких интервалов…» против «с вероятностью 95% параметр здесь»; получить главный инструмент коммуникации P(B>A) и словарь его соответствия с p-value; научиться считать base rate на пальцах (почему «тест с точностью 95%» при редком событии ловит каждого 11-го) и понять, где байесовские вероятности легитимны, а где их выдают под видом гарантий.
...
Байесовский credible interval говорит ровно то, что хочет услышать бизнес — и делает это честно, потому что в байесовской оптике параметр действительно случаен. Плюс два бонуса, которых у частотника нет вовсе: $P(B>A)$ — «вероятность, что новый баннер лучше старого», прямой ответ на вопрос, который все задают аналитику; и профилактика base rate fallacy — той самой ошибки «точность 99%, значит почти всё поймали», на которой горят антифроды, скоринги и скрининги.

И рамка дисциплины: этот урок — про *перевод*, а не про «байес лучше». При плоском априоре credible и CI — это почти одни и те же числа (проверим в практике); различие — в том, что вам разрешено про них говорить.
...
Почему числа совпали? Плоский априор ничего не добавил, и апостериор оказался «данными в байесовской упаковке»; равносторонний credible Beta почти повторяет интервал Вильсона (расхождение в практике — 0,002 п.п.). Но это совпадение хрупкое: с информативным априором Beta(8,92) (историческая конверсия 8%) credible уезжает в [9,81%; 13,60%] — а частотный CI остался при своих, ему неоткуда взять историю. Числа совпадают только когда байесовец отказался от своего главного ингредиента.

![Одно число, две валюты: гарантия CI — на ансамбль, вероятнос](visuals/v63_ci_vs_credible.png)
...
Три чтения таблицы. Первое: при плоских априорах $P(B>A) \approx 1 - p_{\text{одностор}}$ — это **перевод, а не новая информация**: «92%» звучат увереннее «p = 0,17», но содержат ровно то же. Второе: психологически перевод решает — «p = 0,17» стейкхолдер слышит как «тест провален», «вероятность 91%, что баннер лучше» — как «почти наверняка лучше», и это правильная коммуникация одного и того же факта. Третье: настояще-новое у байеса — вопросы, которых у p-value нет вовсе: $P(B > A + 0{,}5\ \text{п.п.})$, ожидаемый лифт $E[B-A]$, риск просадки $P(B < A - 1\ \text{п.п.})$, expected loss как правило остановки (урок 5.3).

![«92% что B лучше» звучит увереннее p=0.17 — но это переформу](visuals/v64_pb_greater_a.png)
*Рис. 2. «92% что B лучше» звучит увереннее p=0.17 — но это переформулировка одного и того же.*
...
![Естественные частоты на 10 000: 95 больных из 1 085 положите](practice_5_2_base_rate.png)
...
1. **Регуляторика**: клинические испытания, значимые продуктовые решения с внешним аудитом — регулятор требует контролируемых error rates на ансамбле исследований, «субъективная вероятность при моём априоре» не принимается.
...
Файл `practice_5_2.py` (percent-формат, numpy/scipy/matplotlib/pandas, seed зафиксирован). Три блока:

1. **CI vs credible на одной задаче** (120/1000): Вальд, Вильсон, credible при трёх априорах (`practice_5_2_ci_vs_cri.png`).
2. **Base rate**: естественные частоты на 10 000 + кривая PPV от превалентности + антифрод «ЕдаДома» (`practice_5_2_base_rate.png`).
3. **P(B>A) Монте-Карло** (600/5000 против 645/5000) против p-value + словарь перевода + чувствительность к скептическому априору (`practice_5_2_p_b_over_a.png`).
...
- P(B>A) = **91,4%** при p = 0,173 — и ровно 1 − p_одностороннее = 91,4%: перевод без потерь; словарь: p = 0,60/0,32/0,13/0,047/0,013/0,003 ↔ P(B>A) = 70/84/93/98/99,3/99,8%;
...
Больны 100 → пойманы 99. Здоровы 99 900 → ложных тревог 999. P(болен|+) = 99/(99+999) = 99/1098 ≈ **9,0%**. «Точность 99%» на редком событии означает: из 11 позитивов болен один. Отсюда двухступенчатая схема тестирования: первый тест режет 99,9% до ~1,1%, второй (на этой подгруппе превалентность уже ~9%) даёт PPV ≈ 91%.
</details>
...
При плоских априорах P(B>A) ≈ 1 − p/2 = 1 − 0,03 = 97% — прав. Информативный априор (общий для групп) сожмёт разность к нулю: P(B>A) уйдёт ниже 97% (в практике урока вес, равный выборке, скинул 91,4% → 83,5%). Продакту: «вероятность, что B лучше — около 97%, но ожидаемый эффект мал (смотри E[B−A] и CI), а вероятность большого выигрыша — смотри P(B > A + δ)». Одна цифра P(B>A) без эффекта — пол-отчёта.
</details>
...
4. $P(B>A) \approx 1 - p_{\text{одностороннее}}$ при плоских априорах — перевод, не новая информация; словарь: p=0,32↔84%, p=0,17↔91%, p=0,047↔98%.
...
9. Правило Кромвеля: априорной вероятности 0 не бывает; данные перевешивают любое убеждение, но сильное — медленно (вес 5000 юзеров скинул 91,4% лишь до 83,5%).
...
- Контраст с пройденным: урок 1.3 (анатомия CI и ансамблевая гарантия; обещание про credible), урок 2.1 (p-value = вероятность данных при H0; таблица заблуждений), урок 3.7 (peeking — почему останавливаемость не бывает бесплатной), урок 4.4 (коммуникация результатов стейкхолдерам — сюда ложится язык P(B>A)).
