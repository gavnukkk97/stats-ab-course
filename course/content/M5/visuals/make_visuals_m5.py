...
# Единый стиль: заголовок = вывод. Запуск: python3 make_visuals_m5.py
import matplotlib
matplotlib.use("Agg")
...
fig.tight_layout(); fig.savefig("v60_posterior_updating.png", bbox_inches="tight"); plt.close(fig)
...
fig.tight_layout(); fig.savefig("v61_prior_washout.png", bbox_inches="tight"); plt.close(fig)
...
fig.tight_layout(); fig.savefig("v62_base_rate.png", bbox_inches="tight"); plt.close(fig)
...
fig.tight_layout(); fig.savefig("v63_ci_vs_credible.png", bbox_inches="tight"); plt.close(fig)
...
a_A, b_A = 412, 988
a_B, b_B = 447, 963
ax.plot(xs, stats.beta.pdf(xs, a_A, b_A), color=C_MAIN, lw=2.8, label="A: 412/1400 (29.4%)")
ax.plot(xs, stats.beta.pdf(xs, a_B, b_B), color=C_ACC, lw=2.8, label="B: 447/1410 (31.7%)")
sA = rng.beta(a_A, b_A, 200000)
sB = rng.beta(a_B, b_B, 200000)
pb = np.mean(sB > sA)
ax.fill_between(xs, stats.beta.pdf(xs, a_B, b_B), where=xs > 0.315, color=C_ACC, alpha=0.15)
ax.set_xlabel("конверсия"); ax.set_ylabel("плотность")
ax.set_title(f"P(B > A) = {pb:.0%} — высказывание, которое понимает продакт\n(против «p-value = 0.036»)")
ax.legend(fontsize=9)
fig.tight_layout(); fig.savefig("v64_pb_greater_a.png", bbox_inches="tight"); plt.close(fig)
...
ax.set_ylabel("плотность (из MC-сэмплов)")
ax.set_title("Expected loss: решение останавливаться, когда жалко даже ошибиться")
ax.legend(fontsize=9)
fig.tight_layout(); fig.savefig("v65_expected_loss.png", bbox_inches="tight"); plt.close(fig)
...
ax.legend(fontsize=9)
fig.tight_layout(); fig.savefig("v66_thompson_alloc.png", bbox_inches="tight"); plt.close(fig)
...
fig.tight_layout(); fig.savefig("v67_regret.png", bbox_inches="tight"); plt.close(fig)
...
fig.tight_layout(); fig.savefig("v68_shrinkage.png", bbox_inches="tight"); plt.close(fig)
...
fig.suptitle("MCMC на пальцах: блуждание по параметрам, из которого складывается апостериор",
             fontsize=12, fontweight="bold")
fig.tight_layout(); fig.savefig("v69_mcmc_trace.png", bbox_inches="tight"); plt.close(fig)

print("Готово: v60–v69 (10 визуализаций) в", __file__.rsplit("/", 1)[0])
