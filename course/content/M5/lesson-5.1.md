...
**Цель урока:** сменить оптику — научиться думать о параметре (конверсии, CTR, честности монеты) как о случайной величине, а о данных — как о фиксированных; разобрать теорему Байеса на три живые части (априор, правдоподобие, апостериор); овладеть механикой Beta-Бернулли, которая считается в голове («апостериор = априор + данные»), и понять, когда априор — это актив (historical конверсии), а когда — Liability.
...
Все три вопроса — про **вероятности утверждений о параметре**, а частотная статистика такие вероятности не выдаёт принципиально (урок 2.1: p-value — вероятность *данных* при гипотезе, не наоборот). Байесовский вывод — это второй, легальный способ рассуждать о неопределённости: он берёт то, что вы знаете *до* данных (на платформе уже 200 прошлых запусков!), перемножает с тем, что сказали данные, и выдаёт распределение убеждений, из которого прямым счётом берутся ответы на вопросы продакта.

И сразу рамка честности (вернёмся в 5.2–5.3): байес не «лучше» частотного подхода — у него другая валюта. Частотник продаёт **гарантии по ансамблю повторений** (контроль α, мощность — контракт из урока 2.1), байесовец — **вероятностные высказывания об этом параметре** при явно названном априоре. Хороший аналитик владеет обеими и знает, какую когда покупать.
...
![Две оптики: частотник фиксирует параметр и рандомит выборки,](visuals/v70_two_lenses.png)
...
![Апостериор = произведение априора и правдоподобия: живёт там](visuals/v71_prior_times_likelihood.png)
...
|---|---|---|---|
| Неинформативный | Beta(1,1) — равномерный на [0;1]; Jeffreys Beta(0,5; 0,5) | «любое значение доли равновозможно» | нет истории, хотим минимального вмешательства |
...
Три оговорки. Первая: «неинформативного» априора не существует — равномерный на [0;1] сдвигает малые выборки к 50% конверсии; Jeffreys Beta(0,5; 0,5) стабилен к репараметризации, но и он что-то утверждает. Вторая: **empirical Bayes** — априор не «из головы», а восстановленный из данных: берём конверсии 200 прошлых ресторанов, подгоняем Beta (методом моментов или MLE) — получаем Beta(8, 92): среднее 8%, «вес» 100 псевдонаблюдений. Третья: априор имеет **вес** — у Beta(α, β) это α+β «псевдонаблюдений», и этот вес конкурирует с объёмом данных (см. ниже).
...
Ключевой факт: цепочка по дням даёт **ровно тот же** апостериор, что и одно обновление всеми данными сразу (порядок и порции не важны, все члены просто складываются). Это и есть «правильное» изменение мнения по мере поступления данных — то, чего частотный NHST не умеет без специальных поправок (подглядывание, урок 3.7; честный разбор байесовской остановки — 5.3).

![Последовательное обновление: вчерашний апостериор = сегодняш](visuals/v60_posterior_updating.png)
...
Файл `practice_5_1.py` (percent-формат, numpy/scipy/matplotlib/pandas, seed зафиксирован). Монета с истинным p = 0,7 — учебная модель любой доли. Четыре блока:

1. **Последовательное обновление** трёх априоров — Beta(1,1), Beta(5,2), Beta(50,50) — по сетке 1/5/20/100/500 бросков (`practice_5_1_sequential.png`).
...
3. **Чувствительность**: три априора на одном графике при n=100 и n=500 (`practice_5_1_priors_fade.png`).
4. **Empirical Bayes**: 1000 монет с честностями из Beta(6,4), по 5 бросков на монету — сырой MLE против байесовской оценки (`practice_5_1_empirical_bayes.png`).
...
- монетный двор: 14,6% монет получают MLE 0% или 100%; байесовская оценка усаживает их к средней двора, **MSE 0,0446 → 0,0144 (в 3,1 раза точнее)**, а SD разброса оценок 0,258 → 0,086 против истинного 0,146 — байесовская оценка «шумит» даже меньше, чем истинные значения, потому что тянет к среднему. Именно так скорят новеньких «ЕдаДома».
...
1. Оптики: частотник — параметр фиксирован, данные случайны; байесовец — параметр случаен, данные фиксированы. p-value: $P(D \mid H_0)$; байес: $P(H \mid D)$.
...
- Контраст с пройденным: урок 1.2 (распределения, MLE), урок 1.3 (CI и ансамблевая гарантия), урок 2.1 (p-value = вероятность данных при H0), урок 3.7 (подглядывание — почему «смотреть по ходу» в NHST запрещено, а в байесе естественно).
