...
**Цель урока:** превращать результат теста в решение roll/hold/iterate. Читать по-взрослому: оценка эффекта + доверительный интервал + p + контекст мощности (не «прокрасилось/не прокрасилось»; наблюдаемый эффект не сравнивают с MDE — для этого CI). Проверять согласованность иерархии метрик от прокси к деньгам (клики → конверсия → выручка) и уважать decision rule 2×2 из 4.1 (наш кейс: OEC +5,0% значимо, guardrail доставки просел — вердикт ITERATE, а не ROLL). Разбираться с сегментами без самообмана: заранее заявленные, поправка Холма, интеракции в регрессии вместо 12 отдельных тестов — «сегмент-победитель» после факта это множественность + winner's curse (наша A/A-симуляция: из 20 псевдосегментов 1 «значим» сырым анализом, после Холма — 0). Локализовать эффект децильным анализом по пре-метрике (эффект в D9–D10: +11%/+16% — резать хвост нельзя, мостик к 3.4). Переводить статзначимость в практическую (₽, юнит-экономика), оформлять карточку отчёта и собирать финальный пайплайн: логи → метрики → валидация → тесты → локализация → вердикт.
...
2. **Доверительный интервал** — диапазон эффектов, совместимых с данными: $[+65; +114]$ ₽. CI — главный артефакт отчёта: он отвечает на вопрос «а сколько может быть на самом деле», включая худший случай. Нижняя граница CI сравнивается с порогом практической значимости — не p-value.
3. **p-value** — мера несовместимости с нулём, прилагательная к CI, а не замена. $p = 2{,}6\cdot10^{-12}$ не «убедительнее», чем $p=0{,}03$ (айсберг: под H0 p-value равномерен; сверхмалые p — свойство огромных n, а не силы эффекта).
4. **Контекст мощности**: какой эффект тест вообще мог поймать (MDE из дизайн-дока). Мощность интерпретируется **на дизайне**; наблюдаемый эффект с MDE сравнивать нельзя — для этого существует CI. И наоборот: значимость в маломощном тесте подозрительна — отобранные «прокрасы» систематически преувеличены (Type M из 2.1; глубже — в 4.5).
...
После теста с серым OEC рука сама тянется: «а если порезать по платформам/городам/когортам — может, где-то выигрывает?» Это самая опасная кнопка в интерфейсе аналитика. Каждый разрез — новая проверка гипотезы: 15 сегментов при α=5% дают ~54% шанса найти «значимый» сегмент под H0 (2.4); а найденный максимум ещё и **завышен** — среди многих шумных оценок наибольшая почти наверняка удачно отклонилась вверх (winner's curse, он же Type M; мостик к 4.5). Двойной удар: ложное открытие и переоценённый эффект — «раскатим на сегмент X, там же +9%» разочарует даже если эффект там правда есть.
...
![«Техасский снайпер»: нарезка сегментов до нужного результата](practice_4_4_segments.png)
*Рис. 2. «Техасский снайпер»: нарезка сегментов до нужного результата — после поправки Холма победитель исчезает (из практики).*
...
- **связь с обработкой метрики из 3.4**: если эффект живёт в хвосте, агрессивная винзоризация (P95 и жёстче) вырезала бы его из теста — «значимых изменений нет» при +5% ARPU. Обратное тоже верно: эффект в теле — резать хвост безопасно. Перцентильный анализ — это проверка, *что* именно измерял ваш тест;
...
Критическая тонкость: децили — только по пре-метрике или контрольным квантилям. Нарезка по **post-метрике** («дециль выручки в тесте») — это selection: в верхний дециль теста попадут юзеры, чью выручку подняла фича, — «эффект в хвосте» получится артефактом сортировки по результату (collider из М6).

![Децильная локализация по пре-метрике: эффект в D9–D10 — хвос](practice_4_4_deciles.png)
*Рис. 1. Децильная локализация по пре-метрике: эффект в D9–D10 — хвост; помни урок 3.4 про винзоризацию (из практики).*
...
4. **Тесты**: OEC и guardrails — без поправок (guardrail-просадка — без множественных поблажек); вторичные и сегменты — с Холмом (2.4); CI — бутстрепом по юзерам (1.4).
...
Тест **«умные рекомендации в корзине»** (дизайн-док из 4.1): юзеры, открывавшие корзину, 40 000/40 000, 2 недели, α = 5%, MDE 3%, порог практической значимости +3% ARPU. Прогон по пайплайну (практика `practice_4_4.py`):

**Валидация**: SRM 39 923/40 077 (p = 0,59), инвариант «показы на юзера» 8,00 vs 8,00 (p = 0,86) — зелёное, читаем.
...
**Guardrails**: обращения в поддержку 4,92% → 5,21% (p = 0,087) — ок; а вот **доля опоздавших заказов 6,66% → 7,24% (+0,58 п.п., p < 0,001) — просадка**. Механизм виден в иерархии метрик: спрос вырос — нагрузка на курьеров — доставка дольше.

**Secondary (Холм на 3 гипотезы)**: CTR рекомендаций 10,03% → 10,88% (**+8,5%**, p_adj < 0,001 — значим); конверсия в заказ 78,40% → 78,91% (+0,51 п.п., p = 0,48 / Холм 0,57 — не значима); средний чек 1 046,7 → 1 050,3 ₽ (+0,3%, не значим). Цепочка сонаправлена: сильнее всего реагирует механика (клик), слабее — посредники; рост ARPU собран из частоты заказов, чек не тронут — механизм соответствует гипотезе.
...
**Вердикт по decision rule 2×2 (OEC↑, guardrail↓): ITERATE** — не раскатывать: чинить влияние на доставку (порог ожидания рекомендера, предзагрузка зон) и перетестировать. Это и есть урок чтения результатов: p-value шикарный, деньги огромные — а катить нельзя, и это видно только если прочитать **все** числа, а не только «прокрас».
...
Файл: `practice_4_4.py` (percent-формат, numpy/scipy/matplotlib/pandas, seed зафиксирован). Полный мини-пайплайн на синтетике «ЕдаДома»: 141 310 заказов у 80 000 юзеров. Что делает:
...
2. **Валидация**: SRM (p = 0,59), инвариант показов (p = 0,86) — гейт пройден.
3. **Тесты**: OEC ARPU — Уэлч + перцентильный бутстреп-CI по юзерам; guardrails — доля опозданий (просела: 6,66% → 7,24%, p < 0,001) и поддержка (ок); secondary — CTR и чек дельта-методом (3.3), конверсия z-тестом, на троицу — поправка Холма.
4. **Локализация**: эффект по децилям пре-выручки с 95% CI — рост к хвосту: D9 +11,4%, D10 +16,3% (график `practice_4_4_deciles.png`).
5. **Сегменты «до» и «после»**: 12 заявленных сегментов — сырой победитель +8,9% (p = 0,021) умирает после Холма (p = 0,19), завышение против OEC +5,0%; A/A-режим: 20 псевдосегментов под H0 — 1 «значим» сырым анализом, 0 после Холма (график `practice_4_4_segments.png`).
6. **report()**: шаблон-функция карточки для стейкхолдеров (эффект, CI, guardrails, secondary с поправкой, локализация, деньги, рекомендация) + `decide()` — decision rule 2×2 с порогом практичности; вердикт кейса: ITERATE.
...
Статистически — значим (CI не содержит 0). Экономика: 0,004 × 800 000 = 3 200 заказов/мес × 190 ₽ = 608 тыс. ₽/мес — меньше 1,5 млн ₽ поддержки: **не окупается даже по верхней границе CI** (0,007 × 800 000 × 190 = 1,06 млн ₽). Вердикт: HOLD/отказ (или удешевлять поддержку) — при значимом эффекте. Классика «значимость ≠ практическая значимость».
</details>
