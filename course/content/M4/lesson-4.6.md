...
**Цель урока:** знать состояние дел на стыке LLM-агентов и A/B-тестирования по кейсу AgentA/B (arXiv 2504.09723, Amazon & Northeastern): идея «симуляционной фазы» перед живым тестом — LLM-агенты с персонами ходят по живому сайту через Selenium; что показали авторы (сходимость направления эффекта с тестом на 2 млн живых юзеров, χ² p=0,03, экономика ~$2900 за 1000 агентов); трезво оценивать ограничения (персона ≠ живой пользователь, валидность симуляции не доказана) и понимать, когда это пробовать: мало трафика, длинные циклы, дорогая ранняя оценка гипотез до живого теста.
...
1. **Персоны.** LLM генерирует ~100 000 персон (возраст, доход, профессия, привычки в покупках), из них сэмплируют 1000 и раскидывают 500/500 — классический **between-subject** A/B (каждая персона видит один вариант; у агента, как у человека, нельзя «показать оба» и не оставить память о первом).
...
![AgentA/B: персонаны → LLM-агенты → поведение → стат-тест, с ](visuals/v58_agent_ab.png)
*Рис. 1. AgentA/B: персонаны → LLM-агенты → поведение → стат-тест, с валидацией живым A/B.*

### 2. Что показали авторы

Тест-кейс — сокращение панели фильтров: в тритменте выбрасывались опции с похожестью на поисковый запрос ниже 80%. Результаты: агенты тритмента купили **414 раз против 403** в контроле, χ²(1)=5,51, **p=0,03**; средний чек подрос с $55 до $61, незначимо. Параллельно тот же тест крутили на **2 млн живых пользователей** — направление эффекта совпало. Экономика: 1000 агентов обошлись в 875 млн токенов и **~$2900** — заметно дешевле и быстрее, чем две недели набора живого трафика.

Читать это надо аккуратно: одно сравнение на одном сайте, один совпавший знак, p=0,03 — это «многообещающе», а не «доказано». Это претест для приоритизации, а не замена живого теста.
