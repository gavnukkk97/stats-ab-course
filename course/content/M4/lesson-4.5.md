...
**Цель урока:** понимать, почему «сумма зелёных тестов» систематически не равна фактическому приросту метрики — из-за дрейфующего контроля при последовательной раскатке, взаимодействий фич (каннибализация, ограниченное внимание) и winner's curse (Type M: отобранные по значимости «победители» переоценены); уметь построить global/long-term holdout и сверять с ним сумму релизов (кейс Airbnb: +40,65% → +30,62%); считать экономику эксперимента — стоимость задержки релиза против стоимости ошибок I/II рода, цену холдаута, «батчить или тестить по отдельности»; знать, как мета-анализ корпуса тестов и ретесты держат платформу в калибровке, и что такое зрелость experimentation-культуры (buy vs build).
...
- **сумма маргинальных эффектов ≠ суммарный эффект.** Аддитивность — исключение, а не правило: эффекты мультипликативны (перекрываются по аудитории), конкурируют за одно внимание пользователя, а некоторые фичи вообще работают только вместе с другими.
...
![Кто съел мой прирост: наивная сумма релизов vs факт по холда](visuals/v54_holdout_gap.png)
*Рис. 3. Кто съел мой прирост: наивная сумма релизов vs факт по холдауту (кейс Airbnb: +40,7% → +30,6%).*
...
Формально: суммарный эффект набора фич — это $\prod_i (1+\theta_i) - 1$ c поправкой на взаимодействия $\lambda_{ij}$, а не $\sum_i \hat\delta_i$. Взаимодействия в продукте — норма, а не экзотика:
...
Механизм — условие на значимость (тот же «отбор удачного шума», что в peeking из урока 3.7, только на уровне портфеля гипотез). Даже если оценка эффекта в каждом тесте несмещённая, среди прокрасившихся $E[\hat\delta \mid \text{зелёный}] > \delta$: при мощности 80% значимые тесты в среднем завышают эффект примерно на 16% (abba_testing: 0,58 против MDE 0,5), при мощности 20–30% — в разы и до ×16 (тот же разбор: среднее значимых 8,45 при MDE 0,5). Gelman & Carlin называют это Type M-ошибкой; abba предлагает рабочую поправку: $type\_m = E[\text{эффект}]/\text{MDE} - 1$, «перевзвешивай эффект»: $\hat\delta_{\text{corr}} = \hat\delta / (type\_m + 1)$, где коэффициент калибруется симуляцией на исторических данных.
...
![Winner's curse: «зелёные» висят над диагональю — заявленный ](visuals/v55_winners_curse.png)
*Рис. 2. Winner's curse: «зелёные» висят над диагональю — заявленный эффект ×1,2–1,6 выше истинного.*
...
- **Airbnb**: long-term holdout против суммы релизов — расхождение заметное; наивная сумма +40,65% против скорректированной +30,62% (см. выше);
...
- размер и длительность холдаута проектировать **от ширины CI**, которую готовы принять (холдаут шумнее любого launch-теста — см. практику: ±4–8 п.п. на 10 тыс. юзеров в месяц);
- стратифицировать по активности (X5) и не трогать холдаут другими экспериментами — иначе он деградирует в «ещё один слой»;
- сверять не каждый месяц, а по накоплению (квартал–полгода) — и смотреть не только на точечную разницу, а на **разрыв «сумма релизов минус холдаут»** как KPD здоровья платформы: он расползается → interactions/Type M растут → пора чистить портфель фич.
...
**Ретесты** — частный случай воспроизведения (abba: «у меня всё получ... не получилось»). Зелёный тест при ретесте даёт серое по четырём причинам: (1) первая ошибка I рода — вы поймали ложноположительный результат; (2) сместилась аудитория/сезон; (3) novelty effect выдохся; (4) другая методология (фильтры, метрика, юнит). Правила гигиены: ретесты планировать **заранее** (как репликацию, а не как «перекинуть кубик»); фиксировать методологию; смотреть на устойчивость эффекта, а не на один p-value; помнить байесовскую арифметику abba: при базовой доле реальных эффектов 20% три зелёных подряд дают <90% уверенности, что эффект реален.
...
![«⅓ идей выигрывают» (Kohavi KDD'13): платформа экспериментов](visuals/v56_ideas_funnel.png)
*Рис. 1. «⅓ идей выигрывают» (Kohavi KDD'13): платформа экспериментов — машина по отклонению идей.*

## Разбор на данных

Из практики `practice_4_5.py` (все числа воспроизводятся seed'ом).

**Winner's curse (2000 миров по 10 фич).** Портфель года: истинные эффекты от −2% до +4,5%; тест 60 тыс. юзеров на руку при CV метрики 2,3 даёт SE аплифта 1,33 п.п. — мощности от 2% (нулевые фичи) до 92%. В среднем за год зелёных 4,0 из 10. Средний **заявленный** эффект зелёных +4,11% при среднем **истинном** +3,47% у этих же фич — переоценка ×1,18; итог года по отчётам +16,5% против истинных +13,9% — «+2,6 п.п. воздуха». Масштаб согласуется с Airbnb (+40,65% → +30,62% ≈ ×1,3 при более слабых тестах).
...
**Холдаут-сверка (год, 200 тыс. юзеров, 5% холдаут).** Запустились 4 фичи (F2 заявлена +4,2% при истинных +2,5%; F8 заявлена +3,7% при истинных +2,0% — отбор в действии; F9 с истинными +4,0% осталась серой — пропуск из-за каннибализации маргинального эффекта и шума). Итоги года: **наивная сумма истинных эффектов зелёных +13,1%; сумма заявленного по launch-тестам +15,9%; факт по глобальному холдауту: годовой уровень +7,0% (95% ДИ +5,6..+8,4 п.п., p=2e-23), уровень декабря +8,5%**. Из 15,9 заявленных п.п. съедено 7,3–8,9 п.п.: взаимодействия съели 3 п.п. (6 пар по 0,5%), остальное — winner's curse и шум. Сезонность за год качалась от ×0,96 до ×1,12 и **не сместила** холдаут-оценку — обе группы под одним дрейфом (рандомизация). При этом первые 2–3 месяца холдаут «молчит»: 95% ДИ шириной ±4–8 п.п. — это цена 10 тыс. юзеров.

**Экономика теста (пуш-фича: +0,5% недельной выручки = 400 тыс. ₽/нед, приор работы 40%, срок жизни 26 недель, ложный роллаут −0,3% с откатом через 13 недель).** Ожидаемые потери по неделям теста: 1 нед — 3,01 млн (почти всё — риск пропуска: мощность 34%), 3 нед — 1,57 млн, **5 нед — 1,17 млн (минимум)**, 8 нед — 1,39 млн (задержка уже дороже риска). Ожидаемая стоимость ложного роллаута — константа 78 тыс. ₽ при любой длительности (её снижает только α). Мелкая фича (40 тыс. ₽/нед, живёт 8 недель): оптимум 3 недели. α 0,05→0,005: ложный роллаут 2,5%→0,25%, но тест длиннее в 1,7 раза при той же мощности.

## Подводные камни

1. **Суммировать зелёные аплифты в презентации для CFO.** Делают: «10 тестов, сумма +25%». Грозит: обязательства, которые продукт не выполнит (в практике: +15,9% заявлено против +7–8,5% факта). Правильно: рядом с суммой всегда пара чисел — оценка с поправкой на Type M и свежий замер по холдауту.
2. **Холдаут «для галочки»: 0,5% юзеров на две недели.** Грозит: CI шире любого мыслимого эффекта — холдаут молчит даже когда прирост есть (в практике первые месяцы ДИ ±4–8 п.п.). Правильно: проектировать от целевой ширины CI (юзеры × длительность), 1–10% на квартал+, стратификация по активности (X5).
...
Файл: `practice_4_5.py` (percent-формат, numpy/scipy/matplotlib/pandas, seed, Agg). Что делает:
...
3. **Холдаут-сверка**: 200 тыс. юзеров, 95/5, помесячная раскатка только зелёных, каннибализация пар 0,5%, общая сезонность ×0,96–×1,12; помесячная таблица «заявлено накопительно против холдаута ± CI» и три итоговых числа года (+13,1% наивно / +15,9% заявлено / +7,0% по холдауту).
...
Графики: `practice_4_5_winners_curse.png` (зелёные над диагональю), `practice_4_5_interaction.png` (столбики A/B/наивная сумма/факт с CI), `practice_4_5_holdout.png` (помесячная сверка с CI-лентой), `practice_4_5_economics.png` (кривые стоимостей и оптимум).
...
Сложение некорректно по трём причинам: (1) winner's curse / Type M — отобраны значимо-положительные, у них шум «в плюс», заявленные эффекты систематически завышены (Airbnb: сумма +40,65% против несмещённых +30,62%); (2) взаимодействия — каждая фича тестировалась против «мира без неё» на момент запуска, вместе фичи каннибализируются (Eppo: целое меньше суммы частей); (3) маргинальные эффекты поздних фич уже «поедены» ранними. Разрыв 15,9 − 8,5 ≈ 7 п.п. раскладывается на смещение отбора, взаимодействия и шум замера; сам холдаут-ответ несмещён (рандомизация), но шумный — ДИ накрывает и 4, и 13 п.п., поэтому сверку делают по накоплению, а не по одному месяцу.
</details>
...
Запишут +10% (или +10,25% при мультипликативном сложении). Фактически $\left[(1{,}05)(1{,}05)(0{,}94) - 1\right] \approx +3{,}6\%$ — и совместный тест при мощности, спроектированной под +5%, будет серым примерно в половине миров (в практике — зелёный в 52%). Процесс: либо факториальный тест 2×2 с явным членом взаимодействия, либо признать сумму оценкой сверху и сверять пару холдаутом; при дефиците слотов — батчить фичи и мерить батч, жертвуя интерпретируемостью.
</details>
...
(1) Объём и стоимость потока гипотез: сколько тестов в год и какая цена неверного решения — если тестов десятки в квартал и решения дорогие, платформа окупается; ориентиры рынка: своя — от ~150 млн ₽ и 3 года, готовая — от ~2,5 млн ₽/год (Trisigma). (2) Зрелость культуры: есть ли методолог, дизайн-доки, дисциплина метрик — платформа без культуры автоматизирует хаос; 80% гипотез пустые и ~10% негативных — т.е. главная ценность не «катить зелёное», а не катить красное. (3) Кто будет поддерживать SLA сплит-системы, SRM-мониторинг, CUPED/sequential — build означает найм команды экспериментов; иначе buy/поддержка готовой и фокус на методологии (холдауты, мета-анализ).
</details>
...
3. Winner's curse / Type M: отбор по значимости завышает эффекты (×1,16 при мощности 80%, до ×16 при малой); поправка $\hat\delta/(type\_m+1)$, калибровка по корпусу тестов (abba), несмещённая оценка суммы (Airbnb KDD'18: +40,65% → +30,62%).
4. Global/long-term holdout (1–10% на квартал+, стратификация по активности) — несмещённая, но шумная оценка кумулятивного эффекта программы; сверять «сумма релизов vs холдаут» (Airbnb, Disney Universal Holdout, X5 GCG, Netflix decision rules).
