...
Запуск:  python3 practice_4_5.py
...
fig.tight_layout()
fig.savefig(HERE / "practice_4_5_winners_curse.png", dpi=150, bbox_inches="tight")
plt.close(fig)
...
def prop_test(x_c, x_t, n):
    ...
    uplift = p_t / p_c - 1
    se_rel = np.sqrt(p_t * (1 - p_t) / n + p_c * (1 - p_c) / n) / p_c
...
fig.tight_layout()
fig.savefig(HERE / "practice_4_5_interaction.png", dpi=150, bbox_inches="tight")
plt.close(fig)
...
MU_M, CV_M = 433.3, 2.3
sig_log = np.sqrt(np.log(1 + CV_M**2))
...
for i in range(N_FEATURES):          # фичи F1..F10 запускаются в месяцы 1..10
    prior = len(launched)
    true_marg = (1 + TRUE_EFFECTS[i]) * (1 - LAMBDA_PAIR * prior) - 1
...
rev = np.where(is_hold[:, None], base * mult_hold[None, :], base * mult_treat[None, :])

treat_mask = ~is_hold
months = np.arange(1, MONTHS + 1)
claim_cum, gap, gap_lo, gap_hi = [], [], [], []
for m in range(MONTHS):
    claim_cum.append(sum(claim_meas[i] for i in claim_meas if i <= m) * 100)
    mt = rev[treat_mask, m].mean()
    mh = rev[is_hold, m].mean()
    st, sh = rev[treat_mask, m].std(), rev[is_hold, m].std()
    se = np.sqrt(st**2 / treat_mask.sum() + sh**2 / HOLD_N) / mh
    g = mt / mh - 1
    gap.append(g * 100)
...
se = np.sqrt(ann_t.var() / treat_mask.sum() + ann_h.var() / HOLD_N) / mh
g = mt / mh - 1
...
print(f"наивная сумма истинных эффектов зелёных:      +{naive_sum:.1f}%")
print(f"сумма заявленного (отчёты launch-тестов):     +{claimed:.1f}%")
print(f"факт по глобальному холдауту:                 +{g * 100:.1f}%"
      f"  (95% ДИ: +{(g - 1.96 * se) * 100:.1f}..+{(g + 1.96 * se) * 100:.1f} п.п.)")
print(f"t-тест Уэлча по годовой выручке: p = {ttest.pvalue:.2e}")
print(f"уровень декабря (все фичи в проду):        +{gap[-1]:.1f}%"
      f"  (95% ДИ: +{gap_lo[-1]:.1f}..+{gap_hi[-1]:.1f} п.п.)")
print(f"«кто съел мой прирост»: {claimed - gap[-1]:.1f} п.п. из {claimed:.1f} заявленных"
      f" (взаимодействия + шум + отбор)")
...
ax.step(months, claim_cum, where="post", color="#7f7f7f", lw=2,
        label="заявлено: сумма релизов по отчётам")
ax.plot(months, naive_sum * np.ones(MONTHS), ":", color="#7f7f7f", lw=1.5,
        label=f"наивная сумма истинных эффектов: +{naive_sum:.1f}%")
...
ax.fill_between(months, gap_lo, gap_hi, color="#1f77b4", alpha=0.2,
                label="95% ДИ холдаут-оценки")
ax.annotate(f"разрыв {claimed - gap[-1]:.1f} п.п.:\nвзаимодействия + winner's curse",
...
fig.tight_layout()
fig.savefig(HERE / "practice_4_5_holdout.png", dpi=150, bbox_inches="tight")
plt.close(fig)
...
# ложно зелёный — вред -0.3% (240 тыс./нед), откатим через 13 недель.
# Срок жизни фичи 26 недель. Трафик 150 тыс. юзеров/нед на руку.
#
# - задержка(w) = w недель без фичи, если она работает: w * ценность * приор;
# - пропуск(w) = серый тест хорошей фичи -> не катим: (1-мощность) * ценность * срок;
...
miss = (1 - power_w) * PRIOR_WORKS * VALUE_WK * LIFE_WEEKS
false_roll = (ALPHA / 2) * HARM_WK * ROLLBACK_WEEKS * np.ones_like(weeks)
...
total2 = (weeks * VALUE2 * PRIOR2
          + (1 - power_w) * PRIOR2 * VALUE2 * LIFE2
          + (ALPHA / 2) * HARM_WK * ROLLBACK_WEEKS)
...
ax.set_title("Длительность покупает мощность, а не защиту от ложного роллаута")
ax.legend(fontsize=9)
fig.tight_layout()
fig.savefig(HERE / "practice_4_5_economics.png", dpi=150, bbox_inches="tight")
plt.close(fig)
print("-> practice_4_5_economics.png")

print("\nГОТОВО: 4 графика в", HERE)
