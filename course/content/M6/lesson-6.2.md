...
**Цель урока:** освоить второй язык каузального вывода — модель потенциальных исходов (potential outcomes, Нейман–Рубин): научиться видеть за любой метрикой пару «миров» Y(1)/Y(0) и понимать, почему один из них всегда потерян (фундаментальная проблема); различать эстиманды ATE/ATT/CATE/ITE и выбирать их *до* анализа; уметь раскладывать наивную разность «-treated минус control» на эффект и смещение отбора формулой; понимать, где в продукте ломается SUTVA; и увидеть, почему рандомизация — золотой стандарт, а что приходится постулировать без неё (ignorability — фундамент уроков 6.3–6.4).
...
![Фундаментальная проблема: у юзера два потенциальных исхода, ](visuals/v74_potential_outcomes.png)
*Рис. 1. Фундаментальная проблема: у юзера два потенциальных исхода, в данные попадает один (закрашенный).*
...
![Один вопрос «в чём эффект?» — три ответа: ATE (всем), ATT (у](visuals/v75_ate_att_cate.png)
*Рис. 2. Один вопрос «в чём эффект?» — три ответа: ATE (всем), ATT (участвовавшим), CATE (по сегментам).*
...
Полный прогон — `practice_6_2.py`; здесь — числа реального запуска (seed зафиксирован) и их чтение.
...
**Шаг 3, SUTVA.** Пары друзей, купон «приведи друга»: прямой эффект 2,0, утечка к другу 1,2, полная раскатка — 3,2. Честный поюзерный сплит видит $+1{,}97 \approx 2{,}0$: друг в тесте с вероятностью 0,5 **в обеих группах одинаково**, спилловор-слагаемое сокращается — остаётся только свой эффект. Занижение 1,23 заказа $=$ 37% общего эффекта. Кластерная рандомизация по парам (обе версии или ни одной) видит $+3{,}08$ — почти истинные 3,2: пары-кластеры не делят друга с контролем.

**Шаг 4, ATT ≠ ATE.** Эффект теперь индивидуален: $\tau_i = -1 + 2{,}2U$ (частым — плюс, редким — минус), вступают снова голодные (их средний $U = +0{,}53$ против $-0{,}53$). Итог: **ATE $= -1{,}00$** (продавать всем — убыток), **ATT $= +0{,}16$** (пришедшим — в плюс), наивная разность $+3{,}32$ (не равна ни тому, ни другому: к неоднородности добавилось смещение состава $+3{,}16$). Три числа — три разных бизнес-решения; какое ваше — вопрос эстиманда, а не статистики.
...
3. **Эстиманд выбирают по ходу анализа.** Делают: посчитали три разности, показали ту, что «звучит лучше». Грозит: та же гибкость, что p-hacking (М2.4), только без p-value. Правильно: вопрос → эстиманд → метод фиксируются в дизайн-доке до данных (М4.1); ATE/ATT/CATE не взаимозаменяемы (практика: −1,00 / +0,16 / ...).
4. **Интерференция юзеров.** Делают: поюзерный сплит фичи со spillover (шеринг, купоны, общий ресурс курьеров). Грозит: систематическое занижение эффекта полной раскатки (37% в практике) при честной рандомизации. Правильно: думать о юните рандомизации до старта (кластеры, гео, switchback — М4.3); в анализе честно оговаривать, «эффект при какой доле лечения» оценен.
...
7. **Перенос оценки не туда.** Делают: эффект на добровольцах RCT объявляют эффектом «для всех» (или наоборот: ATT подают как прогноз раскатки). Грозит: при неоднородности ATE ≠ ATT по знаку (−1,00 против +0,16 в практике) — решение будет противоположным. Правильно: эстиманд = вопрос решения; перенос между популяциями — отдельное обоснование (внешняя валидность).

## Практика

Файл `practice_6_2.py` (numpy/pandas/matplotlib, seed зафиксирован, графики сохраняются рядом). В симуляции доступна божественная перспектива — генерируются оба мира, наблюдается один; это единственный способ увидеть смещение глазами. Четыре блока (кейс «ЕдаДома Прайм»):
...
3. **SUTVA-нарушение**: спилловор купона в парах друзей; поюзерный сплит vs кластерный по парам; механика сокращения бета-слагаемого (`practice_6_2_sutva.png`).
4. **ATT ≠ ATE**: неоднородный эффект $\tau(U)$ + самоселекция; scatter индивидуальных эффектов с линиями ATE/ATT (`practice_6_2_att_ate.png`).
...
- SUTVA: поюзерный сплит $+1{,}97$ (только прямой эффект 2,0), полная раскатка 3,2, кластерный тест $+3{,}08$; занижение 37%;
- эстиманды: ATE $-1{,}00$, ATT $+0{,}16$, наив $+3{,}32$ — три ответа на три разных вопроса, включая противоположные решения.
...
Интерференция идёт через получателей шера, которые рассеяны по всей популяции, — поюзерный сплит не может изолировать лечение. Варианты: кластеры соцграфа (если связи наблюдаемы — редкость), гео-рандомизация или switchback (М4.3), либо смириться и чётко назвать эстиманд: поюзерный тест оценивает эффект при данной доле лечёных (частичная раскатка), где спилловор входит в обе группы и в среднем сокращается — как бета-слагаемое в практике (шаг 3). Для вопроса «что будет при включении всем» это занижение, величина которого растёт с силой spillover.
</details>
...
7. Цена интерференции: поюзерный тест оценивает эффект при данной доле лечения; полная раскатка = прямой эффект + spillover (в практике 2,0 против 3,2 — занижение 37%).
...
9. Без монетки нужно ignorability: $(Y(0), Y(1)) \perp T \mid X$ + positivity — непроверяемо прямо; покупается доменной аргументацией и косвенными проверками (баланс, A/A на пре-периоде, плацебо — 6.5).
