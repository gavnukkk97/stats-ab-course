...
2) Сплит-сервис как класс: домены -> слои -> 1000 бакетов (Google KDD'10),
   ...
   отчёт о ёмкости слоёв + цена слота в MDE (2N >= 16*(s/theta)^2);
...
Запуск:  python3 practice_7_1.py
...
ax.legend(fontsize=8)
fig.tight_layout()
fig.savefig(HERE / "practice_7_1_hash.png", dpi=150)
plt.close(fig)
...
class SplitService:
    ...
    def add_domain(self, name: str, traffic: float = 1.0) -> None:
        self.domains[name] = {"traffic": traffic, "layers": []}
    ...
    def assign(self, layer: str, user_id: str | None = None, cookie: str | None = None,
        ...
        """Приоритетная аллокация KDD'10: user_id -> cookie -> cookie+date -> random.

        Возвращает имя варианта или None (юзер не попал ни в один слот слоя).
        Random-назначения помечаем bias-id: их нельзя смешивать с хэш-стабильными.
        """
        salt = self.layers[layer]["salt"]
        for name, a, size, n_var in self.layers[layer]["alloc"]:
            if user_id is not None:
                bucket = get_bucket(user_id, salt, algo)
            elif cookie is not None and date is None:
                bucket = get_bucket(cookie, salt, algo)
            elif cookie is not None:
                bucket = get_bucket(f"{cookie}|{date}", salt, algo)
            else:
                return f"{name}:random_biasid"  # вне хэша — только с маркировкой
            if a <= bucket < a + size:
                v = (bucket - a) * n_var // size
                return f"{name}:v{v}" if n_var > 2 else (f"{name}:A" if v == 0 else f"{name}:B")
        return None
...
for share in (0.50, 0.25, 0.10, 0.10, 0.05):
    svc.add_experiment("ranking", f"rank_test_{int(share * 100)}", share)
...
# %% [markdown]
# Ёмкость и цена слота. Формула KDD'10 для 95%/80%: 2N >= 16*(s/theta)^2.
...
for share in (1.00, 0.50, 0.25, 0.10, 0.05):
    ...
    mde = math.sqrt(16 * CV ** 2 / n_total)          # 2N >= 16(s/theta)^2 -> theta
...
print(f"""   Проверка на 100%: MDE = sqrt(16*CV^2/2N) = {math.sqrt(16 * CV ** 2 / USERS_2WK):.2%}
...
ax2.grid(alpha=0.3)
fig.tight_layout()
fig.savefig(HERE / "practice_7_1_capacity.png", dpi=150)
plt.close(fig)
...
ax2.grid(alpha=0.3)
fig.tight_layout()
fig.savefig(HERE / "practice_7_1_sticky.png", dpi=150)
plt.close(fig)
