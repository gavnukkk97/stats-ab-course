...
**Цель урока:** собрать финальный виток блока — видение заказчика «платформа = календарь гипотез». Понять, что слоты трафика — конечный ресурс (наша симуляция: спрос бэклога 46 слот-недель при ёмкости 3 слоёв × 13 недель = 39 — очередь неизбежна), а очередь «кто первый» — это потерянные деньги: приоритизация по **экономике слота** (score = p_успеха × ценность / длительность — простая версия E[Δ̂⁺] Аzevedo: ценность информации выше у неопределённых гипотез) против RICE/ICE, которые не видят слот; заменить ручной juggling расписанием в решётке **слой × неделя**, где система сама предлагает ближайший свободный слот с учётом MDE→n→дней, конфликтов слоя и сезонности (blackout-недели); в нашей симуляции экономика слота даёт 55,6 млн ₽ ценности против 36,1 у FIFO при том же трафике (+54%), а трафик ×2 снимает очередь (15/15 гипотез) и смещает узкое место в генерацию гипотез. Освоить velocity-метрики (время идея→решение, доля ожидания в очереди, throughput тестов/неделю) и **паспорт ёмкости** платформы (сколько тестов/квартал она тянет — считать из трафика и MDE-микса: у нас 12,7 → 18,3 тестов/квартал); связать календарь с холдаутами (4.5) и канареечными релизами (4.1); провести границу **buy vs build** (trisigma: своя платформа ~150 млн ₽/3 года против готовой ~2,5 млн ₽/год) и увидеть свободную нишу: ни один вендор не делает очередь с экономикой, capacity planning и velocity-дашборды. Оргвывод: платформа — внутренний продукт с юнит-экономикой, а не «сервис тестов».
...
![Платформа-календарь](visuals/v89_calendar.png)
...
![Экономика слота](visuals/v90_slot_economics.png)
...
![Velocity-воронка](visuals/v91_velocity.png)
...
| **Очередь с экономикой слота** | **нет** — backlog живёт в Jira/Notion, ICE/RICE вне платформ | score = p·V/d, E[Δ̂⁺], деградация ожидания |
...
Вывод: рынок закрыл сплит, статистику, guardrails и каталог; «календарь гипотез» — **подтверждённое white space**: паттерны существуют по частям (Google capacity, LinkedIn ramp assistant, Avito слои, Azevedo экономика), продукта нет. Для проектировщика это легитимная проектная тема; для компании — возможная уникальность платформы.

### 7. Buy vs build: 150 млн ₽ против 2,5 млн ₽/год — и оргвывод

Экономика (Trisigma, интервью «Коммерсанту» и исследование рынка): своя платформа — от ~150 млн ₽ и 3 года; готовая — от ~2,5 млн ₽/год. Но выбор не «что дешевле»: **build** имеет смысл при большой интенсивности (сотни тестов/год), глубокой интеграции в свой прод (свои юниты, сетевые эффекты — switchback 4.3, свои метрики с тяжёлой семантикой) и когда данные/методология — конкурентное преимущество; **buy** — на этапе зрелости «ползём» (crawl-walk-run, Zalando): быстрая time-to-value, методология обновляется вендором, команда учится на чужих ошибках. Прагматичный путь из research-архитектур: гибрид — вендорский сплит-контур + свой отчётный слой на своём озере (warehouse-native Eppo/Statsig) + **свой календарный слой, которого нет ни у кого**.
...
![Buy vs build](visuals/v92_buy_build.png)
*Рис. 4. Buy vs build: спор не «что дешевле», а «где наша уникальность» (trisigma: 150 млн ₽ vs 2,5 млн ₽/год).*

## Разбор на данных

Симулятор календаря `practice_7_4.py`: бэклог из 15 гипотез «ЕдаДома» (по 5 в слоях ранжирование/интерфейс/коммуникации), для каждой MDE→n→длительность, ценность (млн ₽/нед на раскатке) и вероятность успеха; зелёность фиксирована одним «миром» (в нашем мире зелёных 4 из 15 — ожидание ~⅓); квартал 13 недель, blackout 10–11.
...
![Календарь-гант: FIFO vs экономика слота](practice_7_4_calendar.png)
...
![Velocity и ценность](practice_7_4_velocity.png)
...
3. **Уплотнить календарь без мысли о взаимодействиях.** Делают: «в слое „интерфейс" запустим четыре теста одновременно по 25%». Грозит: формально валидно (4.2), но эффекты механик взаимодействуют (4.5: кумулятивные эффекты, каннибализация), а мощность каждого упала (меньше n на тест → длиннее тест → MDE выше). Правильно: одна механика — один слой; уплотнение — осознанный трейд-офф мощности и интерпретируемости, а не «больше тестов в ячейке».
...
7. **Ждать календарь от вендора.** Делают: «возьмём Eppo/Statsig — там точно есть планировщик». Грозит: у вендоров есть cron-расписания и слои, но нет очереди с экономикой, capacity planning и velocity-дашбордов (подтверждено по докам) — «дорогая» покупка не решает главное. Правильно: вендор — для сплита/статистики/holdouts; календарный слой проектировать самим (или гибрид — архитектура D из research).

## Практика

Файл `practice_7_4.py` (percent-формат, numpy/scipy/matplotlib/pandas, seed, Agg). Симулятор календаря гипотез:
...
Графики: `practice_7_4_calendar.png` (гант «слои × недели», FIFO vs slot, blackout, красные × непоместившихся), `practice_7_4_velocity.png` (ценность и velocity-метрики четырёх стратегий).
...
Паспорт: длительности ~падают вдвое (пока не упёрлись в минимум 2 нед), ёмкость растёт примерно вдвое, очередь исчезает — в нашей симуляции 15/15 гипотез и +29..64% ценности (55,6→71,8 у slot-экономики; 36,1→59,1 у FIFO). Но: (а) это разовая мера — очередь вернётся при росте бэклога; (б) бесплатная альтернатива — CUPED/чувствительные OEC (3.4–3.6) и новые слои под независимые домены (7.1), если тесты длиннее двух недель; (в) после разгрузки узкое место смещается в генерацию гипотез — если бэклог не растёт, купленная ёмкость простаивает. Решение о трафике — экономическое: цена слот-недели против ценности ожидающих гипотез (и не по ICE, а по score).
</details>
...
8. Вендоры vs своё: cron-расписания, слои, holdouts, GST, KB — есть (Statsig/Eppo/ABsmartly); очередь с экономикой, календарь пересечений, capacity planning, velocity-дашборды — white space, проектировать самим.
