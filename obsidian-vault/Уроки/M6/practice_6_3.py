...
5) Мини-кейс DiD+PSM: панель 1500 курьеров, 6 периодов, конфаундер в назначении
   ...
   сосед с калипером) -> DiD на отматченных -> DR-DiD в духе Sant'Anna&Zhao'20
   (OR-остатки + IPW-веса): ломаем по очереди модель PS и модель исхода —
   оценка держится, пока верна хотя бы одна (double robustness). Грабля:
   матчинг с post-treatment ковариатой (рейтинг качества в первый пост-месяц,
   на него тренинг уже повлиял) смещает оценку.
...
def synth_weights(j, donors):
    ...
    NNLS, где сумма весов = 1 добавлена как строка с большим весом mu.
    """
    A = Y4[donors, :T4_PRE].T
    y = Y4[j, :T4_PRE]
    mu = 1e3 * max(1.0, float(np.abs(y).max()))
    w, _ = nnls(np.vstack([A, mu * np.ones((1, A.shape[1]))]), np.append(y, mu))
    return w
...
def dr_did(D, dY, X_ps, X_or):
    e = np.clip(1 / (1 + np.exp(-(X_ps @ logit_irls(X_ps, D)))), 0.02, 0.98)
...
print("    пока верна ХОТЯ БЫ одна из моделей (PS или OR) — DR держит оценку у истины;")
