...
Запуск из корня репозитория: python3 course/modules/M6/practice/practice_6_2.py
...
fig.tight_layout()
fig.savefig(IMAGE_DIR / "practice_6_2_randomization.png", dpi=150)
plt.close(fig)
...
# Separate experiment: one independent assignment per pair, both friends share it.
T_pair = RNG.integers(0, 2, N_PAIR)
Y_pair = BASE + (DELTA + BETA) * T_pair + RNG.normal(0, 1.5, (2, N_PAIR)).mean(axis=0)
est_pairlevel = Y_pair[T_pair == 1].mean() - Y_pair[T_pair == 0].mean()
se_pair = np.sqrt(Y_pair[T_pair == 1].var(ddof=1) / (T_pair == 1).sum()
                  + Y_pair[T_pair == 0].var(ddof=1) / (T_pair == 0).sum())
...
print(f"   поюзерный сплит видит                    : {est_userlevel:+.2f} ~ дельта {DELTA:+.2f}: "
      f"друг в тесте с вероятностью 0,5 в ОБЕИХ группах,\n      спилловор-слагаемое сокращается — остаётся только свой эффект")
...
print("      назначения: пары не делят друга с контролем; анализ по независимым парам.")
print(f"   SE оценки по независимым парам: {se_pair:.3f}")
print("   Механика: P(друг в тесте) = 0,5 и не зависит от собственного T —")
print(f"      бета-слагаемое входит в обе группы одинаково и сокращается: оценка -> дельта {DELTA:+.2f}.")
print("      А вопрос продакта про полную раскатку — про дельту + бета. Разрыв = SUTVA-нарушение.")
...
ax.spines[["top", "right"]].set_visible(False)
fig.tight_layout()
fig.savefig(IMAGE_DIR / "practice_6_2_sutva.png", dpi=150)
plt.close(fig)
...
# увеличивает заказы, у редких — уменьшает; прибыль здесь не моделируется.
# Вступают снова голодные -> ATT отвечает «сколько заказов ТЕМ,
...
print("   -> заказов в среднем меньше (ATE < 0), у вступивших больше (ATT > 0).")
print("      Для вывода о прибыльности нужны плата за подписку, выручка и затраты.")
...
ax.spines[["top", "right"]].set_visible(False)
fig.tight_layout()
fig.savefig(IMAGE_DIR / "practice_6_2_att_ate.png", dpi=150)
plt.close(fig)
