...
2) СРАВНЕНИЕ ОЦЕНЩИКОВ: наивная разность -> регрессия с ковариатами ->
   ...
   кого выбросил калипер (смена эстиманда ATT для оставленных treated).
...
Запуск из корня репозитория: python3 course/modules/M6/practice/practice_6_4.py
...
from causal_utils import matched_smd
import numpy as np
...
def smd(x, d, w=None):
    ...
    w -- веса контроля полной длины n (например, счётчик повторного
    использования пары после матчинга). Возвращает
    ...
    if w is None:
        ...
        wc = np.asarray(w, dtype=float)[c]
        mc = np.average(x[c], weights=wc)
...
def match_pairs_rows(Zm, D, caliper):
    """Жадный 1:1 матчинг по евклидову расстоянию между СТРОКАМИ матрицы
    (мини-версия расстояния Махаланобиса без ковариаций).

    Нужно для грабли из блока 3: матчинг «в лоб» по всем ковариатам,
    включая пост-тритментную. Zm -- стандартизованная матрица признаков.
    """
    it, = np.where(D == 1)
    ic, = np.where(D == 0)
    d2 = ((Zm[it][:, None, :] - Zm[ic][None, :, :]) ** 2).sum(-1)
    j = d2.argmin(axis=1)
    dmin = np.sqrt(d2[np.arange(len(it)), j])
    keep = dmin <= caliper
    return it[keep], ic[j[keep]], keep
...
truth = 1.0
print("=" * 88)
print("1) СРАВНЕНИЕ ОЦЕНЩИКОВ. DGP: e(D=1|X)=sigmoid(-2.2+0.45*act+0.8*inc+0.7*mega),")
...
smd_after = [matched_smd(X[:, k], D, idx_t, idx_c) for k in range(X.shape[1])]
...
beta_bad, Xd_bad = fit_logistic(X[:, bad_cols], D)
e_bad = sigmoid(Xd_bad @ beta_bad)
...
print("=" * 88)
print(f"   SMD (|.|) по ковариатам, ориентир 0.1 (Love plot ниже):")
...
print(f"   вывод -- не про всех подписчиков: эстиманд сместился ATT для оставленных treated.")
...
fig.tight_layout()
fig.savefig(IMAGE_DIR / "practice_6_4_love_plot.png", dpi=150)
plt.close(fig)
...
# грабля: «подбери двойника, у которого и здоровой еды столько же!» --
# добавляем в матчинг МЕДИАТОР M (он же следствие подписки, post-treatment)
XM = np.column_stack([X, M])
Zm = (XM - XM.mean(0)) / XM.std(0)
it2, ic2, _ = match_pairs_rows(Zm, D, 0.2)
att_med = (Y[it2] - Y[ic2]).mean()

print(f"   ИСТИНА (полный эффект через медиатор): {total_effect_true:+.3f}")
print(f"   матчинг по конфаундерам «до»:           {att_ok:+.3f}  <- ловит полный эффект")
print(f"   матчинг по [X, M], M -- post-treatment:  {att_med:+.3f}  <- эффект «исчез»")
print("   Механика: прямое влияние D на Y здесь НУЛЕВОЕ -- весь эффект идёт")
print("   по пути D -> M -> Y. Матчинг по M подбирает контролю «такого же по M»")
print("   и тем самым блокирует медиатор: мы спросили «сколько даёт подписка,")
print("   если её главный канал выключить?» -- почти ноль.")
print("   В этом DGP блокируется механизм; в общем случае возможен новый bias.")
print("   Прямой эффект требует дополнительных допущений; это не автоматическая интерпретация.")
...
print(f"     первый этап: E[D|Z=1]-E[D|Z=0] = "
      f"{D[Z == 1].mean() - D[Z == 0].mean():+.3f}, F = {f_stat:.0f} (диагностика первого этапа)")
...
print("     здесь совпадают точки; SE второй обычной OLS не является SE 2SLS")
print(f"     ИСТИНА: +1.000 (compliance неполная -- Wald оценивает LATE)")

Zw, Dw, Yw = make_iv(4000, beta_z=0.12, seed=6411)
f_weak, b_weak = first_stage_F(Zw, Dw)
w_weak = wald(Zw, Dw, Yw)
weak_flag = "F<10: инструмент слаб" if f_weak < 10 else "F>=10: пограничный"
print()
print(f"   СЛАБЫЙ инструмент (beta_z=0.12):")
print(f"     первый этап: сдвиг = {Dw[Zw == 1].mean() - Dw[Zw == 0].mean():+.4f}, "
      f"F = {f_weak:.1f} ({weak_flag})")
...
print("     амплитуда ошибки растёт на порядки. F=10 не универсальная гарантия; нужны методы inference для слабых IV.")
...
ax.legend(fontsize=9)
fig.tight_layout()
fig.savefig(IMAGE_DIR / "practice_6_4_iv.png", dpi=150)
plt.close(fig)
...
# %% DML: the estimand is part of the model, not a consequence of cross-fitting.
from causal_utils import crossfit_binary_demo
print("\n5) Cross-fitting: PLR и AIPW при гетерогенном эффекте")
print(crossfit_binary_demo())
print("PLR близок к overlap-weighted эффекту; AIPW — к ATE при допущениях этого DGP.")
