...
**Цель урока:** научиться проектировать АБ-тест до его запуска: считать размер выборки под MDE через формулу для *двух* выборок (и никогда не терять множитель 2 — главная типичная ошибка), понимать MDE как бизнес-решение, а не статистический параметр, выбирать сплит (почему 50/50 всегда мощнее) и длительность в полных неделях — и проверить готовый дизайн синтетическими A/A-тестами.

## Зачем это аналитику

«ЕдаДома» хочет протестировать новую сортировку ресторанов на главной. Продакт: «Давай просто запустим на недельку и посмотрим». Проблема в том, что «посмотрим» — это тоже дизайн, только неявный и почти всегда плохой: либо тест с мощностью 25% похоронит работающую фичу (урок 2.1: «негатив» слабого теста почти ничего не утверждает), либо, наоборот, ежедневные взгляды на дашборд раздувают ложные тревоги (урок 3.7). Всё, что решает судьбу теста — α, β, MDE, n, длительность — фиксируется *до* запуска, в дизайн-доке. Этот урок — про то, как это считать.

И это не теория ради теории. В марте 2026 в канале shelter_analytics разбирали **реальную типичную ошибку**: аналитик считал размер выборки по *одновыборочной* формуле, забыл множитель 2 — и вместо необходимых **17 422 наблюдений получил 8 711**. Тест запустили, «негатив» посчитали доказательством. Симуляции показали: мощность такого теста — **50,8% вместо проектных 80%**. Монетка почти честнее. Ниже разберём, откуда берётся этот множитель, почему мощность падает ровно до 50,8% (это не совпадение, а арифметика) и как не наступить на те же грабли.
...
где $\Delta$ — MDE (минимальный эффект, который обязаны поймать), $\sigma^2$ — дисперсия метрики на юзера (по пре-периоду). Для конверсии $\sigma^2 = p(1-p)$. Стандартный набор $\alpha = 5\%$, мощность 80% даёт $(1{,}96 + 0{,}84)^2 = 7{,}85$, и формула запоминается как «$15{,}7 \cdot \sigma^2/\Delta^2$».
...
![Самая дорогая опечатка в планировании: тест двухвыборочный —](visuals/v32_mde_factor2.png)
...
Это универсальный результат: **забытый ×2 при α=5% и целевой мощности 80% всегда даёт ~50,8%** — независимо от метрики и MDE. Именно столько и вышло в симуляциях поста shelter_analytics. Половина мощности — это каждая вторая работающая фича отправляется в могилу с вердиктом «эффекта нет» (ошибка II рода из урока 2.1).
...
И обратное направление — MDE при фиксированном трафике: $\Delta = (z_1+z_2)\,\sigma\sqrt{2/n}$. Если у вас есть только 5 000 юзеров на группу, то MDE $= 2{,}80 \cdot 0{,}325 \cdot \sqrt{2/5000} \approx 1{,}82$ п.п. — тест просто не способен увидеть меньше.
...
Функция $r(1-r)$ максимальна при $r=0{,}5$ — **сбалансированный сплит всегда мощнее при том же трафике**, это не конвенция, а арифметика. Перекос 10/90 увеличивает требуемый трафик в $0{,}25/0{,}09 = 2{,}78$ раза: $SE$ разности растёт как $1/\sqrt{r(1-r)}$, при 10/90 — в 1,67 раза против 50/50.
...
![Балансированный сплит 50/50 всегда мощнее: r·(1−r) максимале](visuals/v34_split_power.png)
*Рис. 2. Балансированный сплит 50/50 всегда мощнее: r·(1−r) максимален в середине.*

### 4. MDE — это бизнес-решение

Частая подмена: «посчитаем, какой эффект получится». MDE работает наоборот — это **минимальный эффект, при котором бизнес раскатывает фичу**. Откуда его брать:
...
Правило гигиены: MDE фиксируется в дизайн-доке **до** расчёта n, а не подгоняется под доступный трафик задним числом («ну пусть будет +2 п.п., как раз две недели набирается» — это не дизайн, это рационализация).
...
- **Недельная сезонность.** У «ЕдаДома» выходные дают ×1,4 к заказам против будней; конверсия буднего вечера и воскресного обеда — разные миры. Тест кроится **целыми неделями**: 2 недели, не 13 дней, иначе одна группа видит на выходной больше.
...
![Календарь набора: выходные ×1,4 — «20 дней» наивного расчёта](practice_3_1_calendar.png)
...
1. **Нельзя «продлить тест до прокраса».** Если горизонт выбирается по данным («ещё недельку, пока не зазеленеет»), фактический α перестаёт быть 5% и уезжает к 20–30% — это peeking, урок 3.7. Горизонт фиксируется заранее; накопление сверх него — уже новый тест.
2. **Нельзя сравнивать наблюдаемый эффект с MDE.** «Получили +0,8 п.п. при MDE +1 п.п. — почти поймали!» Наблюдаемый эффект — шумная оценка (урок 2.1: Type M); для «поймали/не поймали» есть CI: если +1 п.п. лежит внутри 95% CI эффекта — тест не исключает целевой эффект. MDE отвечает «что проектировали», CI — «что измерили».
3. **Мощность постфактум бессмысленна.** «Какая была мощность у запущенного теста?» — вопрос к дизайн-доку, не к данным. После остановки можно только честно сказать: «эффект больше X маловероятен» — через CI.
...
Как читать: каждый ряд — переговоры с продактом. «Хотим видеть +0,7 п.п.? Это полтора месяца трафика — что проще: смириться с +1 п.п. или усилить метрику (урок 3.5)?». Ряд «+2 п.п.» — и есть сюжет поста shelter_analytics в миниатюре: колонка «без ×2» при MDE +1 п.п. даёт 8 289 — почти те самые 8 711; правильный ответ вдвое больше, а мощность на «экономном» варианте — 50,8%.

Проверка расчёта одной строки руками: MDE +1,5 п.п. → $\sigma^2/\Delta^2 = 0{,}1056/0{,}0225 = 469{,}3$; $n = 2 \cdot 7{,}85 \cdot 469{,}3 = 7\,367$; запас 5% → ~7 700 на группу, 2 полные недели.

## Подводные камни

1. **Забытый множитель 2.** Делают: берут формулу/калькулятор с $n = (z_1+z_2)^2\sigma^2/\Delta^2$ и пишут в дизайн-док. Грозит: половина выборки, мощность 50,8% вместо 80%, хороните каждую вторую рабочую фичу. Правильно: формула для двух выборок; проверка приёмом из практики — сверка симуляцией.
...
3. **MDE подгоняют под трафик.** Делают: «наберём 2 недели → какой MDE выйдет? → +2,2 п.п., пусть так и будет». Грозит: тест, который «не увидит» все эффекты, ради которых фича делалась; негатив трактуется как «не работает». Правильно: MDE из экономики изменения (раздел 4), потом переговоры про метрику/длительность.
...
6. **Длительность кусками.** Делают: старт в субботу, длительность «10 дней», внутри праздники; или 13 дней = «почти 2 недели». Грозит: неполный недельный цикл — группы несравнимы по смеси будни/выходные; сезонные пики попадают в одну группу. Правильно: целое число недель, одинаковый день старта, запас 5–10%.
...
Файл `practice_3_1.py` (percent-формат, numpy/scipy/matplotlib, seed зафиксирован). Четыре блока:
...
2. **Кривая n(MDE)** с двумя линиями — наивной и правильной (`practice_3_1_mde_curve.png`): аннотация разрыва ×2 (8 289 → 16 577) и история 8 711 → 17 422.
3. **Мощность от сплита r** при фиксированном трафике: аналитика (точная формула через $SE(r)$) против симуляции для r ∈ {0,5 … 0,1} (`practice_3_1_split_power.png`).
4. **Сколько дней ждать**: дневной трафик с выходными ×1,4, накопительное набегание 33 154 юзеров, наивные 20 дней против 3 полных недель (`practice_3_1_calendar.png`).
...
- блок 3: 0,81 / 0,74 / 0,62 / 0,40 (теория) против 0,78 / 0,71 / 0,67 / 0,37 (симуляция, 800 прогонов на точку) при r = 0,5 / 0,3 / 0,2 / 0,1 — расхождения в пределах шума симуляции; возврат мощности при 10/90 стоит ×2,78 трафика;
...
$(z_{0{,}975}+z_{0{,}9})^2 = (1{,}96+1{,}28)^2 = 10{,}51$; $n = 2 \cdot 10{,}51 \cdot 0{,}1056/0{,}0225 = 9\,863$ — около 9 900 на группу. Сравните с 7 368 для 80%: +10 п.п. мощности стоит +34% выборки — мощность дешевеет медленнее, чем растёт.
</details>
...
3. Забыт ×2 → выборка вдвое меньше → мощность ≈50,8% (не 80%) при любых метриках — история «8 711 вместо 17 422» из shelter_analytics.
4. MDE = минимальный эффект, при котором бизнес раскатывает фичу: экономика + история тестов + цена трафика; не подгонять под доступный n.
...
- koch-kir, «Мощность А/В-теста с неравными выборками» (12.10.2021) — https://medium.com/@koch-kir — формулы мощности при неравных группах, сбалансированный сплит всегда мощнее (~0,79 против ~0,64 в их примере), когда небаланс оправдан (конспект: `materials/web/web-articles.md` §3.4).
