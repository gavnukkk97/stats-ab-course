...
**Цель урока:** уметь доказать симуляцией, что «смотреть тест каждый день и останавливаться при $p<0{,}05$» раздувает $\alpha$ с 5% до 25–35%; понимать, почему продление теста после прокраса не чинит проблему; знать три семейства решений — group sequential (альфа-спендинг, O'Brien-Fleming/Pocock), mSPRT и always-valid p-value (механика: смесь по $\lambda$, растущая граница), байесовский взгляд (форвард М5) — и цену гибкости: чуть шире интервалы, чуть больше $n$; решать, когда sequential оправдан (длинные тесты, дорогие фичи, раннее обнаружение вреда в guardrails).
...
Проблема в том, что тест с фиксированным горизонтом — это обязательство: **один** взгляд на **заранее назначенный** день. Каждый «просто посмотрел» — это ещё одна проверка гипотезы, а максимум по проверкам уже не тест с $\alpha = 5\%$ (урок 2.4 про множественность — та же болезнь, только по оси времени). Хуже того: интуиция «ну тогда подождём ещё недельку, чтобы подтвердить» не лечит — альфа уже потрачена.

Но у проблемы есть и вторая сторона: бизнес реально хочет останавливать тесты раньше, когда эффект очевиден, и мгновенно глушить фичу, когда она вредит (guardrails). Для этого существуют **последовательные тесты** — процедуры, валидные при любой частоте заглядывания. Цена — консервативность: чуть позже детекция эффекта размера MDE, зато мгновенная — крупного. Вместе с CUPAC (урок 3.6) это и есть связка «в пару дюжин раз быстрее» из кейса Авито.
...
![Подглядывание = множественные сравнения во времени: α растёт](visuals/v45_peeking_sequential.png)
...
2. **always-valid p-value**: $\tilde p_k = \min_{j \le k} 1/\Lambda_j$ — p-value, валидный одновременно во все моменты времени. Именно его можно вешать на дашборд вместо наивного p.
3. **граница решимости растёт**: в z-шкале пороговое $|z|$ растёт примерно как $\sqrt{2 \ln\ln n}$-типа закона повторного логарифма: в практике $3{,}66$ в день 1 → $3{,}1$–$3{,}2$ к неделям 4–8 (против фиксированных 1,96). Ранние дни строгие — «растущая граница» и есть способ заплатить альфой за гибкость.

![mSPRT в действии: наивный p ныряет под 0,05 в A/A, always-va](practice_3_7_msprt.png)
...
![Цена и выгода sequential: на MDE почти бесплатно, на больших](practice_3_7_speed.png)
...
Из практики `practice_3_7.py`: A/A-тест «ЕдаДома», 200 юзеров/день на руку, 20 000 миров, ежедневный z-тест по накопленным данным.
...
**mSPRT.** Смесь $\lambda \sim N(0, (0{,}1\sigma)^2)$, always-valid p-value $\tilde p_k = \min_j 1/\Lambda_j$: прокрас за 60 ежедневных проверок — 2,0% миров (гарантия $\leq 5\%$ держится). Граница $|z|$: 3,66 (день 1) → 3,04 (день 7) → 3,11 (день 28) → 3,20 (день 60) против фиксированного 1,96.
...
1. **«Мы не останавливались, просто смотрели».** Делают: мониторят p-value, формально решение в конце. Грозит: если знание промежуточных p повлияло на что угодно (продление, смена метрики, правка фичи) — это optional stopping по факту. Правильно: либо фикс-горизонт без взгляда до дня X, либо честный sequential-критерий на дашборде.
...
6. **«Sequential всегда быстрее».** Грозит: на эффекте размера MDE sequential не быстрее (медиана 33 против 32) и детектит скромнее при жёстком потолке дней. Правильно: продавать sequential не как «ускорение», а как «валидность мониторинга + мгновенную реакцию на крупные эффекты и вред».
...
Файл: `practice_3_7.py` (percent-формат, numpy/scipy/matplotlib/pandas, seed, Agg). Что делает:
...
2. **Упрощённый mSPRT** на нормальных данных: смесь $\lambda \sim N(0, \tau^2)$, $\Lambda_k$ аналитически, always-valid p-value; A/A-уровень 2,0% за 60 ежедневных проверок; растущая граница $|z|$ = 3,66 → 3,20.
3. **Дни до решения** fixed vs sequential при эффекте = MDE / 2×MDE / 4×MDE и вреде −2×MDE (33 / 9 / 3 и 9 дней против 32).

Графики: `practice_3_7_peeking.png` (инфляция α + политики), `practice_3_7_msprt.png` (наивный p против always-valid; растущая граница), `practice_3_7_speed.png` (дни до решения).
...
$\Lambda_k$ — мартингейл под H0: он может «блуждать» сколь угодно долго, и чтобы $P(\max_k \Lambda_k \ge 1/\alpha) \le \alpha$ при бесконечном мониторинге, порог должен держать всё блуждание — а экстремум случайного блуждания растёт со временем. Радиус блуждания накопленной z-статистики растёт как $\sqrt{2 \ln \ln n}$ (закон повторного логарифма), поэтому и граница растёт примерно с этой скоростью: ранние взгляды строгие (в практике |z| = 3,7 в день 1), позже порог ~3+. Плата за «смотреть всегда» — консервативность на малых n.
</details>
...
Обычный p-value валиден только в один наперёд заданный момент ($P(p \le \alpha) \le \alpha$ при H0); последовательность обычных p нарушает это при одновременном чтении. Always-valid $\tilde p_k = \min_{j\le k} 1/\Lambda_j$ удовлетворяет $P(\exists k: \tilde p_k \le \alpha) \le \alpha$ — одновременно для всех моментов. Неравенство Вилля: для неотрицательного мартингейла (смесь SPRT $\Lambda$ с $E[\Lambda_0]=1$ под H0) $P(\sup_k \Lambda_k \ge 1/\alpha) \le \alpha$ — при любом правиле остановки. Отсюда правило: на дашборд вешаем $\tilde p$, а не наивный p.
</details>
...
1. Фикс-горизонт = обещание: α гарантируется при одном взгляде в назначенный день. Peeking = множественные сравнения по времени: 28 ежедневных проверок → α ≈ 27%, 60 → 33%.
...
6. Always-valid p-value: $\tilde p_k = \min_{j\le k} 1/\Lambda_j$ — вешать на дашборд вместо наивного p.
7. Граница mSPRT в z-шкале растёт (~√(2 ln ln n), LIL): день 1 — 3,7; неделя 4 — ~3,1. Это и есть цена гибкости.
