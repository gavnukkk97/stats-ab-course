...
Запуск:  python3 practice_3_1.py
...
def power_z(mde, n, sigma=SIGMA, alpha=ALPHA):
    ...
    return stats.norm.cdf(mde / se - Z_A)
...
def simulate_power(mde, n, reps=3000, rng=None):
    ...
    rng = rng or RNG
    a = rng.binomial(1, P0, size=(reps, n)).astype(float)
    b = rng.binomial(1, P0 + mde, size=(reps, n)).astype(float)
    return float(np.mean(stats.ttest_ind(a, b, axis=1, equal_var=False).pvalue < ALPHA))


# %% [markdown]
# Шаг 1. Калькулятор n<->MDE<->power и сверка симуляцией.
# Ключевая проверка: расчётное n даёт ~80% мощности, а ПОЛОВИНА n
# (забытый множитель 2) — универсальные 50.8%.
...
ax.grid(alpha=0.3)
fig.tight_layout()
fig.savefig(HERE / "practice_3_1_mde_curve.png", dpi=150)
plt.close(fig)
...
pw_curve = [stats.norm.cdf(MDE_SPLIT / (SIGMA * math.sqrt(1 / (N_TOTAL * x) + 1 / (N_TOTAL * (1 - x))))) if x < 1 else 0
            for x in rr]

fig, ax = plt.subplots(figsize=(8.8, 4.5))
ax.plot(rr * 100, pw_curve, "-", lw=2.4, color="#4C72B0", label="теория")
ax.plot([x[0] * 100 for x in rows], [x[1] for x in rows], "o", ms=7, color="#55A868",
        label="симуляция (800 прогонов)")
for r, pw_th, _, _, _ in rows:
    ax.annotate(f"{pw_th:.0%}", (r * 100, pw_th), xytext=(0, -14),
...
ax.set_title("При том же трафике 50/50 всегда мощнее:\n"
             "перекос 10/90 стоит x2.78 трафика (или -0.4 мощности в этой точке)")
ax.legend(fontsize=9, loc="lower right")
ax.grid(alpha=0.3)
fig.tight_layout()
fig.savefig(HERE / "practice_3_1_split_power.png", dpi=150)
plt.close(fig)
...
fig.tight_layout()
fig.savefig(HERE / "practice_3_1_calendar.png", dpi=150)
plt.close(fig)
...
print("   универсально (Phi((z_a+z_b)/sqrt(2)-z_a)); это и есть кейс 8 711/17 422.")
