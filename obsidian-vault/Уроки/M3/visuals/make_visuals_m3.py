...
# Единый стиль: заголовок = вывод. Запуск: python3 make_visuals_m3.py
import matplotlib
matplotlib.use("Agg")
...
n_one = (za + zb) ** 2 * sigma ** 2 / mde ** 2 / 2   # (без множителя 2 на группу)
n_two = 2 * (za + zb) ** 2 * sigma ** 2 / mde ** 2 / 2
fig, ax = plt.subplots(figsize=(8, 4.3))
ax.plot(mde * 100, n_two, lw=2.6, color=C_TRUE, label="правильно: двухвыборочная, всего 2·(z₊+z₋)²σ²/MDE²")
...
ax.set_yscale("log")
ax.set_xlabel("MDE, % от σ"); ax.set_ylabel("нужно наблюдений (log)")
ax.set_title("Самая дорогая опечатка в планировании: тест двухвыборочный — множитель 2 обязателен")
ax.legend(fontsize=9, loc="upper right")
fig.tight_layout(); fig.savefig("v32_mde_factor2.png", bbox_inches="tight"); plt.close(fig)
...
for n in ns:
    pw = stats.norm.cdf(mde * np.sqrt(n / 2) - 1.96)
    ax.plot(mde * 100, pw, lw=2.4, label=f"n всего = {n:,}".replace(",", " "))
...
ax.legend()
fig.tight_layout(); fig.savefig("v33_power_map.png", bbox_inches="tight"); plt.close(fig)
...
ax.set_title("Балансированный сплит всегда мощнее: r·(1−r) максимален при 50/50")
fig.tight_layout(); fig.savefig("v34_split_power.png", bbox_inches="tight"); plt.close(fig)
...
fig.tight_layout(); fig.savefig("v35_randomization_unit.png", bbox_inches="tight"); plt.close(fig)
...
fig.tight_layout(); fig.savefig("v36_icc_design_effect.png", bbox_inches="tight"); plt.close(fig)
...
fig.tight_layout(); fig.savefig("v37_ratio_num_denom.png", bbox_inches="tight"); plt.close(fig)
...
def ratio_aa(method, reps=1500, n=300):
    rej = 0
    for _ in range(reps):
        o1 = rng.poisson(2.2, n); o2 = rng.poisson(2.2, n)
        r1 = np.array([rng.lognormal(np.log(900), 0.9, max(k, 1)).sum() for k in o1])
        r2 = np.array([rng.lognormal(np.log(900), 0.9, max(k, 1)).sum() for k in o2])
        p = method(o1, r1, o2, r2)
        rej += p < 0.05
    return rej / reps

def naive(o1, r1, o2, r2):
    # наивный подход: раскидываем выручку юзера поровну по его заказам и t-тест по заказам
    ods1 = np.concatenate([np.full(k, r / k) for k, r in zip(o1, r1) if k > 0])
    ods2 = np.concatenate([np.full(k, r / k) for k, r in zip(o2, r2) if k > 0])
    return stats.ttest_ind(ods1, ods2, equal_var=False).pvalue
...
def delta(o1, r1, o2, r2):
    ...
    var_n, var_d = r1.var(), o2.var() * 0 + o1.var()
...
fig.tight_layout(); fig.savefig("v38_ratio_aa_level.png", bbox_inches="tight"); plt.close(fig)
...
fig.suptitle("Винзоризация сжимает дисперсию — тесты становятся чувствительнее",
             fontsize=12, fontweight="bold")
fig.tight_layout(); fig.savefig("v39_winsorization_sd.png", bbox_inches="tight"); plt.close(fig)
...
fig.tight_layout(); fig.savefig("v40_winsor_hides_effect.png", bbox_inches="tight"); plt.close(fig)
...
ax.legend()
fig.tight_layout(); fig.savefig("v41_thresholds_bias.png", bbox_inches="tight"); plt.close(fig)
...
fig.suptitle("CUPED: коррекция пре-периодом — та же идея, что парный дизайн из М2",
             fontsize=12, fontweight="bold")
fig.tight_layout(); fig.savefig("v42_cuped_scatter_ci.png", bbox_inches="tight"); plt.close(fig)

rhos = np.linspace(0, 0.95, 100)
fig, ax = plt.subplots(figsize=(8, 4.2))
ax.plot(rhos, 1 / (1 - rhos ** 2) * 100, lw=2.6, color=C_MAIN, label="сколько % наблюдений сэкономили")
...
ax.set_title("Чем сильнее «якорь» пре-периода, тем короче тест: 1/(1−ρ²)")
ax.legend()
fig.tight_layout(); fig.savefig("v43_cuped_savings.png", bbox_inches="tight"); plt.close(fig)
...
se_raw = y.std() / np.sqrt(n)
se_cuped = (y - 0.62 * pre1).std() / np.sqrt(n)
se_cupac = (y - pred).std() / np.sqrt(n)
...
ax.set_ylabel("SE оценки эффекта")
ax.set_title("Больше информативных пре-ковариат → сильнее сжатие: от CUPED к CUPAC")
fig.tight_layout(); fig.savefig("v44_cupac.png", bbox_inches="tight"); plt.close(fig)
...
alpha_inf = 1 - (1 - 2 * (1 - stats.norm.cdf(1.96)) * 0 + 0) ** 1  # заменим симуляцией ниже
rej_daily = []
for _ in range(3000):
...
fig, axes[1].clear() if False else None
t = np.arange(1, 61)
bound = np.sqrt((t * np.log(t)) / 2) * 0.0 + np.sqrt(t * np.log(1 / 0.001) / 2)  # мSPRT-подобная
axes[1].plot(t, bound, lw=2.6, color=C_ACC, label="always-valid граница (растёт)")
z_paths = []
for s in range(12):
    a = rng.normal(0, 1, 3000); b = rng.normal(0, 1, 3000)
    zs = [(b[:50 * k].mean() - a[:50 * k].mean()) / np.sqrt(2 / (50 * k)) for k in t]
    axes[1].plot(t, zs, lw=1.1, alpha=0.75, color=C_MAIN if max(np.abs(zs)) < max(bound) else C_BAD)
    z_paths.append(zs)
...
axes[1].set_title("Sequential: граница растёт со временем — можно смотреть каждый день")
axes[1].legend(fontsize=8.5, loc="lower right")
fig.suptitle("Два мира: фиксированный горизонт против последовательного теста",
             fontsize=12, fontweight="bold")
fig.tight_layout(); fig.savefig("v45_peeking_sequential.png", bbox_inches="tight"); plt.close(fig)

print("Готово: v32–v45 (14 визуализаций) в", __file__.rsplit("/", 1)[0])
