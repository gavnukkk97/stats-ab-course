...
Запуск:  python3 practice_3_4.py
...
sd_raw = POP.std()
sd_w99 = np.clip(POP, None, CAP99).std()
sd_w95 = np.clip(POP, None, CAP95).std()

df_sd = pd.DataFrame(
    {
        "SD чека, ₽": [sd_raw, sd_w99, sd_w95],
        "SD меньше, %": [0, 100 * (1 - sd_w99 / sd_raw), 100 * (1 - sd_w95 / sd_raw)],
        "n для той же мощности": [1.0, (sd_w99 / sd_raw) ** 2, (sd_w95 / sd_raw) ** 2],
    },
...
print("   Винзоризация 5/95 режет SD в ~2.5 раза -> той же мощности нужно ~6 раз меньше юзеров.")
...
ax.legend()
fig.tight_layout()
fig.savefig(HERE / "practice_3_4_sd.png", dpi=150)
...
def world_tail(n=N, with_effect=True):
...
def variants(c, t):
    """Единые пороги по объединённой выборке -> набор метрик."""
    pool = np.concatenate([c, t])
    hi99, hi95 = np.percentile(pool, 99), np.percentile(pool, 95)
    return {
        "raw": (t, c),
        "win 1/99": (np.clip(t, None, hi99), np.clip(c, None, hi99)),
        "win 5/95": (np.clip(t, None, hi95), np.clip(c, None, hi95)),
...
for _ in range(W):
    ...
    for d in range(10):
        ...
        dec_diff[d] += t[(t > edges[d]) & (t <= edges[d + 1])].mean() - c[m].mean()
dec_diff /= W
...
fig.tight_layout()
fig.savefig(HERE / "practice_3_4_tail.png", dpi=150)
...
# %%
REPS_AA = 3_000
levels = {"raw": 0, "win 5/95 (общий, пре-период)": 0, "win 5/95 (общий, pooled)": 0,
          "win 5/95 (РАЗДЕЛЬНЫЕ)": 0, "log": 0, "ранги (Манн-Уитни)": 0}
...
REPS_AB = 2_000
est = {"raw": [], "win 5/95 общий (пре-период)": [], "win 5/95 раздельные": []}
for _ in range(REPS_AB):
    c, t = world_tail()
    est["raw"].append(t.mean() - c.mean())
    est["win 5/95 общий (пре-период)"].append(
        np.clip(t, None, CAP95).mean() - np.clip(c, None, CAP95).mean())
    est["win 5/95 раздельные"].append(
...
print("   вместе с шумом квантилей; эстиманд не определён ни в одном дизайне-доке.")
...
fig.tight_layout()
fig.savefig(HERE / "practice_3_4_thresholds.png", dpi=150)
...
print(f"1) SD чека {sd_raw:,.0f} ₽ -> винзоризация 5/95 {sd_w95:,.0f} ₽: нужно в {(sd_raw / sd_w95) ** 2:.1f} раза меньше юзеров".replace(",", " "))
...
print(f"4) Раздельные трешолды: в A/A alpha = {levels['win 5/95 (РАЗДЕЛЬНЫЕ)'] / REPS_AA:.0%} (номинал 5%);")
print("   в A/B оценка зависит от шума квантилей — эстиманд не определён. Порог: один на обе группы,")
print("   посчитан по пре-периоду (или объединённой выборке) и зафиксирован в дизайн-доке ДО запуска.")
print("5) A/A-уровень — обязательная проверка любой трансформации: raw, win, log, ранги держат 5%.")
