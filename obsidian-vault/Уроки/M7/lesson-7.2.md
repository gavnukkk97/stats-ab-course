...
**Цель урока:** развести assignment, exposure и trigger; собрать метрики из событий и оценить точность и стоимость агрегации.
...
Три разных события, которые новички склеивают: **assignment** (юзеру посчитан вариант), **exposure** (фактический первый контакт, не автоматический ITT-знаменатель), **trigger** (юзер столкнулся с изменённой механикой — база triggering-анализа из 4.4). Логировать нужно экспозицию, в момент показа, до рендера варианта (Авито: экспозиция = показ фичи). Правила выживания:
...
![Exposure logging: точка входа юзера в тест — святое; дедуп и](images/v86_exposure_logging.png)
...
Между сырьём и статистикой — промежуточный слой: **юнит-таблица эксперимента**, одна строка на юнит (группа, экспозиция и её дата, метрики юнита за период после экспозиции). Наш пайплайн: 924 тыс. событий за период → 300 тыс. строк юнит-таблицы (сжатие ×3,1; правило начала окна фиксируется заранее; отбор по экспозиции не автоматически ITT). Дальше любой анализ (t-тест, CUPED из 3.5, линеаризация из 3.3, сегменты) читает только её. Это и граница приватизации (персональные данные живут в одном месте с понятным доступом), и граница стоимости: батч анализов не трогает сырьё. Twitter формулирует то же как «как можно больше вычислений в map-фазе»: агрегаты строятся по ключу (юзеру), глобальная статистика — последним шагом.
...
**Бакетизация:** hash(salt,user_id)%K задаёт технические группы хранения. Для обычной средней суммирование n/sum/sumsq точно восстанавливает пользовательский Welch (практика содержит проверку). Невзвешенные средние бакетов и ratio по bucket sums — другие, приближённые процедуры; им нужна калибровка. Нельзя обещать сохранение точности для любой метрики, сегмента и K.
...
| exposure-лог | дедуп на входе (LRU −80% объёма, Uber), честный ts | без дедупа — SRM (наш p=1,6·10⁻¹¹) | диагностика контакта; ITT — assignment |
...
![Бакетизация: миллион юзеров → сотни строк-агрегатов; так пла](images/v87_bucketization.png)
...
| данные для анализа | 924 тыс. событий | юнит-таблица 300 тыс. строк | ×3,1 меньше; популяция и окно определены отдельно |
...
Файл [practice_7_2.py](practice/practice_7_2.py) (percent-формат, numpy/scipy/matplotlib/pandas + hashlib/time, seed, Agg). Пять блоков:

1. **Exposure-лог**: 300 тыс. юзеров, экспозиции в первые 72 ч; дубли от асимметричных ретраев (контроль +4%, тест +7%) → сырой лог 316 475 строк: без дедупа 156 343/160 132, SRM p=1,6·10⁻¹¹; с дедупом p=0,30. Честный timestamp: 17,1% экспозиций меняют дату (батч 1 ч + офлайн-мобайл), день-1: 100 130 по контакту против 83 109 по логам. График [practice_7_2_dedup.png](images/practice_7_2_dedup.png).
2. **Юнит-таблица**: 924 018 событий → 300 000 строк (×3,1), окно начинается в общей eligible-точке; это triggered-анализ при условии независимого от treatment включения.
3. **Бакетизация**: t-тест поюзерно (300k строк) vs бакетные средние K=1000 и K=100 (2k/200 строк): lift GMV 3,182%/3,178%/3,179%, |Δ| ≤ 0,004%; ratio GMV/сессию дельта-методом поюзерно и по 1000 бакетам; актуальные оценки и z печатает практика. GMV и сессии учитываются в одном окне от контакта до конца теста.
4. **Экономика (time.perf_counter, лучшая из 30)**: партия 100 экспериментов × 300 срезов: поюзерно 0,31 мс/анализ → 9 с, 9,0 млрд строк; бакетно GROUP BY 5,9 мс × 100 + 29 мкс × 30 000 = 1,5 с, 90 млн строк — ×6 CPU, ×100 строки. График [practice_7_2_buckets.png](images/practice_7_2_buckets.png).
5. **Партиционирование**: озеро 6 млн строк (100 тестов × 30 дней); запрос одного теста за 11 дней: 6,00 млн / 2,20 млн / 22 тыс. строк — ×273, время ×15. График [practice_7_2_partitions.png](images/practice_7_2_partitions.png).
...
K=100 не является универсальной гарантией: один близкий lift на одном seed ничего не говорит о калибровке. Больше K даёт больше наблюдений на уровне бакетов, но меньше пользователей внутри каждого; в маленьких сегментах возникают пустые бакеты. Невзвешенное среднее бакетных средних меняет веса пользователей. Для средней храните n/sum/sumsq и восстанавливайте точный Welch — это добавлено в практику. Для ratio и иных bucket-оценщиков сравнивайте coverage, power и false-positive rate на нескольких DGP с MC-интервалами, а не только две точки lift.
</details>
...
2 млн × 365 × 150 байт ≈ 110 ГБ/год сырых экспозиций (без реплик и кодеков; Parquet сожмёт, но планёр от этого не меняется). Долговременно по принятой retention policy — агрегаты (тест × метрика × срез × бакет × день): при 1000 бакетов это тысячи строк на тесто-день — копейки, а сохранённые агрегаты позволяют воспроизвести поддерживаемые ими расчёты. Юнит-таблицы — на срок аудита теста + N месяцев (у нас ~19 МБ на тест — можно щедрее). Сырьё — TTL 90 дней с партициями по дате: replay недавних инцидентов возможен, старые тесты живут в агрегатах; плюс снапшоты определений метрик к каждому отчёту — иначе «вечные» агрегаты не интерпретируемы.
</details>
...
1. Три события: assignment ≠ exposure ≠ trigger. Логируется экспозиция: первый показ, до рендера варианта; ITT-знаменатель берётся из назначения eligible-пользователей, включая неэкспонированных.
...
9. Бакетизация: bucket = hash(salt, user) % K. Для средней (n, sum, sum_sq) по полным пользовательским окнам позволяют точно восстановить Welch. t-тест по невзвешенным бакетным средним — другой, приближённый анализ; для ratio нужны также дисперсия знаменателя и ковариация. Дневные суммы нельзя считать независимыми пользователями.
10. Стоимость: практика измеряет одну операцию и экстраполирует условную партию. Это cost model, а не выполненный benchmark 100×300 разных срезов. Сравнивайте полную стоимость подготовки, хранения, обновления и анализа на своей нагрузке.
...
### Популяция и пределы агрегации

Различайте assignment, exposure и trigger. Для ITT сохраняются все назначенные eligible-пользователи. Для triggered estimand требуется общий безопасный trigger до различий вариантов или корректное counterfactual logging. Дедуп экспозиций не восстанавливает неизвестные пропуски и не делает post-treatment фильтр безопасным.

Близость пользовательского и бакетного результата в примере иллюстрирует конкретные данные. Точное восстановление средней из n/sum/sumsq отличается от приближённого анализа bucket means. Для перцентилей эти достаточные статистики не подходят. Стоимость 300 разных метрик/срезов нельзя получить бесплатно из агрегации одной GMV.

В практике 7.2 GMV и сессии ограничены одним post-exposure окном, включая долю первого дня после контакта. Участники с нулевыми заказами и сессиями остаются в юнит-таблице. Время контакта в генераторе независимо от варианта; в продукте этот механизм нужно обосновать до выбора triggered estimand.
