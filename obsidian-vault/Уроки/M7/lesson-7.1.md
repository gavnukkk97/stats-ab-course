...
**Цель урока:** собрать системный взгляд на сплит-платформу как на продукт: четыре компонента любой платформы (портал / сервис назначений / логирование / анализ — Microsoft ExP); канон Google KDD'10 «домен → слой → 1000 бакетов» с приоритетными типами аллокации и launch-слоями; ёмкость слоя как инженерная величина (сколько тестов каких размеров влезает, цена слота в MDE: тест на 5% видит эффект в 4,47 раза крупнее); осознанный выбор хэша — детерминизм/равномерность/лавинность вы проверяли в 4.2, теперь эмпирика Microsoft: SpookyHash/MD5 дают независимые сплиты, FNV — нет (наши числа: лавинность 10,7 из 64 бит, равномерность рушится с одной солью p≈10⁻³⁴ и проходит с другой p=1,0 — непредсказуемость и есть проблема); sticky assignment — где физически живёт назначение (детерминированный хэш как «хранение»: LinkedIn 99,98% вычислений локально; CAP-цена stateful-альтернативы — 240× cache QPS и 26–390 ТБ/день) и что делать при смене ключа cookie→account (без переходного персиста 49,8% сохранили вариант, 17,6% базы сменили группу посреди теста); feature flags vs эксперименты; edge- и мобильный контур.
...
Ключевой ход — **1000 бакетов на слой**: `bucket = f(cookie, layer) % 1000`. Юзер всегда «сидит» в своём бакете, а эксперимент занимает диапазон. Следствия: (а) аллокация теста — это выделение диапазона, а не «пересылка людей», перераспределение долей при ramp-up не трогает данные; (б) ёмкость слоя конечна и видна — карта занятости как карта переговорных; (в) конфликты ловятся до запуска («нужно 10%, макс. свободный непрерывный диапазон 5%» — наш конфликт-детектор в практике).
...
**Ёмкость и цена слота.** Слой = 1000 бакетов, значит на одном слое помещается floor(1/s) тестов доли s: четыре по 25%, десять по 10%, двадцать по 5%. Цена слота — из 3.1: доля f даёт выборку N·f, а MDE ∝ 1/√N, то есть **MDE(f) = MDE(100%)·√(1/f)** и длительность на тот же MDE растёт в 1/f раз. По формуле KDD'10 для 95%/80%: $2N \ge 16\,(s/\theta)^2$ — для метрики GMV/юзера с CV≈2,5 и базы 2 млн юзеров за 2 недели: MDE на 100% = 0,71%, на 25% — 1,41%, на 5% — 3,16% (в 4,47 раза крупнее). Отсюда проектировочный расчёт: 20 команд × тест на 25% → нужно 5+ слоёв; тесты по 5% доступны только для жирных эффектов (или длительных). Это и есть вход в «платформа как календарь гипотез» (7.4).

![Домены, слои и 1000 бакетов (Google KDD'10): слои — независи](visuals/v85_domains_layers.png)
...
**Персист (когда он нужен):** хранить нужно не «имя варианта», а **позицию на отрезке [0,1)** (Philip Walton / GrowthBook sticky bucketing): cookie несёт число, вариант = подотрезок своей доли. Тогда ramp-up — расширение отрезка без смены групп; а имя варианта, записанное в cookie, ломается при каждом изменении долей. Персист уместен ровно там, где меняются доли на живых юзерах (мягкий ramp, миграция ключа) — и только как переходный слой.
...
![Миграция ключа сплита: без sticky-моста половина юзеров меня](practice_7_1_sticky.png)
...
![Анатомия платформы: портал, сплит-сервис, логирование, анали](visuals/v84_platform_architecture.png)
*Рис. 3. Анатомия платформы: портал, сплит-сервис, логирование, анализ — и гейты, которые нельзя обойти.*

## Разбор на данных

Проектирование сплита «ЕдаДома» на 20 команд (все числа — из практики 7.1). Шаг 1 — карта слоёв: домен «search» (100%): слои `ranking`, `ui`, два чекаут-слоя; домен «logistics» (50%): `courier_algo`; плюс CRM-слой. Итог: 6 слоёв → 24 одновременных теста по 25% или 120 по 5%. Шаг 2 — паспорт чувствительности: GMV/юзера CV≈2,5, база 2 млн/2 нед → MDE на 100% трафика 0,71% (2N ≥ 16·(s/θ)²); тест на 25% видит 1,41%, на 5% — 3,16%. Вывод для бэклога: тонкие эффекты (<1%) тестируем только на ≥50% слое или длиннее; «быстрые» 5%-слоты — для грубых проверок.

| доля слоя | тестов на слой | MDE (отн.) | MDE к 100% | недель на ту же точность |
|---|---|---|---|---|
| 100% | 1 | 0,71% | 1,00 | 1 |
| 50% | 2 | 1,00% | 1,41 | 2 |
| 25% | 4 | 1,41% | 2,00 | 4 |
...
3. **Персист имени варианта вместо позиции.** Делают: в cookie пишут "variant_B". Грозит: любой ramp-up/изменение долей делает запись невалидной. Правильно: хранить позицию на [0,1) (или бакет), вариант выводить из текущего конфига.
...
Файл `practice_7_1.py` (percent-формат, numpy/scipy/matplotlib/pandas + hashlib/time, seed, Agg). Четыре блока:

1. **Лаборатория хэшей** (20k последовательных id): лавинность MD5 64,0/128 против FNV 10,7/64; переворот каждого бита (FNV: 0,00–0,99, бит #36 прикован); равномерность 1000 бакетов — FNV p≈10⁻³⁴ с одной солью и p=1,0 с другой; независимость двух солей χ²=315 (p<0,0001) против MD5 p=0,80. График `practice_7_1_hash.png`.
2. **Сплит-сервис классом**: домены (search 100%, logistics 50%) → слои с солями → 1000 бакетов; first-fit аллокация слотов; конфликт-детектор с диагностикой («нужно 10%, свободно 0%» / фрагментация «свободно 10%, но непрерывного куска на 15% нет»); версии конфига v5→v12 и неизменность снапшота; приоритетная лестница assign(user_id → cookie). График `practice_7_1_capacity.png` (карта занятости слоёв + кривая MDE ×√(1/f)).
3. **Ёмкость**: таблица «доля → тестов на слой → MDE → недель»; 2N ≥ 16·(s/θ)² на CV=2,5 и 2 млн юзеров: 0,71% на 100%; сценарий 6 слоёв → 24×25% или 120×5%.
4. **Sticky**: 120k юзеров, 35% с account_id — наивная миграция (49,8% сохранили, 17,6% базы сменили группу) против sticky-перехода (100%); churn 4%/мес × 4 недели: 3,9% затронуты, ~2,0% сменили группу, эффект 4,72%→4,52% (cookie) против 4,71% (account). График `practice_7_1_sticky.png`.
...
4. Ёмкость: на слое floor(1/s) тестов доли s; MDE(f) = MDE(100%)·√(1/f), длительность ×1/f; 2N ≥ 16·(s/θ)² (95%/80%). Тонкие эффекты — большие доли или время.
...
6. Sticky: по умолчанию stateless-хэш (LinkedIn: 99,98% локально, 41k тестов); персист — позиция на [0,1), не имя варианта; Persist только в переходных окнах.
...
- Tang D., Agarwal D., O'Brien D., Meyer M. — «Overlapping Experiment Infrastructure: More, Better, Faster Experimentation» (Google, KDD 2010) — домены/слои/1000 бакетов, приоритетные аллокации, bias-id, launch-слои, capacity, 2N ≥ 16(s/θ)² — dl.acm.org/doi/10.1145/1835804.1835810 (`materials/web/research-ab-platform.md` §1).
