...
Запуск из корня репозитория: python3 course/modules/M2/practice/practice_2_1.py
...
def power_theory(effect, n):
    """Двусторонняя normal-мощность: сумма вероятностей двух хвостов."""
    z_a = stats.norm.ppf(1 - ALPHA / 2)
    return normal_power(effect, SIGMA * np.sqrt(2 / np.asarray(n, dtype=float)), ALPHA)
...
print(f"       KS-тест равномерности: p = {ks.pvalue:.2f}  (сравните с заранее выбранным α; это непрерывная модель)")
print()
...
fig.tight_layout()
fig.savefig(IMAGE_DIR / "practice_2_1_pvalue_hist.png", dpi=150)
plt.close(fig)
print("\nСохранено: practice_2_1_pvalue_hist.png")

# %% [markdown]
# Шаг 2. (в) Кривые мощности от n для трёх эффектов (+30 / +60 / +120 руб.).
# Точки — симуляция (1000 тестов на точку) с MC-интервалами, линии — нормальная теория.
...
print("-" * 78)
print(f"   {'эффект':>9} | {'d':>5} | {'n для 80% теории':>16} | {'первая точка >=80%':>19}")
power_sim = {}
for e in effects:
    ...
    crossings = np.flatnonzero(pw >= .8)
    n80_sim = ns[crossings[0]] if len(crossings) else np.nan
    print(f"   {e:>7.0f} руб | {e / SIGMA:>5.2f} | {n80_th:>16.0f} | {n80_sim:>19.0f}")
print("   Симуляционная граница — грубое пересечение шумной сетки, не точный расчёт n.")

# %%
fig, ax = plt.subplots(figsize=(8.6, 4.5))
for e in effects:
    pw = power_sim[e]
    bounds = np.array([mc_interval(round(rate * REPS_CURVE), REPS_CURVE) for rate in pw])
    ax.errorbar(ns, pw, yerr=np.maximum(0, np.array([pw - bounds[:, 0], bounds[:, 1] - pw])),
                fmt="o", ms=3, capsize=2, elinewidth=.7, color=colors[e], alpha=.55)
...
ax.set_title("Мощность растёт с n и с величиной эффекта:\nточки и усы — симуляция с 95% MC CI, линии — теория")
ax.legend(fontsize=9, loc="center right")
fig.tight_layout()
fig.savefig(IMAGE_DIR / "practice_2_1_power_curves.png", dpi=150)
plt.close(fig)
...
print(f"   Type M (преувеличение)               : x{type_m(eff_sig, TRUE_EFF):.2f}")
...
ax.axvline(eff_sig.mean(), color="#C44E52", lw=2.2, ls="--",
           label=f"среднее значимых {eff_sig.mean():+.0f} руб. (x{type_m(eff_sig, TRUE_EFF):.1f})")
...
fig.tight_layout()
fig.savefig(IMAGE_DIR / "practice_2_1_type_m.png", dpi=150)
plt.close(fig)
...
print("1) В этой непрерывной модели под H0 p-value примерно равномерен: доли p<0.05 и p<0.01 совпадают с")
print("   номиналами в пределах MC-погрешности. Примерно каждый 20-й A/A-тест «прокрасится» — это")
...
print("MC alpha:", mc_report(np.count_nonzero(p_aa < ALPHA), len(p_aa)))
print("MC power:", mc_report(np.count_nonzero(p_ab < ALPHA), len(p_ab)))
print("MC power Type M world:", mc_report(np.count_nonzero(sig), len(sig)))
print("Дискретная монета n=10: P(p≤.05) =", sum(stats.binom.pmf(k,10,.5) for k in range(11) if stats.binomtest(k,10,.5).pvalue<=.05))
