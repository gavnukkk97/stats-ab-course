...
Запуск из корня репозитория: python3 course/modules/M2/practice/practice_2_4.py
...
fig.tight_layout()
fig.savefig(IMAGE_DIR / "practice_2_4_aa20.png", dpi=150)
...
# В ЭТОЙ гауссовой модели корреляция снижает FWER; это не общее свойство зависимости.
...
print("   В данной однофакторной гауссовой модели FWER ниже независимой")
...
for values in (fwer_ind, fwer_corr):
    values=np.asarray(values); lo,hi=mc_interval(values*WORLDS3,WORLDS3)
    ax.errorbar(m_grid, values, yerr=[values-lo,hi-values], fmt="none", ecolor="gray", capsize=3)
...
fig.tight_layout()
fig.savefig(IMAGE_DIR / "practice_2_4_fwer.png", dpi=150)
...
for m, fi, fc in zip(m_grid, fwer_ind, fwer_corr):
    print(f"m={m} independent", mc_report(round(fi*WORLDS3), WORLDS3), "correlated", mc_report(round(fc*WORLDS3), WORLDS3))
