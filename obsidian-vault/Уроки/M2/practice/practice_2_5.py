...
Запуск из корня репозитория: python3 course/modules/M2/practice/practice_2_5.py
...
# %% [markdown]
# Учебные времена повторного заказа полностью наблюдены у каждого участника.
# В реальном продукте учитывайте цензурирование; нельзя отбирать только вернувшихся.
...
fig.tight_layout()
fig.savefig(IMAGE_DIR / "practice_2_5_anova.png", dpi=150)
...
print(f"   6 попарных t без поправки: FWER = {pairwise_fwer / WORLDS:.1%}"
      f" (ориентир НЕЗАВИСИМЫХ тестов 1-0.95^6 = {1 - 0.95 ** 6:.0%})")
...
rates=np.array([np.mean(p_omnibus<ALPHA), pairwise_fwer/WORLDS])
lo,hi=mc_interval(rates*WORLDS,WORLDS)
axes[1].errorbar([0,1],rates,yerr=[rates-lo,hi-rates],fmt="none",ecolor="black",capsize=4)
axes[1].axhline(ALPHA, color="black", ls="--", lw=1.6)
axes[1].set_ylabel("доля миров с >=1 «открытием» под H0")
axes[1].set_title(f"Одна проверка против шести: {np.mean(p_omnibus<ALPHA):.1%} против {pairwise_fwer/WORLDS:.1%}")
fig.suptitle("Честный тест держит 5% под H0 — множественность ломает уровень",
             fontsize=12, fontweight="bold")
fig.tight_layout()
fig.savefig(IMAGE_DIR / "practice_2_5_puniform.png", dpi=150)
...
print("ANOVA MC", mc_report(np.count_nonzero(p_omnibus<ALPHA), WORLDS))
print("pairwise FWER MC", mc_report(pairwise_fwer, WORLDS))
