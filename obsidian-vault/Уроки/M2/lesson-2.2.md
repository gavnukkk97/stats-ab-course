...
**Цель урока:** научиться выбирать между одновыборочным, двухвыборочным (Стьюдент), Уэлча и парным t-тестом; понимать статистику $t$ как «эффект/SE»; знать, что условия применимости — это независимость и нормальность *средних* (правило Денга $100\,s^2$), и уметь проверять сам тест симуляцией его фактической α.
...
История, случающаяся каждый месяц: аналитик прогнал `scipy.stats.ttest_ind(a, b)` — а у него `equal_var=True` по умолчанию. В тесте шла акция «двойной кэшбэк», дисперсия чека выросла втрое, группа к тому же меньше — и фактическая доля ложных прокрасов уехала с 5% к 37%: каждый третий «значимый» результат — мусор. Никакой ошибки в коде нет — ошибочна версия теста. Ниже мы устроим это безобразие лично и посмотрим, как держится тест Уэлча.
...
![От z к t: σ оценена по выборке → хвосты тяжелее → критически](visuals/v24_z_to_t.png)
*Рис. 1. От z к t: σ оценена по выборке → хвосты тяжелее → критический порог строже (1,96 → 2,35 при n=4).*
...
df может получиться **дробным** и меньше $n_1+n_2-2$ — Уэлч «не верит» малой шумной группе и перестраховывается. При равных дисперсиях Уэлч вырождается в Стьюдента (потеря — доли процента мощности), поэтому это **безрисковый дефолт**: так поступают R (`t.test` по умолчанию) и любой аккуратный АБ-пайплайн. В scipy: `stats.ttest_ind(a, b, equal_var=False)`.

Заметьте симметрию примера: при *равных* n pooled-SE совпадает с SE Уэлча — Стьюдент ломается только на сочетании «разные σ **и** разные n». Но равенство n в реальном АБ всегда приблизительное (отсечение ботов, потери данных), поэтому правило простое: Уэлч всегда.

![Неравные дисперсии + разные n: Стьюдент врёт про α (37%), Уэ](visuals/v17_welch.png)
...
где $r$ — корреляция между «до» и «после». При высокой $r$ вычитание гасит почти весь разброс: NPS-скор с $s_x = s_y = 12$ и $r = 0{,}9$ даёт $s_d = \sqrt{144+144-2\cdot0{,}9\cdot144} \approx 5{,}4$ — шум в 2,2 раза тише, а требуемый n (он пропорционален дисперсии, урок 2.1) меньше в $\approx 10$ раз. В нашей практике с $r = 0{,}92$ экономия — ×11.
...
![Парный дизайн = бесплатное снижение дисперсии: SD разностей ](visuals/v18_paired_design.png)
*Рис. 3. Парный дизайн = бесплатное снижение дисперсии: SD разностей вдвое меньше — задел на CUPED в М3.*
...
**2. Нормальность — про средние, а не про данные.** t-тесту нужна приблизительная нормальность $\bar{x}$, а не $x_i$ — это ЦПТ из урока 1.3. «Народная теорема» «n > 30 ⇒ данные нормальные» перепутывает распределения. Скорость сходимости зависит от асимметрии: **правило Денга — порядка $100\,s^2$ наблюдений** ($s$ — skewness). Чек «ЕдаДома» (медиана 700 ₽, $\sigma_{\log} = 1{,}2$) имеет $s \approx 11{,}2$ — честная работа t-теста начинается с ~12 500 наблюдений; n = 30 — это в 400 раз меньше. Проверять нормальность Shapiro–Уилка на сырых чеках бессмысленно: на больших n он отвергнет всегда (данные-то действительно не нормальны — и не надо), а на малых сам ничего не гарантирует. Чек корректности — **A/A-симуляция фактической α** (метод ГМ): генерируем H0-миры, гоняем тест, сравниваем долю p < 0,05 с номиналом.

Полезная деталь: разность двух независимых одинаково скошенных средних почти симметрична, поэтому **двухвыборочный тест выносливее одновыборочного** (Денг отмечает симметричность Δ как ускоритель сходимости; виден и в нашей практике). Если тест всё же не держит номинал — бутстрап, перестановки, ранги (урок 2.3) или трансформации метрики (М3.4).

**3. Равенство дисперсий** — лечится автоматически выбором Уэлча (раздел 3). Предварительный F-тест на равенство дисперсий («сначала проверю, можно ли Стьюдента») — антипаттерн: сам шумный, решает не то и при малых n часто ошибается; рекомендация литературы по практической статистике одна — просто всегда Уэлч.

![A/A на логнормальном чеке: двухвыборочный t-тест держит α (Ц](visuals/v25_skew_alpha_n.png)
...
Итоговый формат вывода теста: **оценка эффекта + CI + p + n (+ d)**: «+40 ₽, 95% CI [12; 68], p = 0,006, d = 0,13, n = 2000 на группу». CI и тест согласованы (ноль вне 95% CI ⇔ p < 0,05 — урок 1.4), но CI информативнее: показывает величину и точность, а не только «не шум».
...
1. **`ttest_ind(a, b)` с забытым `equal_var=False`.** Делают: дефолт scipy (Стьюдент) на группах с разными σ и n. Грозит: фактическая α до 37% — треть «прокрасов» ложные; или обратная конфигурация — тест тихо не отвергает вообще. Правильно: Уэлч всегда (`equal_var=False`); при равных σ потери — доли процента.
2. **Shapiro на сырых данных как «проверка условий t-теста».** Делают: «p < 0,05 ⇒ данные не нормальны ⇒ t-тест нельзя». Грозит: на больших n отвергается всегда (и незачем), на малых — не защищает; тест заброшен зря или выбран неподходящий «непараметрический» с другой H0 (урок 2.3). Правильно: нормальность нужна средним — правило Денга $100\,s^2$ + A/A-симуляция фактической α.
3. **«n = 30 достаточно» на скошенном чеке.** Делают: пилот на 30 заказов в группе, t-тест, вывод. Грозит: эмпирическая α одновыборочного теста ~15% (втрое выше номинала) — ложные прокрасы в пилотах. Правильно: $n \sim 100\,s^2$ (для чека ~12 500); меньше — бутстрап/ранги (2.3), лог/винзоризация (М3.4).
...
Файл `practice_2_2.py` (percent-формат, только numpy/scipy/matplotlib, seed зафиксирован). Четыре блока:

1. **(а) равные дисперсии, n₁ = n₂ = 1000**: 10 000 A/A — эмпирическая α Стьюдента и Уэлча (`practice_2_2_student_welch.png`, левая панель).
2. **(б) неравные дисперсии + разные n** (n = 1000/SD 300 против n = 100/SD 900) и обратная конфигурация — правая панель.
3. **(в) парный vs непарный** на одних данных (NPS 60 курьеров): SD групп против SD разностей, оба теста, CI, экономия выборки (`practice_2_2_paired.png`).
4. **(г) скошенный чек** (логнормал: медиана 700, $\sigma_{\log} = 1{,}2$): эмпирическая α одно- и двухвыборочного t-теста при n = 30/300/3000 (`practice_2_2_skew_alpha.png`).
...
$\bar d = 480/6 = 80$; $\sum(d_i-\bar d)^2 = 21\,400$; $s_d^2 = 21\,400/5 = 4\,280$, $s_d \approx 65{,}4$; $SE = 65{,}4/\sqrt{6} \approx 26{,}7$; $t = 80/26{,}7 \approx 3{,}0$ при $df = 5$. Критическое $t_{0{,}975,\,5} = 2{,}571 < 3{,}0$ — отвергаем H0, p ≈ 0,030: программа подняла среднюю выручку. Заметьте: одна разность отрицательная — тест работает с этим сам, «выбросом» её делать не нужно.
</details>
...
Честный $SE = \sqrt{300^2/2000 + 600^2/500} = \sqrt{45 + 720} \approx 27{,}7$ ₽. Pooled: $s_p^2 \approx 143\,975$, $SE_{\text{Стьюдент}} = \sqrt{143\,975 \cdot (1/2000 + 1/500)} \approx 19{,}0$ ₽. Шум занижен в 1,46 раза, порог $z = 1{,}96$ фактически соответствует $z \approx 1{,}35$: $\alpha \approx 2\Phi(-1{,}35) \approx 18\%$ — почти каждый пятый «прокрас» ложный. Уэлч считает честный SE — и держит 5%.
</details>
...
**Задача 5.** Эффект +40 ₽ при pooled SD 300 ₽. Посчитайте d Коэна и оцените n на группу для мощности 80% (α = 5%). Сверьте с кривыми мощности из `practice_2_1.py`.

<details><summary>Ответ</summary>

$d = 40/300 \approx 0{,}133$. $n \approx 2(z_{0{,}975}+z_{0{,}8})^2/d^2 = 2 \cdot 7{,}85 / 0{,}133^2 \approx 880$ на группу. Это ровно между кривыми d = 0,1 (1570) и d = 0,2 (393) из практики 2.1 — формула и симуляция согласованы. Отчёт: «+40 ₽, 95% CI [12; 68], p = 0,006, d = 0,13, n ≈ 900 на группу».
</details>
...
3. Уэлч: $SE=\sqrt{s_1^2/n_1+s_2^2/n_2}$, df Уэлча–Саттертуэйта (дробные — это нормально); при равных σ совпадает со Стьюдентом — дефолт без рисков.
...
7. Правило Денга: n ~ 100·s² для честной нормальности среднего; чек со skew 11 → ~12 500 наблюдений; «n = 30» — миф; двухвыборочный тест выносливее одновыборочного (Δ симметричнее).
