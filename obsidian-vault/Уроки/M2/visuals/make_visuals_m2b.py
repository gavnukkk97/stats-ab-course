...
# Запуск: python3 make_visuals_m2b.py
import matplotlib
matplotlib.use("Agg")
...
# ============================================================
ks = np.arange(11)
p_two = 2 * np.array([stats.binom.cdf(min(k, 10 - k) - 1, 10, 0.5) if min(k, 10 - k) > 0 else 1.0
                      for k in ks])
p_two = np.clip(p_two, 0, 1)
...
ax.set_title("Линейка, проградуированная нулевой гипотезой: p-value = насколько далеко данные от нормы H0")
ax.text(0.02, 0.72, "7+ орлов из 10: p=18% — не удивительно\n8: p=5,5% · 9: p=2% — уже подозрительно",
        transform=ax.transAxes, fontsize=9.5, bbox=dict(boxstyle="round,pad=0.3", fc="#fff3cd"))
fig.tight_layout(); fig.savefig("v23_coin_pvalue_scale.png", bbox_inches="tight"); plt.close(fig)
...
ax.text(2.05, 0.40, "критические 2,5%:\n1,96 | t₂₀=2,09 | t₃=2,35", fontsize=9)
ax.set_title("Почему t вместо z: σ оценена → хвосты тяжелее → порог строже")
ax.legend()
fig.tight_layout(); fig.savefig("v24_z_to_t.png", bbox_inches="tight"); plt.close(fig)

# ============================================================
# 25. Скошенный чек: эмпирическая α от n
# ============================================================
def alpha_curve(n_arr, reps=3000, two_sample=True):
    res = []
    for n in n_arr:
        rej = 0
        for _ in range(reps):
...
ax.set_title("Двухвыборочный t-тест держит α на скошенных данных (ЦПТ работает)\n(одновыборочный при n=30 давал бы ~15% — см. practice_2_2)")
ax.legend(fontsize=9, loc="upper right")
fig.tight_layout(); fig.savefig("v25_skew_alpha_n.png", bbox_inches="tight"); plt.close(fig)
...
ax.set_title(f"Подглядывание в A/A-тесте: {crossed}/30 «зелёных» траекторий прокрасились в первую неделю\n(эффекта нет вообще) — и вернулись")
fig.tight_layout(); fig.savefig("v26_peeking.png", bbox_inches="tight"); plt.close(fig)
...
fig.tight_layout(); fig.savefig("v27_anova_ss.png", bbox_inches="tight"); plt.close(fig)
...
fig.tight_layout(); fig.savefig("v28_anova_f.png", bbox_inches="tight"); plt.close(fig)
...
fig.tight_layout(); fig.savefig("v29_rank_transform.png", bbox_inches="tight"); plt.close(fig)
...
reps = 2500
for _ in range(reps):
    a = rng.lognormal(np.log(700), 1.0, 40)
    b_eq = rng.lognormal(np.log(700), 1.0, 40)
    b_un = rng.lognormal(np.log(700), 2.0, 40)
...
axes[0].set_ylabel("доля p<0.05 в A/A, %")
axes[0].set_title("Уровень: кит в одной группе раздувает σ̂\n→ интервалы шире «нужного», но n мал — α уезжает", fontsize=9.5)
for i, v in enumerate([rej_eq / reps * 100, rej_un / reps * 100]):
    axes[0].text(i, v + 0.3, f"{v:.1f}%", ha="center", fontweight="bold")
n_ = 200
pw1 = stats.norm.cdf(30 / (np.sqrt((np.e**1.0 - 1) * np.e) / np.sqrt(n_) * np.sqrt(2)) - 1.96) if False else None
sd1 = np.sqrt((np.exp(1.0) - 1) * np.exp(2 * np.log(700) + 1.0))
sd2 = np.sqrt((np.exp(4.0) - 1) * np.exp(2 * np.log(700) + 4.0))
ns_arr = np.arange(50, 3000, 50)
pw_no = stats.norm.cdf(30 / (sd1 * np.sqrt(2 / ns_arr)) - 1.96)
pw_wh = stats.norm.cdf(30 / (np.sqrt((sd1**2 + sd2**2) / 2) * np.sqrt(2 / ns_arr)) - 1.96)
axes[1].plot(ns_arr, pw_no, color=C_TRUE, lw=2.4, label="без китов")
axes[1].plot(ns_arr, pw_wh, color=C_BAD, lw=2.4, label="кит в тесте (σ×≈30)")
axes[1].axhline(0.8, color="gray", ls=":", lw=1.3)
axes[1].set_xlabel("n на группу"); axes[1].set_ylabel("мощность (эффект +30 ₽)")
axes[1].set_title("Мощность: раздутая китом дисперсия съедает чувствительность\nнужен кратно больший n", fontsize=9.5)
axes[1].legend(fontsize=9)
fig.suptitle("Два режима поломки t-теста на тяжёлых хвостах", fontsize=12, fontweight="bold")
fig.tight_layout(); fig.savefig("v30_whales_break_ttest.png", bbox_inches="tight"); plt.close(fig)
...
fig.tight_layout(); fig.savefig("v31_anova_cuped.png", bbox_inches="tight"); plt.close(fig)

print("Готово: v23–v31 (9 визуализаций) сохранены в", __file__.rsplit("/", 1)[0])
