...
**Цель урока:** выбрать модель распределения, оценить параметры и проверить её по гистограмме, ECDF и QQ-графику.
...
- **CDF:** $F(x)=P(X\le x)$. Квантиль уровня p∈(0,1) задаём обобщённой обратной: $q_p=\inf\{x:F(x)\ge p\}$. Для дискретного F равенство F(x)=p может не иметь решения; выборочные квантили также зависят от принятого правила интерполяции.
...
Мини-пример: $p = 0{,}12$, недельная когорта $n = 1000$: $sd = \sqrt{0{,}12 \cdot 0{,}88 / 1000} \approx 0{,}0103$ — конверсия гуляет ±1 п.п. от недели к неделе сама по себе. Увидели 11% — это шум; увидели 9,7% — это около 2,24 стандартных ошибок ниже 12%; три SE соответствуют примерно 8,92%. При $np \ge 5$ и $n(1-p) \ge 5$ биномиальное хорошо приближается нормальным — отсюда z-тест для долей (модуль 2).
...
Контрпример, критичный для практики: **число заказов на юзера за месяц**. Среднее 2,1, а дисперсия легко 6–8: юзеры неоднородны (у кого-то привычка, у кого-то отпуск). Это **overdispersion** (сверхдисперсия), $Var \gg Mean$ — Пуассон не подходит, рабочая модель — отрицательное биномиальное (Пуассон со случайной $\lambda$ по юзерам). Агрегаты по времени/территории тоже бывают сверхдисперсными из-за меняющейся интенсивности и зависимости событий. Для Пуассона нужны обоснованные независимость и интенсивность, а не просто подходящий уровень агрегации.
...
![Выбор числа бинов](images/v05_bins_choice.png)
...
![Как читать ECDF](images/v06_ecdf_reading.png)
...
- S-образный изгиб → отличие хвостов от выбранной модели; симметричные тяжёлые хвосты тоже дают S, это не автоматический признак асимметрии;
...
![QQ-plot: сырой чек против логарифма](images/v07_qq_plot.png)
...
Шаг 4. Что даёт модель: sd = √1,33 ≈ 1,15 заказа — шумовая полка индивидуума; район из 300 таких юзеров делает в неделю $400 \pm \sqrt{400} = 400 \pm 20$ заказов. Теперь «район сделал 415 вместо 400» — не новость, а «сделал 445» — 2,25σ; четырём σ соответствует 480. Распределение превратило число в суждение.
...
4. **Пуассон для поюзерных метрик «на веру».** Так делают: число заказов на юзера моделируют Пуассоном, потому что «это счётчик». Чем грозит: реальные юзеры сверхдисперсны (var/mean = 3–5), Пуассон занижает разброс в разы — SE и пороги занижены, частота ложных тревог растёт. Как правильно: проверять $s^2$ vs $\bar{x}$; при var > mean — отрицательное биномиальное или ресэмплинг с проверенными условиями независимости и достаточности хвоста.
...
Сначала выполните самостоятельную версию [`exercise_1_2.py`](exercises/exercise_1_2.py): прогноз → расчёт → письменное решение. Готовый [practice_1_2.py](practice/practice_1_2.py) ниже — разбор для сверки после попытки.

Файл [practice_1_2.py](practice/practice_1_2.py) (percent-формат; `python3 practice/practice_1_2.py`). Все генерации — `np.random.default_rng(2027)`.
...
- Для чистой логнормалы сравните MoM и MLE. Смесь с корпоративными заказами уже не принадлежит одному логнормальному семейству: MoM по построению воспроизводит среднее смеси, MLE логарифмов может лучше описывать тело, но недооценивать сумму денег. В практике добавлены implied mean/median и QQ/ECDF обеих fitted-моделей для смеси. Устойчивость одной оценки не доказывает корректности семейства.
...
3. Поюзерные счётчики обычно сверхдисперсны (var/mean ≫ 1) → отрицательное биномиальное; у агрегатов тоже бывает сверхдисперсия: проверяйте независимость и интенсивность.
