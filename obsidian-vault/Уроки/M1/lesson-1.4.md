...
**Цель урока:** построить bootstrap-интервал с правильной единицей пересэмплирования и распознать ограничения метода.
...
Пусть $\hat\theta = \hat\theta(x_1,\dots,x_n)$ — интересующая статистика. Вычислить реплики можно почти всегда, но валидный CI из них получается не автоматически: обычный n-out-of-n bootstrap, например, несостоятелен для некоторых статистик границы/максимума.
...
![Схема бутстрапа](images/v11_bootstrap_schema.png)
...
При симметричных репликах интервалы могут быть близки, но симметрия не доказывает покрытие. Percentile учитывает форму реплик, basic отражает ошибки вокруг оценки, BCa корректирует смещение и ускорение. Ни один метод не универсален при малом n, тяжёлом хвосте или нерегулярной статистике. В практике сравниваем покрытие percentile/basic/BCa на повторных независимых исходных выборках; QQ одной серии реплик для этого недостаточен.

Бонус: через CI можно проверять гипотезы — если гипотетическое значение параметра лежит вне 95% интервала, отвергаем H0 на уровне 5% (симулятор, урок 5). В полный рост это раскроем в М2 (p-value), а в М2.3 обсудим условия инвертирования CI в тест.

### 4. Для каких статистик бутстрап незаменим

- **медиана, перцентили** ($p_{95}$ времени доставки, $p_{99}$ времени загрузки — квантильные метрики из симулятора, урок 5): аналитические SE для квантилей существуют, но требуют знания плотности; bootstrap требует условий для квантили (например, достаточной плотности около неё);
...
Для среднего t-интервал часто проще и дешевле; bootstrap-интервал не обязан с ним совпадать и не гарантирует улучшения покрытия. Выбор зависит от распределения и процедуры CI. Планировать n для bootstrap-анализа можно симуляцией будущих экспериментов при заданных эффектах, а иногда — через асимптотическую SE.
...
![Ловушка объёма бут-выборки](images/v12_bootstrap_trap.png)
...
Что видим руками: (1) медиана бут-выборок — всегда либо значение из выборки, либо полусумма соседних в бут-выборке (не обязательно в исходной выборке) — статистика **квантуется**; (2) уже на пяти репликах разброс медиан [700; 850] — это и есть зародыш бут-распределения. С $B = 5000$ (практика) перцентили этого разброса станут устойчивыми.
...
7. **Бутстрап без проверки покрытия.** Метод и число реплик не заменяют достаточность исходных данных. Сравните percentile/basic/BCa или t-интервал симуляцией при реалистичном DGP; размер выборки можно планировать такой же симуляцией при заданном эффекте. Не выбирайте статистику только потому, что её CI уже.

## Практика

Сначала выполните самостоятельную версию [`exercise_1_4.py`](exercises/exercise_1_4.py): прогноз → расчёт → письменное решение. Готовый [practice_1_4.py](practice/practice_1_4.py) ниже — разбор для сверки после попытки.

Файл: [practice_1_4.py](practice/practice_1_4.py) (percent-формат, numpy/scipy/matplotlib, seed зафиксирован). Что делает:
...
4. **Проверка покрытия**: 300 независимых исходных выборок по 80 наблюдений; для каждой percentile/basic/BCa с 999 репликами. Выводим покрытие и MC CI; близкие оценки не означают превосходство.
5. **Визуализация**: гистограммы выборки и бут-распределений ([bootstrap_checks.png](images/bootstrap_checks.png)), QQ-plot бут-распределения медианы против нормального ([bootstrap_qq_median.png](images/bootstrap_qq_median.png)).
...
- CI среднего и медианы относятся к разным параметрам; меньшая ширина не делает медиану лучшей бизнес-метрикой. При 70% неплатящих медиана выручки равна нулю даже при росте ARPU — денежный эффект требует оценки среднего.
...
- QQ-plot описывает форму реплик условно на одной выборке; сам по себе он не подтверждает покрытие percentile-CI. Для выбора метода используйте повторные исходные выборки из отдельного блока проверки покрытия.
...
Возможны все целые значения от 2 до 12 включительно. Для чётного n медиана — полусумма двух центральных значений именно бут-выборки: четыре 2 и четыре 4 дают 3; восемь 2 дают 2. Любая пара из исходного набора может стать центральной благодаря повторениям. Распределение медианы дискретно, что особенно заметно при малом n.
</details>
...
4. Percentile CI $[q^*_{.025};q^*_{.975}]$ — один из вариантов; basic и BCa отличаются. Скошенность реплик не гарантирует корректное покрытие ни одного из них.
