...
axes[1].hist(before, bins=np.geomspace(before.min(), before.max(), 81), color="#4C72B0", edgecolor="white", linewidth=0.3)
...
fig.tight_layout()
fig.savefig(IMAGE_DIR / "practice_1_1_hist_logscale.png", bbox_inches="tight")
plt.close(fig)
...
fig.tight_layout()
fig.savefig(IMAGE_DIR / "practice_1_1_boxplot.png", bbox_inches="tight")
plt.close(fig)
...
true_std = before.std(ddof=0)  # population SD for empirical sampling with replacement
...
for ax, g in zip(axes, group_sizes):
    means = np.array([rng.choice(before.values, size=g, replace=True).mean()
                      for _ in range(REPS)])
...
fig.suptitle(f"Средние групп мечутся: шум падает как 1/√n (SD выручки = {true_std:.0f} ₽)",
             y=1.04)
fig.tight_layout()
fig.savefig(IMAGE_DIR / "practice_1_1_small_numbers.png", bbox_inches="tight")
plt.close(fig)
