...
fig.suptitle("Зоопарк распределений «ЕдаДома»: одна ось — симуляция, кривая — теория",
             y=1.01)
fig.tight_layout()
fig.savefig(IMAGE_DIR / "practice_1_2_distributions.png", bbox_inches="tight")
plt.close(fig)
...
fig.tight_layout()
fig.savefig(IMAGE_DIR / "practice_1_2_bins.png", bbox_inches="tight")
plt.close(fig)
...
fig.suptitle("Подгонка Пуассона по среднему: когда var/mean ≈ 1 — ок, когда ≫ 1 — нет",
             y=1.04)
fig.tight_layout()
fig.savefig(IMAGE_DIR / "practice_1_2_poisson_fit.png", bbox_inches="tight")
plt.close(fig)
...
print()
print(f"Диагностика смеси: MoM изменился (σ̂ {np.sqrt(sigma2_mom):.2f} → "
...
print("После подмешивания китов модель одной логнормалы неверна: методы описывают разные части смеси.")
print("MoM воспроизводит mean смеси; MLE может лучше описывать тело, но не общую выручку. Проверьте обе цели.")

# Diagnose both fitted models on the contaminated mixture.
fig, axes = plt.subplots(1, 2, figsize=(11, 4))
prob = (np.arange(len(mix)) + 0.5) / len(mix)
observed = np.sort(mix)
for label, mu_fit, sd_fit in [("MoM", mu_mom_m, sigma_mom_m), ("MLE", mu_mle_m, sigma_mle_m)]:
    theoretical = stats.lognorm.ppf(prob, s=sd_fit, scale=np.exp(mu_fit))
    axes[0].plot(theoretical, observed, '.', ms=2, label=label)
    axes[1].plot(observed, stats.lognorm.cdf(observed, s=sd_fit, scale=np.exp(mu_fit)), label=label)
    print(f"{label}: ошибка implied mean к mean смеси = {np.exp(mu_fit+sd_fit**2/2)-mix.mean():+.1f} ₽; "
          f"ошибка implied median = {np.exp(mu_fit)-np.median(mix):+.1f} ₽")
axes[0].plot(observed, observed, 'k--', lw=1)
axes[0].set(xscale="log", yscale="log", xlabel="квантили fitted-модели", ylabel="квантили смеси", title="QQ: одна логнормала для смеси")
axes[1].plot(observed, prob, 'k--', label="ECDF смеси")
axes[1].set(xscale="log", xlabel="чек, ₽", ylabel="CDF", title="Тело и хвост: разные цели подгонки")
for ax in axes: ax.legend()
fig.tight_layout(); fig.savefig(IMAGE_DIR / "practice_1_2_mixture_fit.png"); plt.close(fig)
...
fig.tight_layout()
fig.savefig(IMAGE_DIR / "practice_1_2_ecdf.png", bbox_inches="tight")
plt.close(fig)
...
fig.tight_layout()
fig.savefig(IMAGE_DIR / "practice_1_2_qq.png", bbox_inches="tight")
plt.close(fig)

r_raw = stats.probplot(check, dist="norm")[1][2]
r_log = stats.probplot(np.log(check), dist="norm")[1][2]
print("=== QQ-plot чека ===")
print(f"коэффициент корреляции точек QQ (сырой чек):  R² = {r_raw**2:.3f}")
print(f"коэффициент корреляции точек QQ (ln чек):      R² = {r_log**2:.3f}")
...
# - MoM и MLE для Пуассона совпадают (λ̂ = x̄); для правильно заданной логнормалы сравниваем оценки; смесь требует новой модели.
...
print(f"ECDF доставки: медиана {med:.0f} мин, P90 {p90:.0f} мин, "
      f"≤40 мин: {f40 * 100:.0f}%")
print(f"QQ-plot R²: сырой чек {r_raw**2:.3f} vs ln(чек) {r_log**2:.3f}")
