...
**Цель урока:** построить иерархическую модель, проверить posterior predictive fit, чувствительность к prior и веса целевой популяции.

**Перед уроком:** [проверьте зависимости темы](../../../course/PREREQUISITES.md); обозначения — в [словаре](../../../course/GLOSSARY.md).

## Зачем это аналитику

В данных 12 городов с разным трафиком. У Москвы 52 000 показов, у Норильска 40 и ни одного заказа. Raw 0% не доказывает нулевую истинную конверсию: Wilson CI примерно [0%;8,8%]. Но он также плохо согласуется с типичной конверсией других городов около 30%. Нельзя автоматически объявлять такой исход случайным шумом и поднимать его к общей средней.

Partial pooling помогает оценивать малые группы, когда они достаточно сопоставимы. В этом уроке базовая иерархия намеренно проверяется на трудном наблюдении: posterior predictive check и изменение prior показывают, что хорошая сходимость MCMC ещё не означает хорошую модель. Для A/B отдельно определим пользовательский эффект с заданными весами и гиперсреднее эффектов городов; общий тест и семейство сегментных тестов отвечают на разные вопросы.

## Теория

### 1. Два крайних подхода: no pooling и complete pooling

Формализуем два предельных подхода для конверсий по группам (города, сегменты, рестораны):
...
| Ограничение | Высокая дисперсия в малых группах; крайние оценки требуют интервала и проверки данных | Игнорирует гетерогенность: общая оценка может плохо описывать отдельный город |
...
Классическая теорема James–Stein относится к оценке 3+ **нормальных средних** при заданной структуре дисперсии и суммарной квадратичной функции потерь. Определённый shrinkage estimator улучшает risk относительно вектора raw means. Она не гарантирует выигрыш любых весов, любой beta-binomial иерархии или каждого отдельного города. В нашей задаче MSE и coverage проверяются для выбранной модели.

![Partial pooling: маленькие города тянутся к среднему, больши](images/v68_shrinkage.png)
...
Второй механизм — $\kappa$ **оценивается из данных всех городов вместе**. Если конверсии городов похожи (маленький разброс), $\kappa$ окажется большой → сжатие сильное; если города сильно разные, $\kappa$ маленькая → каждый почти сам по себе. Данные и prior совместно задают силу заимствования, — ровно то, что empirical Bayes из 5.1 делал «вручную» (подгонкой Beta к историческим монетам), только теперь $\mu$, $\kappa$ и все $p_c$ оцениваются одновременно, и неопределённость по $\kappa$ честно разъезжает по интервалам городов (HDI Норильска шире, чем у Москвы, — и это правильно).

![κ измеряется в юзерах: вес города w=n/(n+κ) — сколько «стран](images/practice_5_5_shrinkage.png)
...
1. **Уточнить глобальный estimand.** δ — среднее hyperdistribution эффектов городов, а не автоматически средний эффект пользователя. Для бизнес-популяции нужны заранее выбранные весаw_c и posterior Δ_user=Σw_c·d_c. Один общий frequentist contrast не требует поправки за 12 сегментных гипотез; его и сравниваем с общим Bayesian contrast.
...
![12 гипотез с поправкой против одной иерархической: на одних ](images/practice_5_5_ab_segments.png)
*Рис. 3. 12 гипотез с поправкой против одной иерархической: на одних данных — разные вопросы: сегментные гипотезы и общий эффект (из практики).*
...
**Метрополис–Гастингс в пять предложений.** (1) Стоим в точке $\theta$ пространства параметров. (2) Предлагаем шаг в случайную сторону: $\theta' = \theta + \text{шум}$. (3) Считаем, во сколько раз в новой точке выше «апостериорная плотность» (правдоподобие × приор): $r = P(\theta' \mid D)/P(\theta \mid D)$. (4) Если $r \geq 1$ — апостериорная плотность выросла — идём в $\theta'$; если $r < 1$ — идём с вероятностью $r$ (монетка), иначе остаёмся. (5) Повторяем тысячи раз: след блуждания — выборка из апостериора; доля времени в области = вероятностная масса области. Вся соль — в шаге (4): иногда мы принимаем и точки с меньшей posterior density, иначе застряли бы в первом же локальном горбе; а отвергнутые шаги оставляют след на месте — отсюда «плато» на trace-plot.

**Гиббс** — частный случай MH: если для каждого параметра его условное распределение при остальных известно в замкнутой форме (сопряжённые кусочки — как в бета-биномиальных иерархиях), сэмплируем из него напрямую, и каждый шаг принимается автоматически ($\alpha = 1$). Так работали классические движки (BUGS, JAGS; в book_about_bayes — Гиббс и Метрополис–Гастингс разобраны подряд). Современный PyMC по умолчанию крутит **NUTS** — тот же принцип «блуждания с приёмом», но шаг и длина шага подстраиваются автоматически под форму апостериора: адаптивный Hamiltonian Monte Carlo с градиентами и выбором длины траектории.

**Диагностика: как понять, что цепи можно верить.**

- **Trace plot** (след параметра по итерациям): здоровая цепь — «хвост гусеницы» (fuzzy caterpillar): плотный шум вокруг уровня без трендов и застреваний. Больная — ползёт (не сошлась), застревает плато (высокая доля отклонений или плохое смешивание), мечется змейкой (шаг слишком крупный).
...
![MCMC не считает распределение — он его гуляет; гистограмма с](images/v69_mcmc_trace.png)
...
Из практики [practice_5_5.py](practice/practice_5_5.py) (12 городов, seed зафиксирован; таблица — конверсия показа в первый заказ за месяц).

**Данные требуют диагностики модели.** Норильск 0/40 несовместим с простым объяснением «обычный шум вокруг 30%»: при p=.3 вероятность такого исхода≈6,37·10⁻⁷. Это может быть structural zero, другой режим или ошибка логирования. Прежде решения проверяем данные, posterior predictive распределение и чувствительность к силе pooling. Усадка не отменяет подозрительный сигнал.

**Иерархическая модель (PyMC 5.28.5, ArviZ 0.23.4):** две цепи по 2000 warmup + 2000 posterior draws. В проверенном запуске κ≈419 (94% HDI [51;800]), R-hat max=1,0045, ESS bulk min=1512, divergences=0. Точные MCMC-округления могут слегка меняться с версиями; таблица выводится практикой.

**Три оценки рядом** ([practice_5_5_shrinkage.png](images/practice_5_5_shrinkage.png)):

| город | n | raw | с усадкой | вес своего $w$ |
|---|---|---|---|---|
| Москва | 52 000 | 30,4% | 30,4% | 0,99 |
| Ростов-на-Дону | 900 | 28,7% | 29,2% | 0,69 |
| Сыктывкар | 80 | **47,5%** | 33,7% (HDI 29,0–39,4) | 0,16 |
| Норильск | 40 | **0,0%** | **26,8%** (HDI 21,3–32,6) | 0,09 |

При данном prior Норильск подтягивается к 26,8%, но PPC даёт **0 из 4000** реплик с нулём заказов из 40: модель плохо воспроизводит этот город. Это Monte Carlo результат, не математически нулевая вероятность. При замене prior κ~Gamma(2,0.01), mean=200, на Gamma(2,0.1), mean=20, posterior Норильска падает до **12,3%**. У sensitivity-модели R-hat≤1,010, ESS≥3449, divergences=0. Такая зависимость и predictive mismatch требуют проверки логирования/обменности и более подходящей модели до решения.

**Иерархия в АБ против «нарезки + Холм»** ([practice_5_5_ab_segments.png](images/practice_5_5_ab_segments.png)). Сценарий: тест промо-баннера, истинное гиперсреднее эффектов +1,0 п.п. (по городам разброс $\sigma = 0{,}6$ п.п.), те же 12 городов, сплит 50/50.

- **Сегментные гипотезы:** Holm проверяет, какие конкретные города можно объявить имеющими ненулевой эффект при контроле FWER. Ноль отклонений не означает «нечего раскатывать» и не отвечает на вопрос об общем эффекте. Общий pooled test здесь p≈.0010; это обязательный benchmark, а не второстепенная справка.
- **Иерархическая модель:** гиперсреднее городов δ≈+0,84 п.п., P(δ>0)≈98,7%. Пользовательский эффект с фиксированными весами n_c/Σn_c — **+0,85 п.п., 94% HDI [+0,37;+1,30]**, при истине этого состава +0,90 п.п. Именно weighted_effect сопоставляем с общим частотным тестом; δ отвечает о гиперсреднем городов.
- Отдельные городские выводы и общий эффект — разные задачи. Иерархия добавляет частичное заимствование и оценку гетерогенности; неуспех Holm не означает неуспех общего частотного теста.

## Подводные камни

1. **Автоматически исправлять редкий город сжатием.** 0/40 даёт широкий интервал, но плохо согласуется с типичными 30%. Показывайте raw данные, интервал, PPC и prior sensitivity; при нарушении обменности общий pooling может скрыть важный сигнал.
...
3. **Верить MCMC без диагностики.** Проверяйте R-hat, bulk/tail ESS, divergences и trace. Повышение target_accept и нецентрированная параметризация могут помочь, но не заменяют пересмотр модели и геометрии posterior. Отсутствие divergences не доказывает пригодность likelihood — для этого нужны predictive checks.
4. **Слепое сжатие при слабых данных о гиперпараметрах.** Делают: 3–4 сегмента и «модель сама оценит $\kappa$/$\tau$». Грозит: гиперпараметр почти не идентифицируется малым числом групп — силу сжатия молча определяет приор; реальный выброс сегмента задавлен к среднему (смещение), а HDI это не покажет. Правильно: sensitivity по приорам (5.1), смотреть HDI $\kappa$/$\tau$ (у нас $\kappa$: 51–800 — «сжатие есть, но его силу мы знаем грубо»), собирать больше групп.
...
2. **Иерархическая модель в PyMC**: `p_city ~ Beta(mu*kappa, (1-mu)*kappa)`, NUTS 2×2000; диагностика ArviZ: R-hat, ESS, HDI; trace plot ([practice_5_5_trace.png](images/practice_5_5_trace.png)).
3. **Три оценки рядом**: raw / pooled / с усадкой по городам + сверка с аналитической формулой $w\cdot raw + (1-w)\cdot\mu$; shrinkage-график: стрелки raw → с усадкой, цвет = размер города, справа кривая $w = n/(n+\kappa)$ с городами на ней ([practice_5_5_shrinkage.png](images/practice_5_5_shrinkage.png)).
4. **АБ-сценарий «нарезка + Холм»** (урок 4.4): тот же набор городов, истинное гиперсреднее эффектов +1,0 п.п.; 12 z-тестов, сырые «прокрасы», поправка Холма, winner's curse сырого максимума.
5. **Та же задача иерархической моделью** (нецентрированная параметризация — и почему): глобальный $\delta$ с $P(\delta>0)$, сжатые городские эффекты против истины, средняя ошибка raw vs с усадкой; сводный график CI против сжатых оценок с истинными эффектами ([practice_5_5_ab_segments.png](images/practice_5_5_ab_segments.png)).

Что должны увидеть (числа реального запуска):

- Модель конверсий: κ≈419, R-hat≤1,0045, ESS≥1512, divergences=0. PPC и sensitivity выявляют проблему обменности Норильска, несмотря на приемлемую диагностику цепей.
...
- формула сжатия воспроизводит MCMC приближённо (расхождение до 0,8 п.п.); MCMC здесь интегрирует неопределённость гиперпараметров, которую plug-in формула не учитывает;
- АБ: сырых значимых городов 2/12, после Holm 0; общий pooled p≈0,0010. Posterior пользовательского weighted-effect +0,85 п.п. [0,37;1,30]; средняя ошибка городских оценок уменьшилась с 1,37 до 0,41 п.п. в этой реализации, что не является универсальной гарантией.

## Домашнее задание

1. Норильск: $k = 0$, $n = 40$; глобальные $\mu = 0{,}30$, $\kappa = 400$. Посчитайте с усадкой-оценку и вес собственного опыта города. Что станет с оценкой, когда за месяц придут ещё 360 показов и снова 0 заказов?
...
2. Город с $n = 52\,000$ и raw 30,4% при тех же $\mu, \kappa$: посчитайте $w$ и с усадкой-оценку. Почему для больших городов иерархия «ничего не меняет» — и зачем тогда она нужна?
...
Иерархия помогает оценивать noisy segment effects и разбросτ. Для глобального вопроса сравниваем **одинаковый** estimand с одним общим frequentist contrast, без штрафа за 12 несвязанных гипотез. Для сегментных выводов отдельно оцениваем риск/покрытие и multiplicity; shrinkage не означает автоматического FWER control. Заранее определить сегменты, веса популяции, priors и модельные проверки.
</details>
...
1. No pooling имеет большую дисперсию в малых группах; complete pooling может скрыть heterogeneity; partial pooling зависит от допущения exchangeability и priors. Теорема James–Stein не гарантирует преимущество любой иерархии.
...
6. Метрополис–Гастингс: случайный шаг + принять с вероятностью $\min(1, r)$, $r$ — отношение апостериорных плотностей для симметричного proposal; в общем MH нужна также обратная/прямая proposal-density ratio; отвержения оставляют на месте. Гиббс — частный случай с точными условными ($\alpha=1$); NUTS — адаптивный Hamiltonian Monte Carlo (PyMC по умолчанию).
...
9. Иерархия оценивает d_c и их распределение(δ,τ); user-weighted effect=Σw_c d_c. Глобальный и сегментные вопросы требуют разных сравнений. Shrinkage снижает шум при подходящей модели, но не автоматически контролируетFWER.
