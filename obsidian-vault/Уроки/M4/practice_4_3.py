...
Запуск:  python3 practice_4_3.py
...
fig.tight_layout()
fig.savefig(HERE / "practice_4_3_simpson.png", dpi=150)
...
fig.tight_layout()
fig.savefig(HERE / "practice_4_3_spillover.png", dpi=150)
...
fig.tight_layout()
fig.savefig(HERE / "practice_4_3_novelty.png", dpi=150)
...
def validity_checklist(cfg):
    ...
    add("Популяция анализа: ITT или симметричный triggered-фильтр", "OK" if trig == "itt" else ("ПРОВЕРИТЬ" if trig == "triggered_even" else "СТОП"),
        "анализируем всех рандомизированных (ITT)" if trig == "itt" else
        "triggered по правилу, симметричному в обеих группах (Kohavi гл.20)" if trig == "triggered_even" else
        "фильтр «видел фичу» асимметричен -> survivorship/Симпсон: вернуть ITT")
    add("Телеметрия: инвариантные метрики и потери событий по вариантам", "OK" if cfg["telemetry_ok"] else "СТОП",
        "инварианты (сессии, показы) не разошлись" if cfg["telemetry_ok"] else
        "потери событий различаются по вариантам: тест невалиден, чинить логирование")
    add("Guardrails-инварианты живы (краши, время ответа)", "OK" if cfg["guardrails_alive"] else "СТОП",
