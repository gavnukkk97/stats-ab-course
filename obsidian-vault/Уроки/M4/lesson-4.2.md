...
**Цель урока:** проверить назначение пользователей и SRM; отличать калибровку размеров групп от A/A-проверки метрик.
...
![Хэш-рандомизация: равномерность и лавинный эффект — смена со](images/practice_4_2_split.png)
*Рис. 1. Хэш-рандомизация: равномерность и лавинный эффект — смена соли полностью пересобирает сплит (из практики).*
...
![Слои = независимые лотереи: у каждого слоя своя соль, юзер у](images/v48_split_layers.png)
*Рис. 2. Слои = независимые лотереи: у каждого слоя своя соль, юзер участвует в тестах разных слоев независимо.*
...
**Мощность детектора.** Перекос $\delta$ (разность долей) даёт $z = \delta\sqrt{N}$, и детектор — обычный тест со своей мощностью: при пороге 0,001 ($z_{crit}=3{,}29$) перекос 1% ловится с мощностью 80% только от $N \approx 171\,000$ юзеров суммарно. Отсюда два следствия: рамп-стадии 1–5% слепы к тонким багам (не «тест честный», а «выборки мало»); маленькие тесты — слепая зона платформы (наш «фильтр новинок» −2% теста ловится в 16% тестов при $N=50$k, в 90% при 200k, почти во всех симуляциях при 1M).

![Мощность SRM-детектора: перекос 1% ловится от ~170к юзеров (](images/practice_4_2_srm_power.png)
*Рис. 3. Мощность SRM-детектора: перекос 1% ловится от ~170к юзеров (из практики).*
...
![A/A — ЭКГ платформы: здоровая система даёт равномерные p; сл](images/v50_aa_health.png)
*Рис. 4. Симуляция p-value под H0 и смеси с заданным эффектом. Правая панель не является A/A или аудитом платформы; полный учебный пайплайн проверяется в практике.*
...
4. **«A1 не отличается от A2 — сплит ок».** Делают: А/А/В в каждом тесте как ритуал надёжности. Грозит: ×1,5 трафика, FWER около 12% (нормальная модель равных групп; сравнения зависимы), и всё равно не аудит — мощности на вывод нет. Правильно: SRM-чек на каждом тесте + платформенные A/A; второй контроль — только в самом A/A.
...
Файл [practice_4_2.py](practice/practice_4_2.py) (percent-формат, numpy/scipy/matplotlib/pandas + hashlib, seed зафиксирован). Три блока:

1. **Сплит на хэше**: `get_bucket(unit_id, salt) = int(md5(salt|id)) % 10 000` на 20 000 юзеров. Проверки: равномерность 100 макро-бакетов ($\chi^2=105{,}5$, p=0,31); лавинный эффект (64,0 из 128 бит, sd 5,6); независимость слоёв «ranking»/«frontend» — таблица 10×10, $\chi^2=93{,}1$, p=0,17; пересечение слотов 222 против ожидаемых $n/(m_1 m_2)=200$; SRM-чек внутри слота (A=5 062, B=5 112, p=0,62). График [practice_4_2_split.png](images/practice_4_2_split.png).
2. **SRM-детектор**: `srm_check()` (хи-квадрат долей) на пяти классических примерах чисел — таблица из §4 (49995/50005 → 0,97; 49800/50200 → 0,21; 49500/50500 → 0,0016; 495000/505000 → $1{,}5\cdot10^{-23}$; 252000/248000 → $1{,}5\cdot10^{-8}$); кривые мощности от перекоса для N = 10k/100k/1M при пороге 0,001 (1% перекос: 45% мощности на 100k, ~100% на 1M; расчёт N для 80% мощности — 170 746). График [practice_4_2_srm_power.png](images/practice_4_2_srm_power.png).
3. **A/A-аудит платформы**: 500 симулированных A/A по 200 000 юзеров. Здоровая система: KS p=0,08, p<0,05 в 5,4%, алертов 0,001-порога — 1 из 500 (0,2%). Сломанная (введён баг «фильтр новинок»: −2% тест-группы): 88,8% алертов; при N = 50k — только 16,0%, при 1M — 100%. График [practice_4_2_aa_hist.png](images/practice_4_2_aa_hist.png).
...
### Что именно проверяет практика

Первый блок A/A в практике 4.2 калибрует **только SRM** на случайных размерах групп. Он не тестирует расчёт выручки или ratio. Дополнительный блок `aa_metric_pipeline` создаёт назначения и события, агрегирует их до пользователя и проверяет ARPU, ratio и CUPED в 400 мирах H0. Частоты отвержений сопровождаются биномиальными Monte Carlo CI. Для дискретных тестов p-values могут быть консервативными; требование к валидному p — super-uniform, а не обязательная идеальная равномерность.

![Калибровка метрик через учебный A/A-пайплайн](images/practice_4_2_metric_calibration.png)

Даже этот учебный блок не проверяет реальные логи компании. В производственной системе A/A проводят через её собственный путь от назначения до отчёта и отдельно проверяют покрытие CI, сегменты, ratio и выбранные корректировки.

Для независимых проверок можно использовать 1−(1−α)^m. У попарных A1/A2/B общие группы, поэтому точный FWER этой формулой не определяется; применяйте выбранную процедуру контроля или симуляцию именно своего дизайна.
