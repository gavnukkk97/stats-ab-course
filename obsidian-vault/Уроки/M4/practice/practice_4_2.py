...
Запуск из корня репозитория: python3 course/modules/M4/practice/practice_4_2.py
...
fig.tight_layout()
fig.savefig(IMAGE_DIR / "practice_4_2_split.png", dpi=150)
plt.close(fig)
...
ax.grid(alpha=0.3)
fig.tight_layout()
fig.savefig(IMAGE_DIR / "practice_4_2_srm_power.png", dpi=150)
plt.close(fig)
...
print("=" * 78)
print(f"3) A/A: КАЛИБРОВКА SRM-ДЕТЕКТОРА: {N_TESTS} тестов по N={N_BROKEN:,} юзеров")
print("-" * 78)
...
axes[0].set_title(f"SRM при исправном назначении: p-value равномерны\nKS p = {ks_h.pvalue:.2f}, "
...
fig.tight_layout()
fig.savefig(IMAGE_DIR / "practice_4_2_aa_hist.png", dpi=150)
plt.close(fig)
...
# %% [markdown]
# ## Дополнение: A/A через данные и метрики, а не только размеры групп
# В каждом мире новые пользователи, pre-outcome, assignment и события.
# Два события пользователя агрегируем обратно; единица анализа — пользователь.
def aa_metric_pipeline(worlds=400, users=2000, seed=4204):
    rng = np.random.default_rng(seed)
    pvalues = {key: [] for key in ("SRM", "ARPU", "ratio", "CUPED")}
    for _ in range(worlds):
        pre = rng.lognormal(0, .6, users)
        arm = rng.binomial(1,.5,users).astype(bool)
        revenue = 10*pre + rng.normal(0,4,users)
        events = np.repeat(revenue/2, 2).reshape(users,2)
        y = events.sum(axis=1)
        count = rng.poisson(2,users)+1
        pvalues["SRM"].append(srm_check([np.sum(~arm),np.sum(arm)])[1])
        pvalues["ARPU"].append(stats.ttest_ind(y[arm],y[~arm],equal_var=False).pvalue)
        variances, ratios = [], []
        for mask in (~arm,arm):
            yy,cc=y[mask],count[mask]
            r=yy.sum()/cc.sum();ratios.append(r)
            variances.append(np.var(yy-r*cc,ddof=1)/(len(yy)*cc.mean()**2))
        pvalues["ratio"].append(2*stats.norm.sf(abs(ratios[1]-ratios[0])/np.sqrt(sum(variances))))
        # Коэффициент 10 задан независимо генератором, без fit на тестовых исходах.
        adjusted=y-10*(pre-pre.mean())
        pvalues["CUPED"].append(stats.ttest_ind(adjusted[arm],adjusted[~arm],equal_var=False).pvalue)
    return {key: np.asarray(value) for key,value in pvalues.items()}

aa_metrics = aa_metric_pipeline()
print("\nA/A метрик: частота ложных отвержений и 95% Monte Carlo CI")
fig, axs=plt.subplots(1,4,figsize=(14,3.5))
for ax,(name,values) in zip(axs,aa_metrics.items()):
    hits=int(np.sum(values<.05)); size=len(values)
    interval=stats.binomtest(hits,size).proportion_ci()
    print(f"{name}: {hits/size:.1%} [{interval.low:.1%};{interval.high:.1%}]")
    ax.hist(values,bins=15);ax.set_title(name);ax.set_xlabel("p-value под H0")
fig.suptitle("Учебный A/A-пайплайн: результат калибровки имеет Monte Carlo погрешность")
fig.tight_layout();fig.savefig(IMAGE_DIR / "practice_4_2_metric_calibration.png",dpi=150);plt.close(fig)
