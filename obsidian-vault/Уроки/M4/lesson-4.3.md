...
**Цель урока:** проверять валидность теста **до** чтения p-value. Различать внутреннюю валидность (измеряет ли тест причинный эффект здесь и сейчас: SUTVA/интерференция, survivorship, ITT, телеметрия) и внешнюю (обобщается ли результат на полную раскатку и другие недели: новизна, обучение, ramp-up). Знать механику интерференции на двухстороннем рынке: спилловор-доля γ — поюзерный сплит видит лишь $(1-\gamma)\cdot\delta$ истинного эффекта (наша симуляция: при $\gamma = 0{,}9$ — десятую часть, мощность падает до 22%, при том что гео-дизайн видит всё) — и три лекарства: кластерная/гео-рандомизация и switchback. Понимать пересечение тестов (важна величина, а не факт; 2×2-проверка взаимодействия), эффект новизны/обучения (одна неделя может показать +3,0% или −0,5% при стационарных +1,0%), cookie churn против account-id, парадокс Симпсона (в обоих сегментах хуже на ~1 п.п., в агрегате «лучше» на +1,0 п.п., $p=3\cdot10^{-7}$) и различающиеся потери телеметрии. Собрать всё в чек-лист валидности — гейт перед анализом.
...
![Спилловер на двухстороннем рынке: поюзерный сплит видит лишь](practice_4_3_spillover.png)
*Рис. 4. Спилловер на двухстороннем рынке: поюзерный сплит видит лишь (1−γ)·δ эффекта, A/A этого не замечает (из практики).*
...
![Три ответа на интерференцию: кластерная/гео-рандомизация и s](visuals/v51_interference_designs.png)
*Рис. 3. Три ответа на интерференцию: кластерная/гео-рандомизация и switchback — запирают взаимодействие внутри кластера.*
...
![Новизна и обучение: траектории эффекта по неделям — окно тес](practice_4_3_novelty.png)
*Рис. 2. Новизна и обучение: траектории эффекта по неделям — окно теста решает, что вы увидите (из практики).*
...
**Triggered-анализ** (Kohavi гл.20) — легальное сужение: из всех рандомизированных берём тех, кто реально мог увидеть фичу (открывал корзину). Выигрыш — чувствительность: эффект не разбавляется нулевыми юзерами (trigger dilution). Условия честности: (а) триггер-событие логируется в момент экспозиции; (б) правило **симметрично** в обеих группах (в контроле фичи нет, но триггер тот же — «открыл корзину», а не «кликнул на рекомендации», которых в контроле не существует). Асимметричный фильтр = selection bias, и дальше два сценария: тихое смещение или открытый парадокс Симпсона — как в нашей практике ниже.
...
![Парадокс Симпсона: агрегат выигрывает, оба сегмента проигрыв](visuals/v53_simpson.png)
*Рис. 1. Парадокс Симпсона: агрегат выигрывает, оба сегмента проигрывают — микс сегментов различается.*
...
Проверки: **инвариантные метрики** — число сессий, показов, крашей, время ответа — не должны различаться между вариантами (различие = телеметрия/производительность сломаны раньше, чем мы что-то измерили); SRM на уровне отдельных событий (событий «show_cart» в группах поровну?); доля потерь/дозаписей по вариантам; свежий A/A после каждого релиза клиента (4.2).
...
8. Популяция анализа: ITT или симметричный triggered-фильтр; баланс состава по пре-сегментам.
9. Телеметрия: потери событий по вариантам одинаковы; MDM-чеки пройдены.
10. Guardrails-инварианты живы (краши, latency).
...
8. **Телеметрию проверяют «на глаз».** Делают: после релиза клиента не перезапускают A/A и не смотрят инварианты. Грозит: дифференциальные потери событий → систематически смещённая метрика, тест невалиден при любых p-value. Правильно: инвариантные метрики и SRM по событиям — в автоматическом чек-листе каждого теста.

## Практика

Файл: `practice_4_3.py` (percent-формат, numpy/scipy/matplotlib/pandas, seed зафиксирован). Что делает:

1. **Симпсон-симуляция**: мобильные/web конверсии; внутри сегментов тест хуже (−1,16 и −1,14 п.п., оба значимо), в агрегате «лучше» (+0,96 п.п., $z=5{,}1$); сегментный SRM $\chi^2 = 13\,903$. График `practice_4_3_simpson.png`.
2. **Спилловор на рынке**: 400 миров для $\gamma \in \{0; 0{,}3; 0{,}6; 0{,}9\}$: поюзерный сплит видит $(1-\gamma)\cdot\delta$ (−2,79; −1,61; −0,39 мин при δ=−4), при γ=0,9 мощность 22%; гео-дизайн видит −4,0 мин всегда; A/A при выключенной фиче — 4,8% ложных срабатываний (интерференцию не ловит). График `practice_4_3_spillover.png`.
3. **Новизна/обучение**: недельные траектории и кумулятив «что видит тест W недель»: новизна 1 нед +3,0% → 6 нед +1,44% (стационар +1,0%); обучение 1 нед −0,5% (хороним фичу) → 6 нед +0,67%. График `practice_4_3_novelty.png`.
...
11. Телеметрия: дифференциальные потери событий инвалидируют тест; инварианты (сессии, показы, краши) + SRM по событиям + A/A после каждого релиза клиента.
