...
Запуск:  python3 practice_4_1.py
...
# %%
def decision_rule(oec, guardrails):
    """Decision rule 2x2 (знак эффекта × значимость) + политика «нет решения»."""
    g0 = guardrails[0]
    return [
        "DECISION RULE (пишется ДО запуска):",
        f"  [+] {oec} значимо вырос (p<{ALPHA:.2f}) и guardrails целы -> SHIP: 50% -> 100%",
        f"  [+] {oec} значимо вырос, но просел guardrail ({g0}) -> НЕ катим: разбор trade-off",
        f"  [-] {oec} значимо упал -> ROLLBACK (откат)",
        f"  [ ] {oec} незначим (CI эффекта включает 0) -> НЕТ РЕШЕНИЯ:",
        "        продлевать «до прокраса» нельзя (peeking, урок 3.7); один заход",
        "        «углубить»: сегменты и вторичные метрики с поправкой на",
        "        множественность (урок 2.4); затем закрыть и записать в архив",
        "        тестов — институциональная память (Kohavi, гл. 8).",
    ]
...
def build_design_doc(number, name, hyp, oec, oec_p0, mde, guardrails,
    ...
    t1 = t0 + pd.Timedelta(days=7 * weeks - 1)
    ...
    lines = [
        ...
        "  Invariants (не должны измениться вообще): SRM-чек p>0.001 (урок 4.2),",
        "      доля юзеров без единого события, скорость загрузки логов",
        "",
        "ЮНИТ рандомизации = юнит анализа = user_id (урок 3.2)",
        "",
        f"СТАТИСТИКА: alpha={alpha:.0%}, мощность={power:.0%}, "
        f"MDE=+{mde * 100:.1f} п.п. (+{mde / oec_p0:.1%} отн.)",
        f"  sigma^2 = p0(1-p0) = {oec_p0 * (1 - oec_p0):.4f}",
        f"  n = 2*({Z_A:.2f}+{Z_B:.2f})^2*sigma^2/mde^2 = {n_raw:,.0f}"
        f" -> с запасом 5%: {n:,} юзеров/группу",
        "",
        f"ДЛИТЕЛЬНОСТЬ: {weeks} полные недели: {t0.date()} ({t0.day_name()[:3]})"
        f" -> {t1.date()}, набираем ~{weekly * weeks:,} юзеров",
        "КРИТЕРИЙ ОСТАНОВКИ: fixed horizon, смотрим в конце; в процессе — только",
        "  invariants и guardrails (ранняя остановка по вреду); последовательные",
        "  схемы — только заранее выбранные (урок 3.7)",
        "",
        *decision_rule(oec, guardrails),
        "",
    ...
    meta = dict(n=n, weeks=weeks, t0=t0, t1=t1, sigma=sigma)
    return "\n".join(lines), meta
...
doc1, m1 = build_design_doc(
    ...
    hyp={"x": "юзеры не находят самые быстрые рестораны (медиана поиска 4.2 мин, "
         ...
         "z": "больше заказов -> больше выручки (заказы — драйвер выручки, не прокси)"},
...
doc2, m2 = build_design_doc(
    ...
    oec="доля открытий пуша", oec_p0=0.062, mde=0.01,
    guardrails=["отписки от пушей (+<5% отн.)", "деинсталлы (+<0.2 п.п.)",
                "crash-free rate (+<-0.1 п.п.)"],
...
SE_TEST = 0.02 / (Z_A + Z_B)  # шум теста: истинный эффект +2% ловится с ~80%
...
for _ in range(WORLDS):
    ...
    res[STRATS[0]] = (true_eff[o0].copy(), true_eff[o0])
    # S1: тестируем 12 случайных идей, катим «значимые плюсы»
    o1 = RNG.permutation(N_IDEAS)[:CAPACITY]
    res[STRATS[1]] = (np.where(ship[o1], true_eff[o1], 0.0), true_eff[o1])
    # S2: тестируем топ-12 по приоритету
    o2 = np.argsort(-prior)[:CAPACITY]
    res[STRATS[2]] = (np.where(ship[o2], true_eff[o2], 0.0), true_eff[o2])

    for s, (value, tested_true) in res.items():
        cum = np.cumsum(value)
        cum_mean[s][: len(cum)] += cum / WORLDS
        finals[s].append(cum[-1])
        false_ship[s].append(int(np.sum((value > 0) & (tested_true <= 0))))
        ship_share[s].append(int(np.sum(value != 0)))
...
ax.grid(alpha=0.3)
fig.tight_layout()
fig.savefig(HERE / "practice_4_1_third_wins.png", dpi=150)
plt.close(fig)
...
for label, arr, typical in (
    ...
    rows.append({
        ...
        "ловит": "да" if mde_rel < typical else "НЕТ",
...
print("   «ловит» — MDE против типичного эффекта таких фич из истории тестов:")
print("   UI-механики двигают CTR/CR на ~10%, выручку — на ~3% (эффект размывается")
print("   по чеку); MDE > типичного эффекта = тест «не увидит» рабочую фичу.")
mde_cr_design = (Z_A + Z_B) * cr2.std(ddof=1) / cr2.mean() * math.sqrt(2 / 12_978)
print(f"   при n дизайн-дока (12 978) MDE CR = {mde_cr_design:.1%} < 10% — дизайн ловит")
print(f"   p0 в дизайн-доке 23%, в синтетике вышло {cr2.mean():.1%}: p0 по пре-периоду")
print("   плывёт — а sigma^2 входит в n квадратично (урок 3.1), перепроверяйте.")
...
ax2.grid(alpha=0.3)
fig.tight_layout()
fig.savefig(HERE / "practice_4_1_oec_map.png", dpi=150)
plt.close(fig)
...
print("   эффектах 3% — тест «не увидит» фичу даже с винзоризацией и CUPED;")
print("   CR — драйвер: ловит свои типичные эффекты; выручку — в guardrails.")
