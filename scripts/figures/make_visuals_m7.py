...
OUTPUT_DIR = ROOT / "course" / "modules" / "M7" / "images"
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

# %% [markdown]
# # Визуализации для М7 «Проектирование АБ-платформы»
# Единый стиль: заголовок = вывод. Запуск: python3 scripts/figures/make_visuals_m7.py
import matplotlib
matplotlib.use("Agg")
...
box(ax, 0.3, 0.4, 5.5, 1.4, "Гейты качества: SRM-х², инварианты,\nguardrail авто-стоп: заданный error budget", "#ffe3e3", fs=8.5)
...
fig.savefig(OUTPUT_DIR / "v84_platform_architecture.png", bbox_inches="tight"); plt.close(fig)
...
left = 4.0
for name, w in [("тест A 40%", .4), ("тест B 20%", .2), ("свободно 40%", .4)]:
    box(ax, left, 4.6, 9.6 * w, 1.2, name, "#fafafa", fs=8)
    left += 9.6 * w
ax.text(8.8, 6.1, "Бакеты 0–999: hash(user, S1) % 1000", ha="center", fontsize=8.5, color="#555")
...
ax.set_title("Домены, слои и 1000 бакетов (Google, KDD 2010): слоты внутри слоя не пересекаются",
             fontsize=12, fontweight="bold")
fig.savefig(OUTPUT_DIR / "v85_domains_layers.png", bbox_inches="tight"); plt.close(fig)
...
box(ax, 0.3, 1.0, 9.0, 1.9, "Exposure = фактический контакт; храним время события и загрузки.\nITT начинается с назначения eligible-пользователя, даже если контакта не было.", "#ffe3e3", fs=9)
ax.text(5, 0.4, "Потери и дубли проверяем отдельно: SRM не обнаруживает все нарушения логирования.",
        ha="center", fontsize=9, style="italic", color="#555")
ax.set_title("Assignment и exposure — разные события и разные популяции анализа",
             fontsize=12, fontweight="bold")
fig.savefig(OUTPUT_DIR / "v86_exposure_logging.png", bbox_inches="tight"); plt.close(fig)
...
for nb in [1000, 100]:
    ...
    bucket_times[nb] = time.perf_counter() - t0
fig, ax = plt.subplots(figsize=(8.6, 4.2))
ax.bar([0, 1, 2], [t_full * 1000, bucket_times[1000] * 1000, bucket_times[100] * 1000], color=[C_BAD, C_MAIN, C_TRUE], width=0.55)
ax.set_xticks([0, 1, 2]); ax.set_xticklabels(["поюзерно\n(1 млн строк)", "1000 бакетов", "100 бакетов"])
for i, v in enumerate([t_full * 1000, bucket_times[1000] * 1000, bucket_times[100] * 1000]):
    ax.text(i, v * 1.05, f"{v:.1f} мс", ha="center", fontweight="bold")
ax.set_ylabel("время одного вызова теста, мс")
ax.set_yscale("log")
ax.set_title("Измеренный пример на 1 млн пользователей: стоимость теста.\nПодготовка агрегатов не включена; bucket means дают приближение.")
fig.tight_layout(); fig.savefig(OUTPUT_DIR / "v87_bucketization.png", bbox_inches="tight"); plt.close(fig)
...
ax.text(7, 1.2, "До интерпретации эффекта проверяем качество данных.\nДля регулярных проверок заранее задаём error budget.",
...
ax.set_title("Путь гипотезы через платформу: два красных гейта, которые нельзя обойти",
             fontsize=12, fontweight="bold")
fig.savefig(OUTPUT_DIR / "v88_gates_pipeline.png", bbox_inches="tight"); plt.close(fig)
...
fig.tight_layout(); fig.savefig(OUTPUT_DIR / "v89_calendar.png", bbox_inches="tight"); plt.close(fig)
...
for _ in range(n_sim):
    ...
    cap = 3.0
    # Один и тот же first-fit: различается только порядок очереди.
    fifo = hyp
    used, total = 0, 0
    for v, d in fifo:
        if used + d <= cap:
...
ax.set_xticks([0, 1]); ax.set_xticklabels(["Порядок поступления\n(first-fit)", "Приоритет ценность/длительность\n(first-fit, эвристика)"])
for i, v in enumerate([fifo_total / n_sim, econ_total / n_sim]):
    ax.text(i, v + 0.05, f"{v:.2f}", ha="center", fontweight="bold")
ax.set_ylabel("реализованная ценность за окно (среднее)")
ax.set_title(f"В этой синтетической модели приоритет реализует на ~{(econ_total/fifo_total-1)*100:.0f}% больше ценности\nдлительности и истинная ценность известны; это не гарантия для продукта")
fig.tight_layout(); fig.savefig(OUTPUT_DIR / "v90_slot_economics.png", bbox_inches="tight"); plt.close(fig)
...
ax.set_title("Учебная velocity-воронка: доля и время прохождения этапов")
fig.tight_layout(); fig.savefig(OUTPUT_DIR / "v91_velocity.png", bbox_inches="tight"); plt.close(fig)
...
box(ax, 0.3, 4.6, 5.4, 2.6, "BUILD (своя)\n+ полная интеграция с твоими данными\n+ собственный процесс планирования\n− затраты на разработку и поддержку", "#e8f0fe", fs=9)
box(ax, 6.3, 4.6, 5.4, 2.6, "BUY (Statsig/Eppo/GrowthBook)\n+ готовые интеграции и процедуры\n− проверить статистические условия\n− проверить интеграцию данных\n− проверить нужный календарный процесс", "#fff3cd", fs=9)
box(ax, 0.3, 1.2, 11.4, 2.4, "Сравните требования, интеграцию, поддержку и полный TCO на одном горизонте.\nНаличие функций и цены проверяйте по актуальной документации и предложению.\nГибридная архитектура — один из вариантов, а не универсальный ответ.", "#d8f3dc", fs=9.5)
ax.set_title("Buy vs build: решение по требованиям и полной стоимости владения",
             fontsize=11.5, fontweight="bold")
fig.savefig(OUTPUT_DIR / "v92_buy_build.png", bbox_inches="tight"); plt.close(fig)

print("Готово: v84–v92 (9 визуализаций) в", OUTPUT_DIR)
