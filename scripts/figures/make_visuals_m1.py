...
OUTPUT_DIR = ROOT / "course" / "modules" / "M1" / "images"
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

# %% [markdown]
# # Визуализации для М1 «Базовая статистика»
# Единый стиль: заголовок = вывод, подписи объясняют чтение графика.
# Запуск: python3 scripts/figures/make_visuals_m1.py  (PNG сохраняются в course/modules/M1/images)
import matplotlib
matplotlib.use("Agg")
...
ax.set_title(f"Средний чек «ЕдаДома»: mean / median = {m/med:.2f}")
...
fig.tight_layout(); fig.savefig(OUTPUT_DIR / "v01_mean_vs_median.png", bbox_inches="tight"); plt.close(fig)
...
takeaway(fig, "Один заказ: среднее −52%, медиана −2,3% → медиана устойчива к выбросам, среднее — нет")
fig.tight_layout(rect=(0, 0.05, 1, 1)); fig.savefig(OUTPUT_DIR / "v02_whale_effect.png", bbox_inches="tight"); plt.close(fig)

# ============================================================
# 3. Закон малых чисел: траектории выборочного среднего
# ============================================================
true_mean = np.exp(np.log(900) + 1.1**2 / 2)
...
fig.tight_layout(rect=(0, 0.06, 1, 1)); fig.savefig(OUTPUT_DIR / "v03_law_small_numbers.png", bbox_inches="tight"); plt.close(fig)
...
fig.tight_layout(); fig.savefig(OUTPUT_DIR / "v04_distribution_zoo.png", bbox_inches="tight"); plt.close(fig)
...
for ax, bins, name, verdict in [
    ...
    (axes[2], "scott", f"{len(np.histogram_bin_edges(data, bins='scott'))-1} бинов (Скотт)", "ширина рассчитана по данным"),
...
fig.tight_layout(); fig.savefig(OUTPUT_DIR / "v05_bins_choice.png", bbox_inches="tight"); plt.close(fig)
...
axes[1].text(np.percentile(times, 97), 0.35, "читаем P90:\nот 0,9 на Y → к кривой,\nзатем вертикаль ↓ к X",
             fontsize=9, ha="right", bbox=dict(boxstyle="round,pad=0.3", fc="#fff3cd"))
fig.tight_layout(); fig.savefig(OUTPUT_DIR / "v06_ecdf_reading.png", bbox_inches="tight"); plt.close(fig)
...
fig.tight_layout(); fig.savefig(OUTPUT_DIR / "v07_qq_plot.png", bbox_inches="tight"); plt.close(fig)
...
fig.suptitle("ЦПТ: данные — любые, средние — нормальные (чем больше n, тем ближе)",
             fontsize=12, fontweight="bold")
fig.tight_layout(); fig.savefig(OUTPUT_DIR / "v08_clt_intuition.png", bbox_inches="tight"); plt.close(fig)
...
fig.tight_layout(); fig.savefig(OUTPUT_DIR / "v09_se_sqrt_n.png", bbox_inches="tight"); plt.close(fig)
...
fig.tight_layout(); fig.savefig(OUTPUT_DIR / "v10_ci_coverage.png", bbox_inches="tight"); plt.close(fig)
...
fig.tight_layout(); fig.savefig(OUTPUT_DIR / "v11_bootstrap_schema.png", bbox_inches="tight"); plt.close(fig)
...
fig.tight_layout(); fig.savefig(OUTPUT_DIR / "v12_bootstrap_trap.png", bbox_inches="tight"); plt.close(fig)

print("Готово: 12 визуализаций сохранены в", OUTPUT_DIR)
