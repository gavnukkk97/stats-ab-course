...
# Единый стиль с М1: заголовок = вывод. Запуск: python3 scripts/figures/make_visuals_m2.py
import matplotlib
matplotlib.use("Agg")
...
fig.tight_layout(); fig.savefig(OUTPUT_DIR / "v13_pvalue_h0_h1.png", bbox_inches="tight"); plt.close(fig)

# ============================================================
# 14. Матрица ошибок 2×2
# ============================================================
fig, ax = plt.subplots(figsize=(8.2, 4.8))
ax.set_xlim(-1.7, 10); ax.set_ylim(0, 6); ax.axis("off")
cells = [
    (0.3, 3.2, C_TRUE, "Верно отрицательное", "не отвергли H0,\nэффекта действительно нет", "(1−α)"),
    (5.2, 3.2, C_BAD, "Ошибка II рода (FN)", "не отвергли H0,\nхотя эффект есть", "β = 20%"),
    (0.3, 0.3, C_BAD, "Ошибка I рода (FP)", "отвергли H0,\nхотя эффекта нет", "α = 5%"),
...
ax.text(-0.1, 4.45, "не отвергли\nH0", fontsize=10, ha="right")
ax.text(-0.1, 1.55, "отвергли\nH0", fontsize=10, ha="right")
ax.set_title("Матрица ошибок: α покупаем заранее, β — дизайном (n и эффект)")
fig.savefig(OUTPUT_DIR / "v14_error_matrix.png", bbox_inches="tight"); plt.close(fig)
...
for eff, c in [(30, C_BAD), (60, C_MAIN), (120, C_TRUE)]:
    z_a = 1.96
    ns_arr = np.array(ns)
    pw = normal_power(eff, sd * np.sqrt(2 / ns_arr))
...
ax.legend()
fig.tight_layout(); fig.savefig(OUTPUT_DIR / "v15_power_curves.png", bbox_inches="tight"); plt.close(fig)
...
power_true = normal_power(true_eff, sd * np.sqrt(2 / n))
ax.set_title(f"Type M: при мощности ~{power_true:.0%} значимые оценки в среднем "
             f"имеют средний модуль ×{type_m(effs, true_eff):.1f} от истины")
ax.legend(fontsize=9)
fig.tight_layout(); fig.savefig(OUTPUT_DIR / "v16_type_m.png", bbox_inches="tight"); plt.close(fig)
...
fig.tight_layout(); fig.savefig(OUTPUT_DIR / "v17_welch.png", bbox_inches="tight"); plt.close(fig)
...
for i, (ax, ttl) in enumerate([(axes[0], "Непарный взгляд: два облака"),
    ...
    if i == 0:
        ax.scatter(rng.uniform(-0.18, 0.18, n), before, s=42, color=C_MAIN, alpha=0.8, label="до")
        ax.scatter(rng.uniform(0.82, 1.18, n), after, s=42, color=C_ACC, alpha=0.8, label="после")
        ax.set_xticks([0, 1]); ax.set_xticklabels(["до", "после"])
        ax.legend(); sd_ = after.std()
        ax.set_title(f"{ttl}\nSD «после» = {sd_:.1f} → шум глушит сдвиг", fontsize=10)
    else:
        ax.axhline(0, color="gray", ls="--")
        ax.scatter(np.linspace(-.12, .12, n), change, s=42, color=C_TRUE, alpha=0.85)
        ax.set_xlim(-.5, .5)
        ax.set_xticks([0]); ax.set_xticklabels(["после − до, одна точка на пару"])
        sd_d = change.std()
        ax.set_title(f"{ttl}\nSD разностей = {sd_d:.1f}; причинность требует контроля", fontsize=10)
    ax.set_ylabel("NPS-скор" if i == 0 else "изменение NPS-скора")
fig.suptitle("Парный дизайн: выигрыш при положительной корреляции (задел на CUPED в М3)",
             fontsize=12, fontweight="bold")
fig.tight_layout(); fig.savefig(OUTPUT_DIR / "v18_paired_design.png", bbox_inches="tight"); plt.close(fig)

# ============================================================
# 19. Что на самом деле проверяет Манн–Уитни
# ============================================================
fig, axes = plt.subplots(1, 2, figsize=(9.5, 4.0))
x1 = rng.normal(800, 100, 4000)
x2 = rng.normal(860, 100, 4000)
...
axes[0].set_title("Одинаковая форма, только сдвиг:\nв этой модели MW можно интерпретировать как тест сдвига", fontsize=10)
axes[0].legend()
y1 = rng.lognormal(np.log(800), 0.35, 4000)
y2 = rng.gamma(2.2, 800 / stats.gamma.ppf(.5, 2.2), 4000)
...
axes[1].set_title(f"Разные формы; истинные медианы = 800\nвыборочные {m1:.0f}/{m2:.0f}; MW p={stats.mannwhitneyu(y1,y2).pvalue:.3f}", fontsize=10)
axes[1].legend()
fig.suptitle("Манн–Уитни: H0 — одинаковые распределения; эффект U/(mn) — AUC",
             fontsize=12, fontweight="bold")
fig.tight_layout(); fig.savefig(OUTPUT_DIR / "v19_mann_whitney.png", bbox_inches="tight"); plt.close(fig)
...
obs = b.mean() - a.mean()
perm = permutation_differences(a, b, rng, repeats=20000)
...
p_perm = permutation_pvalue(obs, perm)
...
ax.legend(fontsize=9)
fig.tight_layout(); fig.savefig(OUTPUT_DIR / "v20_permutation.png", bbox_inches="tight"); plt.close(fig)
...
axes[0].set_title("Независимые проверки: риск хотя бы одной ошибки")
...
axes[1].set_xlabel("число метрик с p<0.05 (все ложные!)"); axes[1].set_ylabel("доля миров")
...
fig.tight_layout(); fig.savefig(OUTPUT_DIR / "v21_multiple_testing.png", bbox_inches="tight"); plt.close(fig)
...
ax.scatter(i, p, s=46, color=C_MAIN, zorder=3, label="отсортированные иллюстративные p-value")
...
ax.step(i, holm_thresholds(m_), where="post", color=C_ACC, lw=2.2, ls="--", label="Холм: α/(m−i+1), остановка при первом превышении")
ax.set_xlabel("ранг i (отсортированные p)"); ax.set_ylabel("p-value")
stop = np.flatnonzero(p > holm_thresholds(m_))[0] + 1
ax.axvline(stop, color=C_ACC, alpha=.5, ls=":", label=f"Холм: остановка на ранге {stop}")
ax.set_yscale("log"); ax.set_ylim(.001, 1)
ax.set_title("Поправки на одной картине: Холм — step-down (FWER), BH — step-up (FDR)")
ax.legend(fontsize=8.5, loc="upper left")
fig.tight_layout(); fig.savefig(OUTPUT_DIR / "v22_corrections.png", bbox_inches="tight"); plt.close(fig)

print("Готово: 10 визуализаций М2 сохранены в", OUTPUT_DIR)
